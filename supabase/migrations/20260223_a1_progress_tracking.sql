...
CREATE POLICY "Users view own a1 progress" ON a1_progress FOR SELECT USING (auth.uid() = user_id);
CREATE POLICY "Users insert own a1 progress" ON a1_progress FOR INSERT WITH CHECK (auth.uid() = user_id);
CREATE POLICY "Users update own a1 progress" ON a1_progress FOR UPDATE USING (auth.uid() = user_id);

CREATE POLICY "Users view own a1 exercise" ON a1_exercise_results FOR SELECT USING (auth.uid() = user_id);
CREATE POLICY "Users insert own a1 exercise" ON a1_exercise_results FOR INSERT WITH CHECK (auth.uid() = user_id);

CREATE POLICY "Users view own a1 milestones" ON a1_milestones FOR SELECT USING (auth.uid() = user_id);
CREATE POLICY "System insert a1 milestones" ON a1_milestones FOR INSERT WITH CHECK (auth.uid() = user_id);
...
CREATE INDEX idx_a1_milestones_user ON a1_milestones(user_id);
