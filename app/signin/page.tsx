...
function SignInForm() {
  ...
  const callbackUrl = searchParams.get('callbackUrl') || '/curso-b2';
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-coral-600 via-peach-600 to-melon-700 flex items-center justify-center p-4">
      {/* Animated background elements */}
      <div className="absolute inset-0 overflow-hidden">
        <div className="absolute -top-40 -right-40 w-80 h-80 bg-peach-500 rounded-full mix-blend-multiply filter blur-xl opacity-70 animate-blob"></div>
        <div className="absolute -bottom-40 -left-40 w-80 h-80 bg-coral-500 rounded-full mix-blend-multiply filter blur-xl opacity-70 animate-blob animation-delay-2000"></div>
        <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-80 h-80 bg-melon-500 rounded-full mix-blend-multiply filter blur-xl opacity-70 animate-blob animation-delay-4000"></div>
      </div>
        ...
        <div className="bg-white/95 backdrop-blur-lg rounded-3xl shadow-2xl p-8 md:p-10">
          ...
          <div className="flex justify-center mb-6">
            <Link href="/" className="group">
              <div className="w-16 h-16 rounded-2xl bg-gradient-to-br from-coral-600 to-peach-600 flex items-center justify-center text-white text-3xl font-black shadow-lg group-hover:scale-105 transition-transform">
                F
              </div>
            </Link>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full bg-gradient-to-r from-coral-600 to-peach-600 text-white font-bold py-4 px-6 rounded-xl hover:shadow-xl hover:scale-[1.02] transition-all disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:scale-100"
            >
              {loading ? (
...
export default function SignInPage() {
  return (
    <Suspense fallback={
      <div className="min-h-screen bg-gradient-to-br from-coral-600 via-peach-600 to-melon-700 flex items-center justify-center">
        <div className="text-center">
