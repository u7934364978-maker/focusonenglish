...
import { getSolutionText } from "@/lib/premium-utils";
...
export default function FocusedPremiumSession({ block, onComplete, onExit }: Props) {
  ...
  const stepContent = block.content[currentStep];
  const progress = (currentStep / block.content.length) * 100;
  ...
  return (
    <div className="fixed inset-0 bg-white z-50 flex flex-col overflow-y-auto">
      ...
      {/* Content */}
      <main className="flex-1 flex flex-col justify-center px-6 py-12 overflow-hidden">
      ...
      <footer className={`p-8 md:p-12 border-t-2 border-slate-100 bg-white ${feedback ? 'invisible' : ''}`}>
