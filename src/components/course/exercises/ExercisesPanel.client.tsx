...
export default function ExercisesPanel({ data }: { data: ExercisesFile }) {
  ...
  const coreStats = useMemo(() => getCoreStats(goal, level, weekId, coreTotal), [goal, level, weekId, coreTotal, gateTick]);
  ...
  async function resetProgressUI() {
    // 🆕 C.4 - Confirmación antes de reset
  ...
  function onResetCycle() {
    ...
    const nextGate = resetPracticeCycle(goal, level, weekId);
    resetProgressUI();
    setGateTick((t) => t + 1);
    return nextGate;
  }
    ...
    <div>
      ...
      <div className="flex flex-wrap items-center gap-3 justify-between">
        <div className="flex flex-wrap items-center gap-2">
          ...
          <button
            ...
            className="inline-flex h-10 items-center justify-center rounded-2xl border border-rose-200 bg-rose-50 px-4 text-sm font-black text-rose-700 hover:bg-rose-100 transition"
          >
            🔄 Reset progreso
          </button>
        </div>
      ...
      <div className="mt-5 rounded-2xl border border-slate-200 bg-white p-4">
        ...
        <div className="mt-2 text-xs text-slate-500">
          💾 Guardado automáticamente en este dispositivo
        </div>
      </div>
      ...
      <div className="mt-6 rounded-2xl border border-slate-200 bg-slate-50 p-5">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            ...
            <div className="mt-2 text-xs text-slate-600 space-y-1">
              <div>Ciclos usados: {gate.cyclesUsed}/{gate.maxCycles}</div>
              {gate.lastAccuracy !== null && (
                <div>Último resultado: {pct(gate.lastAccuracy)}%</div>
              )}
              <div className="font-bold">
                ✅ Requisito: completar todo el CORE con ≥{pct(gate.passThreshold)}%
              </div>
            </div>
          </div>
          ...
          <div className="flex flex-wrap gap-2">
            <button
              ...
              className="inline-flex h-11 items-center justify-center rounded-xl bg-slate-900 px-5 text-sm font-black text-white disabled:opacity-50 hover:brightness-95 transition"
            >
              ✓ Finalizar práctica
            </button>
        ...
        {!coreStats.complete && (
          <div className="mt-4 rounded-xl bg-amber-50 border border-amber-200 p-3 text-sm text-amber-900">
            <span className="font-black">⚠️ Atención:</span> Para finalizar práctica debes intentar <strong>todos</strong> los {coreTotal} ejercicios CORE.
          </div>
        )}
      </div>
              ...
              <div className="mt-4">
                <ExerciseRenderer
                  ...
                  onResult={(isCorrect) => {
                    ...
                    const currentData = currentAnswers[ex.id] || { attempts: 0, correct: false, lastAttemptAt: 0 };
