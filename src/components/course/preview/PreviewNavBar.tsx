      ...
      <div className="max-w-7xl mx-auto px-4 py-3">
        <div className="flex items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-2 min-w-0 flex-1">
            <Link
              href="/"
              className="p-2 min-w-[44px] min-h-[44px] flex items-center justify-center hover:bg-slate-100 rounded-lg transition-colors flex-shrink-0 focus:outline-none focus:ring-2 focus:ring-coral-400 focus:ring-offset-2"
              aria-label="Home"
            >
            ...
            <div className="relative" ref={dropdownRef}>
              <button
                ...
                className="flex items-center gap-2 px-3 py-2 min-h-[44px] rounded-lg hover:bg-slate-100 transition-colors text-slate-900 font-medium focus:outline-none focus:ring-2 focus:ring-coral-400 focus:ring-offset-2"
              ...
              {isDropdownOpen && (
                <div className="absolute top-full left-0 mt-2 w-80 max-h-96 overflow-y-auto bg-white border border-slate-200 rounded-xl shadow-xl z-50" role="listbox">
                  {allUnits.map((unit) => (
                    <button
                      key={unit.unitId}
                      onClick={() => handleUnitSelect(unit.unitId)}
                      className={`w-full text-left px-4 py-3 min-h-[56px] hover:bg-slate-50 transition-colors border-b border-slate-100 last:border-b-0 focus:outline-none focus:ring-2 focus:ring-coral-400 focus:ring-inset ${
                        unit.unitId === currentUnitId ? 'bg-coral-50 hover:bg-coral-100' : ''
                      }`}
                      role="option"
                      aria-selected={unit.unitId === currentUnitId}
                    >
          ...
          <div className="flex items-center gap-2 flex-shrink-0">
            <button
              ...
              disabled={!prevUnit}
              className="p-2 min-w-[44px] min-h-[44px] flex items-center justify-center rounded-lg bg-slate-100 hover:bg-slate-200 disabled:opacity-30 disabled:cursor-not-allowed transition-all focus:outline-none focus:ring-2 focus:ring-coral-400 focus:ring-offset-2"
              aria-label={prevUnit ? `Previous unit: ${prevUnit.title}` : 'Previous unit (not available)'}
            >
            ...
            <button
              ...
              disabled={!nextUnit}
              className="p-2 min-w-[44px] min-h-[44px] flex items-center justify-center rounded-lg bg-slate-100 hover:bg-slate-200 disabled:opacity-30 disabled:cursor-not-allowed transition-all focus:outline-none focus:ring-2 focus:ring-coral-400 focus:ring-offset-2"
              aria-label={nextUnit ? `Next unit: ${nextUnit.title}` : 'Next unit (not available)'}
            >
