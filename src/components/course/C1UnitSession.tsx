...
import C1ExerciseDispatcher from './C1ExerciseDispatcher';
import { Progress } from '@/components/ui/progress';
import { Button } from '@/components/ui/button';
import Link from 'next/link';
import { useGamification } from '@/lib/hooks/use-gamification';
import { saveExerciseProgress } from '@/app/curso/ingles-c1/practica/actions';

interface C1UnitSessionProps {
  unit: Lesson;
}

export default function C1UnitSession({ unit }: C1UnitSessionProps) {
  ...
  const handleBlockComplete = (blockScore: number) => {
    ...
    completeExercise(`c1-block-${unit.id}-${currentBlockIndex}`, blockScore, 150); // C1 gives more XP
    ...
    <div className="min-h-screen bg-slate-50 pb-20">
      <div className="sticky top-0 z-50 bg-white/80 backdrop-blur-md border-b border-slate-200 px-4 py-4">
        <div className="max-w-5xl mx-auto flex items-center justify-between gap-6">
          <Link href="/curso/ingles-c1" className="flex-shrink-0">
