...
import C2ExerciseDispatcher from './C2ExerciseDispatcher';
import { Progress } from '@/components/ui/progress';
import { Button } from '@/components/ui/button';
import Link from 'next/link';
import { useGamification } from '@/lib/hooks/use-gamification';
import { saveExerciseProgress } from '@/app/curso/ingles-c2/practica/actions';

interface C2UnitSessionProps {
  unit: Lesson;
}

export default function C2UnitSession({ unit }: C2UnitSessionProps) {
  ...
  const handleQuestionCorrect = async (questionId: string) => {
    // Persistent progress in DB
    await saveExerciseProgress(questionId);
  };
  ...
  const handleBlockComplete = (blockScore: number) => {
    ...
    completeExercise(`c2-block-${unit.id}-${currentBlockIndex}`, blockScore, 200); // C2 gives even more XP
    ...
    if (newCompletedBlocks.size === totalBlocks) {
      ...
      }, 1000); // Small delay to see final feedback before trophy
        ...
        >
          <div className="mb-8 flex justify-center">
            <div className="relative">
              ...
              >
                <Star className="h-10 w-10 text-blue-400 fill-current" />
              </motion.div>
            </div>
          </div>
          ...
          <p className="text-xl text-slate-600 mb-8">
            Congratulations! You've completed <strong>{unit.title}</strong> and all exercises.
          </p>
    ...
    <div className="min-h-screen bg-slate-50 pb-20">
      <div className="sticky top-0 z-50 bg-white/80 backdrop-blur-md border-b border-slate-200 px-4 py-4">
        <div className="max-w-5xl mx-auto flex items-center justify-between gap-6">
          <Link href="/curso/ingles-c2" className="flex-shrink-0">
      ...
      <main className="max-w-5xl mx-auto px-4 pt-8">
        <div className="mb-8">
          <div className="flex items-center gap-3 mb-2">
            ...
            <span className="text-blue-600 text-sm font-black uppercase tracking-wider">
              {currentBlock?.type?.replace('-', ' ') || 'Practice'}
            </span>
          </div>
