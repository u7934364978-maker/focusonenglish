...
export default async function B2ExamPrepCoursePage() {
  const modules = await courseService.getModules('B2', 'examenes');

  // Fallback to empty if nothing found (e.g. before migration)
  if (!modules || modules.length === 0) {
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          <h1 className="text-2xl font-bold">Course content coming soon...</h1>
          <p>Please run the migration script to populate the database.</p>
        </div>
      </div>
    );
  }

  // Simplified title/desc from first module if needed, or static
  const courseTitle = 'B2 First (FCE) Exam Preparation - Complete Course';
  const courseDescription = 'Complete B2 First (FCE) preparation course with interactive exercises, voice recording, pronunciation evaluation, and comprehensive practice.';
