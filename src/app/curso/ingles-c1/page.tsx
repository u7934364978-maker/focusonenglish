...
import { ALL_C1_UNITS, C1_MODULES } from '@/lib/course-data-c1';

export default async function C1CoursePage() {
  const supabase = await createClient();
  const { data: { user } } = await supabase.auth.getUser();

  const userId = user?.id || 'anonymous';

  const units = ALL_C1_UNITS.map(unit => {
  ...
  const completedIds = await premiumCourseService.getC1Progress ? await premiumCourseService.getC1Progress(userId) : [];
  const completedSet = new Set(completedIds);
  
  const totalExercises = units.reduce((acc, u) => acc + u.totalExercises, 0);
  const completedExercises = Array.from(completedSet).filter(id => id.startsWith('c1-u')).length;
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="text-center mb-12">
            <h1 className="text-5xl font-black text-slate-900 mb-4">
              Curso <span className="text-[#4A90E2]">Inglés C1</span>
            </h1>
            <p className="text-xl text-slate-600">
              60 Unidades y 6,000 Ejercicios de nivel Real C1 (Advanced).
            </p>
          </div>
            ...
            <div className="space-y-6">
                ...
                <ul className="space-y-4">
                  <li className="flex gap-3">
                    ...
                    <p className="text-slate-600 text-sm">
                      Cada unidad se centra en objetivos específicos del nivel C1 Advanced.
                    </p>
                  </li>
                  <li className="flex gap-3">
                    <div className="mt-1"><CheckCircle2 size={18} className="text-green-500" /></div>
              ...
              <div className="bg-blue-50 p-6 rounded-3xl border border-blue-100">
                ...
                <p className="text-blue-700 text-sm mb-4">
                  El curso cubre gramática avanzada, vocabulario académico y preparación para el CAE.
                </p>
