...
function UnitContent() {
  ...
  const { recordResult } = useSpacedRepetition('REC_B1');
  ...
  useEffect(() => {
    ...
    async function loadUnit() {
      try {
        ...
        let unitModule: any;
        try {
          unitModule = await import('@/lib/course/recepcionista-b1/unit-' + unitNumber);
        } catch {
          unitModule = await import('../../../lib/course/recepcionista-b1/unit-' + unitNumber);
        }
  ...
  if (error) return (
    <div className="min-h-screen flex items-center justify-center bg-slate-50 p-4">
      <div className="max-w-md text-center">
        ...
        <Link href="/curso-recepcionista-b1" className="text-blue-600 font-bold hover:underline">Volver al curso</Link>
      </div>
    </div>
  );
  ...
  if (showUnitSummary) return (
    <div className="min-h-screen flex items-center justify-center bg-slate-50 p-4">
      <div className="max-w-md w-full bg-white rounded-3xl shadow-xl p-10 text-center">
        ...
        />
        <Link href="/curso-recepcionista-b1" className="mt-4 block text-slate-500 hover:text-slate-700 font-semibold text-sm">
          ← Volver al curso
        </Link>
      </div>
    </div>
  ...
  if (!exercises.length) return (
    <div className="min-h-screen flex items-center justify-center bg-slate-50">
      <div className="text-center">
        ...
        <Link href="/curso-recepcionista-b1" className="text-blue-600 font-bold hover:underline">Volver</Link>
      </div>
    </div>
  );
    ...
    <div className="min-h-screen bg-slate-50">
      <nav className="sticky top-0 z-20 bg-white border-b border-slate-200 shadow-sm">
        <div className="max-w-4xl mx-auto px-4 py-3 flex items-center justify-between gap-4">
          <Link href="/curso-recepcionista-b1" className="flex items-center gap-2 text-slate-600 hover:text-slate-900 font-semibold text-sm">
            ← Curso
          </Link>
