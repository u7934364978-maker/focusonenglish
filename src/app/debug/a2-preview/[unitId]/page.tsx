...
export default function A2UnitPreviewPage() {
  ...
  useEffect(() => {
    async function loadUnit() {
      try {
        ...
        const module = await import(`@/lib/course/a2/unit-${unitNumber}`);
  ...
  if (loading) return <div className="p-8 text-center">Cargando Unidad {unitId} de A2...</div>;
  if (exercises.length === 0) return <div className="p-8 text-center text-red-500">No se encontraron ejercicios para la {unitId} de A2</div>;
      ...
      <nav className="bg-white border-b p-4 flex justify-between items-center sticky top-0 z-50">
        <div className="flex items-center gap-4">
          ...
          <h1 className="font-black text-xl text-slate-800 uppercase tracking-tight">
            A2 Preview: Unidad {unitId.replace('unit-', '')} 
