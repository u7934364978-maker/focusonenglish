...
export default async function ProfessionalLessonPage({ params }: Props) {
  const { sector, level, trimester, weekId, lessonId } = await params;
  
  const lesson = await localCourseService.getProfessionalLesson(sector, level, trimester, weekId, lessonId);
