...
export class UltraAdaptiveEngine {
  ...
  static async getNextExercise(userId: string): Promise<IndexedInteraction | null> {
    ...
    const fallback = this.provider.getInteractionsByLevel(state.current_level);
    return fallback.length > 0 ? this.pickRandom(fallback) : this.pickRandom(this.provider.getAllInteractions());
  }
