import { Exercise } from '@/lib/exercise-generator';

export const UNIT_13_EXERCISES: Exercise[] = [
  // --- SUB-TOPIC 1: Social Issues & Problems (1-10) ---
  {
    id: "b1-u13-e1",
    type: "flashcard",
    level: "B1",
    topic: "Society & Culture",
    topicName: "Vocabulary",
    difficulty: "easy",
    content: {
      items: [
        {
          front: "Poverty",
          back: "Pobreza",
          pronunciation: "/ˈpɒv.ə.ti/",
          example: "The government is trying to reduce poverty in rural areas.",
          explanation: "El estado de ser extremadamente pobre."
        }
      ]
    } as any
  },
  {
    id: "b1-u13-e2",
    type: "flashcard",
    level: "B1",
    topic: "Society & Culture",
    topicName: "Vocabulary",
    difficulty: "easy",
    content: {
      items: [
        {
          front: "Unemployment",
          back: "Desempleo",
          pronunciation: "/ˌʌn.ɪmˈplɔɪ.mənt/",
          example: "Unemployment rates have increased during the economic crisis.",
          explanation: "El número de personas que no tienen trabajo."
        }
      ]
    } as any
  },
  {
    id: "b1-u13-e3",
    type: "flashcard",
    level: "B1",
    topic: "Society & Culture",
    topicName: "Vocabulary",
    difficulty: "easy",
    content: {
      items: [
        {
          front: "Crime",
          back: "Crimen / Delincuencia",
          pronunciation: "/kraɪm/",
          example: "The city has seen a decrease in violent crime recently.",
          explanation: "Actividades ilegales en general."
        }
      ]
    } as any
  },
  {
    id: "b1-u13-e4",
    type: "flashcard",
    level: "B1",
    topic: "Society & Culture",
    topicName: "Vocabulary",
    difficulty: "easy",
    content: {
      items: [
        {
          front: "Pollution",
          back: "Contaminación",
          pronunciation: "/pəˈluː.ʃən/",
          example: "Air pollution is a major problem in big cities.",
          explanation: "Daño causado al agua, aire, etc. por sustancias dañinas."
        }
      ]
    } as any
  },
  {
    id: "b1-u13-e5",
    type: "flashcard",
    level: "B1",
    topic: "Society & Culture",
    topicName: "Vocabulary",
    difficulty: "easy",
    content: {
      items: [
        {
          front: "Homelessness",
          back: "Falta de vivienda / Personas sin hogar",
          pronunciation: "/ˈhəʊm.ləs.nəs/",
          example: "Charities are working to tackle the issue of homelessness.",
          explanation: "El estado de no tener un lugar donde vivir."
        }
      ]
    } as any
  },
  {
    id: "b1-u13-e6",
    type: "flashcard",
    level: "B1",
    topic: "Society & Culture",
    topicName: "Vocabulary",
    difficulty: "medium",
    content: {
      items: [
        {
          front: "Inequality",
          back: "Inequidad / Desigualdad",
          pronunciation: "/ˌɪn.ɪˈkwɒl.ə.ti/",
          example: "There is still significant gender inequality in the workplace.",
          explanation: "La situación de no ser igual, especialmente en estatus, derechos y oportunidades."
        }
      ]
    } as any
  },
  {
    id: "b1-u13-e7",
    type: "flashcard",
    level: "B1",
    topic: "Society & Culture",
    topicName: "Vocabulary",
    difficulty: "medium",
    content: {
      items: [
        {
          front: "Discrimination",
          back: "Discriminación",
          pronunciation: "/dɪˌskrɪm.ɪˈneɪ.ʃən/",
          example: "Laws have been passed to prevent discrimination based on race.",
          explanation: "Trato injusto de una persona o grupo basado en prejuicios."
        }
      ]
    } as any
  },
  {
    id: "b1-u13-e8",
    type: "flashcard",
    level: "B1",
    topic: "Society & Culture",
    topicName: "Vocabulary",
    difficulty: "medium",
    content: {
      items: [
        {
          front: "Corruption",
          back: "Corrupción",
          pronunciation: "/kəˈrʌp.ʃən/",
          example: "The investigation revealed widespread corruption in the police force.",
          explanation: "Comportamiento deshonesto o ilegal por parte de personas en posiciones de poder."
        }
      ]
    } as any
  },
  {
    id: "b1-u13-e9",
    type: "flashcard",
    level: "B1",
    topic: "Society & Culture",
    topicName: "Vocabulary",
    difficulty: "medium",
    content: {
      items: [
        {
          front: "Bullying",
          back: "Acoso",
          pronunciation: "/ˈbʊl.i.ɪŋ/",
          example: "The school has a strict policy against bullying.",
          explanation: "El uso de la fuerza, amenazas o coacción para abusar o intimidar a otros."
        }
      ]
    } as any
  },
  {
    id: "b1-u13-e10",
    type: "flashcard",
    level: "B1",
    topic: "Society & Culture",
    topicName: "Vocabulary",
    difficulty: "medium",
    content: {
      items: [
        {
          front: "Global warming",
          back: "Calentamiento global",
          pronunciation: "/ˌɡləʊ.bəl ˈwɔː.mɪŋ/",
          example: "Global warming is causing the ice caps to melt.",
          explanation: "Un aumento gradual en la temperatura general de la atmósfera terrestre."
        }
      ]
    } as any
  },

  // --- SUB-TOPIC 2: Customs & Traditions (11-20) ---
  {
    id: "b1-u13-e11",
    type: "flashcard",
    level: "B1",
    topic: "Society & Culture",
    topicName: "Vocabulary",
    difficulty: "easy",
    content: {
      items: [
        {
          front: "Tradition",
          back: "Tradición",
          pronunciation: "/trəˈdɪʃ.ən/",
          example: "It's a tradition in our family to have a big dinner on Sundays.",
          explanation: "Una costumbre o creencia que se ha transmitido de generación en generación."
        }
      ]
    } as any
  },
  {
    id: "b1-u13-e12",
    type: "flashcard",
    level: "B1",
    topic: "Society & Culture",
    topicName: "Vocabulary",
    difficulty: "easy",
    content: {
      items: [
        {
          front: "Custom",
          back: "Costumbre",
          pronunciation: "/ˈkʌs.təm/",
          example: "It's a local custom to shake hands when you meet someone.",
          explanation: "Una forma de comportamiento tradicional o habitual entre personas en una sociedad."
        }
      ]
    } as any
  },
  {
    id: "b1-u13-e13",
    type: "flashcard",
    level: "B1",
    topic: "Society & Culture",
    topicName: "Vocabulary",
    difficulty: "easy",
    content: {
      items: [
        {
          front: "Belief",
          back: "Creencia",
          pronunciation: "/bɪˈliːf/",
          example: "She has a strong belief in the importance of education.",
          explanation: "Algo que aceptas como verdadero o real."
        }
      ]
    } as any
  },
  {
    id: "b1-u13-e14",
    type: "flashcard",
    level: "B1",
    topic: "Society & Culture",
    topicName: "Vocabulary",
    difficulty: "easy",
    content: {
      items: [
        {
          front: "Festival",
          back: "Festival",
          pronunciation: "/ˈfes.tɪ.vəl/",
          example: "The town holds a music festival every summer.",
          explanation: "Un día o período especial de celebración."
        }
      ]
    } as any
  },
  {
    id: "b1-u13-e15",
    type: "flashcard",
    level: "B1",
    topic: "Society & Culture",
    topicName: "Vocabulary",
    difficulty: "medium",
    content: {
      items: [
        {
          front: "Heritage",
          back: "Patrimonio / Herencia cultural",
          pronunciation: "/ˈher.ɪ.tɪdʒ/",
          example: "We must protect our national heritage for future generations.",
          explanation: "Las tradiciones, logros y creencias que forman parte de la historia de un grupo o nación."
        }
      ]
    } as any
  },
  {
    id: "b1-u13-e16",
    type: "flashcard",
    level: "B1",
    topic: "Society & Culture",
    topicName: "Vocabulary",
    difficulty: "medium",
    content: {
      items: [
        {
          front: "Ceremony",
          back: "Ceremonia",
          pronunciation: "/ˈser.ɪ.mə.ni/",
          example: "The wedding ceremony was held in a beautiful old church.",
          explanation: "Un evento formal realizado en una ocasión especial."
        }
      ]
    } as any
  },
  {
    id: "b1-u13-e17",
    type: "flashcard",
    level: "B1",
    topic: "Society & Culture",
    topicName: "Vocabulary",
    difficulty: "medium",
    content: {
      items: [
        {
          front: "Stereotype",
          back: "Estereotipo",
          pronunciation: "/ˈster.i.ə.taɪp/",
          example: "The movie challenges common stereotypes about teenagers.",
          explanation: "Una idea fija y simplificada de un tipo particular de persona o cosa."
        }
      ]
    } as any
  },
  {
    id: "b1-u13-e18",
    type: "flashcard",
    level: "B1",
    topic: "Society & Culture",
    topicName: "Vocabulary",
    difficulty: "medium",
    content: {
      items: [
        {
          front: "Values",
          back: "Valores",
          pronunciation: "/ˈvæl.juːz/",
          example: "Every culture has its own set of moral values.",
          explanation: "Las creencias de una persona o grupo sobre lo que es importante y correcto."
        }
      ]
    } as any
  },
  {
    id: "b1-u13-e19",
    type: "flashcard",
    level: "B1",
    topic: "Society & Culture",
    topicName: "Vocabulary",
    difficulty: "hard",
    content: {
      items: [
        {
          front: "Etiquette",
          back: "Etiqueta / Protocolo",
          pronunciation: "/ˈet.ɪ.ket/",
          example: "Social etiquette varies from one country to another.",
          explanation: "El conjunto de reglas o costumbres para el comportamiento aceptable en un grupo social."
        }
      ]
    } as any
  },
  {
    id: "b1-u13-e20",
    type: "flashcard",
    level: "B1",
    topic: "Society & Culture",
    topicName: "Vocabulary",
    difficulty: "hard",
    content: {
      items: [
        {
          front: "Ritual",
          back: "Ritual",
          pronunciation: "/ˈrɪtʃ.u.əl/",
          example: "Drinking tea in the morning is a daily ritual for many people.",
          explanation: "Una serie de acciones realizadas siempre de la misma manera."
        }
      ]
    } as any
  },

  // --- SUB-TOPIC 3: Grammar: Reported Speech (Statements) (21-30) ---
  {
    id: "b1-u13-e21",
    type: "multiple-choice",
    level: "B1",
    topic: "Society & Culture",
    topicName: "Grammar: Reported Speech",
    difficulty: "easy",
    content: {
      question: "Direct: 'I am tired,' he said. \nReported: He said that he ________ tired.",
      options: [
        "was",
        "is",
        "am",
        "has been"
      ],
      correctAnswer: 0,
      explanation: "El presente simple cambia a pasado simple en estilo indirecto."
    } as any
  },
  {
    id: "b1-u13-e22",
    type: "multiple-choice",
    level: "B1",
    topic: "Society & Culture",
    topicName: "Grammar: Reported Speech",
    difficulty: "easy",
    content: {
      question: "Direct: 'I have finished,' she said. \nReported: She said that she ________ finished.",
      options: [
        "had",
        "has",
        "was",
        "having"
      ],
      correctAnswer: 0,
      explanation: "El presente perfecto cambia a pasado perfecto en estilo indirecto."
    } as any
  },
  {
    id: "b1-u13-e23",
    type: "multiple-choice",
    level: "B1",
    topic: "Society & Culture",
    topicName: "Grammar: Reported Speech",
    difficulty: "easy",
    content: {
      question: "Direct: 'We will help you,' they said. \nReported: They said that they ________ help us.",
      options: [
        "would",
        "will",
        "can",
        "should"
      ],
      correctAnswer: 0,
      explanation: "'Will' cambia a 'would' en estilo indirecto."
    } as any
  },
  {
    id: "b1-u13-e24",
    type: "multiple-choice",
    level: "B1",
    topic: "Society & Culture",
    topicName: "Grammar: Reported Speech",
    difficulty: "medium",
    content: {
      question: "Direct: 'I'm watching TV,' she told me. \nReported: She told me that she ________ TV.",
      options: [
        "was watching",
        "is watching",
        "had watched",
        "watches"
      ],
      correctAnswer: 0,
      explanation: "El presente continuo cambia a pasado continuo."
    } as any
  },
  {
    id: "b1-u13-e25",
    type: "multiple-choice",
    level: "B1",
    topic: "Society & Culture",
    topicName: "Grammar: Reported Speech",
    difficulty: "medium",
    content: {
      question: "Direct: 'I went to the museum,' he said. \nReported: He said that he ________ to the museum.",
      options: [
        "had gone",
        "went",
        "has gone",
        "goes"
      ],
      correctAnswer: 0,
      explanation: "El pasado simple cambia a pasado perfecto."
    } as any
  },
  {
    id: "b1-u13-e26",
    type: "multiple-choice",
    level: "B1",
    topic: "Society & Culture",
    topicName: "Grammar: Reported Speech",
    difficulty: "medium",
    content: {
      question: "Direct: 'You can leave,' the boss said. \nReported: The boss said that I ________ leave.",
      options: [
        "could",
        "can",
        "must",
        "will"
      ],
      correctAnswer: 0,
      explanation: "'Can' cambia a 'could'."
    } as any
  },
  {
    id: "b1-u13-e27",
    type: "multiple-choice",
    level: "B1",
    topic: "Society & Culture",
    topicName: "Grammar: Reported Speech",
    difficulty: "medium",
    content: {
      question: "Direct: 'I'll see you tomorrow,' she said. \nReported: She said she'd see me ________.",
      options: [
        "the following day",
        "tomorrow",
        "yesterday",
        "today"
      ],
      correctAnswer: 0,
      explanation: "Las expresiones de tiempo también cambian: 'tomorrow' pasa a 'the following day' o 'the next day'."
    } as any
  },
  {
    id: "b1-u13-e28",
    type: "multiple-choice",
    level: "B1",
    topic: "Society & Culture",
    topicName: "Grammar: Reported Speech",
    difficulty: "hard",
    content: {
      question: "Direct: 'I've been working hard,' he said. \nReported: He said that he ________ hard.",
      options: [
        "had been working",
        "has been working",
        "was working",
        "is working"
      ],
      correctAnswer: 0,
      explanation: "El presente perfecto continuo cambia a pasado perfecto continuo."
    } as any
  },
  {
    id: "b1-u13-e29",
    type: "multiple-choice",
    level: "B1",
    topic: "Society & Culture",
    topicName: "Grammar: Reported Speech",
    difficulty: "hard",
    content: {
      question: "Direct: 'I must leave now,' she said. \nReported: She said that she ________ then.",
      options: [
        "had to leave",
        "must leave",
        "musted leave",
        "has to leave"
      ],
      correctAnswer: 0,
      explanation: "'Must' suele cambiar a 'had to' y 'now' a 'then'."
    } as any
  },
  {
    id: "b1-u13-e30",
    type: "multiple-choice",
    level: "B1",
    topic: "Society & Culture",
    topicName: "Grammar: Reported Speech",
    difficulty: "hard",
    content: {
      question: "Direct: 'I lived here for ten years,' he said. \nReported: He said that he had lived ________ for ten years.",
      options: [
        "there",
        "here",
        "that",
        "this"
      ],
      correctAnswer: 0,
      explanation: "'Here' cambia a 'there' en estilo indirecto."
    } as any
  },

  // --- SUB-TOPIC 4: Grammar: Reported Speech (Questions & Commands) (31-40) ---
  {
    id: "b1-u13-e31",
    type: "multiple-choice",
    level: "B1",
    topic: "Society & Culture",
    topicName: "Grammar: Reported Questions",
    difficulty: "medium",
    content: {
      question: "Direct: 'Where do you live?' he asked. \nReported: He asked me where I ________.",
      options: [
        "lived",
        "live",
        "do live",
        "was living"
      ],
      correctAnswer: 0,
      explanation: "En preguntas indirectas usamos el orden de afirmación (sujeto + verbo) y cambiamos el tiempo verbal."
    } as any
  },
  {
    id: "b1-u13-e32",
    type: "multiple-choice",
    level: "B1",
    topic: "Society & Culture",
    topicName: "Grammar: Reported Questions",
    difficulty: "medium",
    content: {
      question: "Direct: 'Do you like tea?' she asked. \nReported: She asked me ________ I liked tea.",
      options: [
        "if",
        "that",
        "whether if",
        "do"
      ],
      correctAnswer: 0,
      explanation: "Para preguntas de sí/no usamos 'if' o 'whether'."
    } as any
  },
  {
    id: "b1-u13-e33",
    type: "multiple-choice",
    level: "B1",
    topic: "Society & Culture",
    topicName: "Grammar: Reported Questions",
    difficulty: "medium",
    content: {
      question: "Direct: 'Why did you arrive late?' they asked. \nReported: They asked me why I ________ late.",
      options: [
        "had arrived",
        "arrived",
        "did arrive",
        "have arrived"
      ],
      correctAnswer: 0,
      explanation: "Pasado simple cambia a pasado perfecto en estilo indirecto."
    } as any
  },
  {
    id: "b1-u13-e34",
    type: "multiple-choice",
    level: "B1",
    topic: "Society & Culture",
    topicName: "Grammar: Reported Commands",
    difficulty: "medium",
    content: {
      question: "Direct: 'Sit down!' the teacher said. \nReported: The teacher told us ________ down.",
      options: [
        "to sit",
        "sit",
        "that sit",
        "sitting"
      ],
      correctAnswer: 0,
      explanation: "Para órdenes usamos 'told/ordered + object + to + infinitive'."
    } as any
  },
  {
    id: "b1-u13-e35",
    type: "multiple-choice",
    level: "B1",
    topic: "Society & Culture",
    topicName: "Grammar: Reported Commands",
    difficulty: "medium",
    content: {
      question: "Direct: 'Don't talk!' she said. \nReported: She told me ________.",
      options: [
        "not to talk",
        "don't talk",
        "to not talk",
        "not talk"
      ],
      correctAnswer: 0,
      explanation: "Para órdenes negativas usamos 'not to + infinitive'."
    } as any
  },
  {
    id: "b1-u13-e36",
    type: "multiple-choice",
    level: "B1",
    topic: "Society & Culture",
    topicName: "Grammar: Reported Questions",
    difficulty: "hard",
    content: {
      question: "Direct: 'Can you help me?' he asked. \nReported: He asked me if I ________ help him.",
      options: [
        "could",
        "can",
        "would",
        "might"
      ],
      correctAnswer: 0,
      explanation: "'Can' cambia a 'could'."
    } as any
  },
  {
    id: "b1-u13-e37",
    type: "multiple-choice",
    level: "B1",
    topic: "Society & Culture",
    topicName: "Grammar: Reported Questions",
    difficulty: "hard",
    content: {
      question: "Direct: 'When will the train arrive?' she asked. \nReported: She asked when the train ________ arrive.",
      options: [
        "would",
        "will",
        "did",
        "was"
      ],
      correctAnswer: 0,
      explanation: "Orden de afirmación y cambio de 'will' a 'would'."
    } as any
  },
  {
    id: "b1-u13-e38",
    type: "multiple-choice",
    level: "B1",
    topic: "Society & Culture",
    topicName: "Grammar: Reported Requests",
    difficulty: "hard",
    content: {
      question: "Direct: 'Please, lend me some money,' he said. \nReported: He asked me ________ some money.",
      options: [
        "to lend him",
        "that I lend him",
        "lend him",
        "to lend me"
      ],
      correctAnswer: 0,
      explanation: "Para peticiones usamos 'asked + object + to + infinitive'."
    } as any
  },
  {
    id: "b1-u13-e39",
    type: "multiple-choice",
    level: "B1",
    topic: "Society & Culture",
    topicName: "Grammar: Reported Commands",
    difficulty: "hard",
    content: {
      question: "Direct: 'Go home immediately!' the officer said. \nReported: The officer ordered us ________ home immediately.",
      options: [
        "to go",
        "go",
        "going",
        "that we go"
      ],
      correctAnswer: 0,
      explanation: "Uso de infinitivo con 'to' tras el verbo de reporte 'ordered'."
    } as any
  },
  {
    id: "b1-u13-e40",
    type: "multiple-choice",
    level: "B1",
    topic: "Society & Culture",
    topicName: "Grammar: Reported Questions",
    difficulty: "hard",
    content: {
      question: "Direct: 'Have you seen my keys?' he asked. \nReported: He asked me whether I ________ his keys.",
      options: [
        "had seen",
        "have seen",
        "saw",
        "was seeing"
      ],
      correctAnswer: 0,
      explanation: "Pregunta de sí/no en presente perfecto cambia a pasado perfecto con 'whether'."
    } as any
  },

  // --- SUB-TOPIC 5: Politics & Government (41-50) ---
  {
    id: "b1-u13-e41",
    type: "multiple-choice",
    level: "B1",
    topic: "Society & Culture",
    topicName: "Politics",
    difficulty: "easy",
    content: {
      question: "A ________ is an official process to choose a leader or government.",
      options: [
        "election",
        "party",
        "campaign",
        "debate"
      ],
      correctAnswer: 0,
      explanation: "Una elección es el proceso de votación."
    } as any
  },
  {
    id: "b1-u13-e42",
    type: "multiple-choice",
    level: "B1",
    topic: "Society & Culture",
    topicName: "Politics",
    difficulty: "easy",
    content: {
      question: "To ________ means to express your choice in an election.",
      options: [
        "vote",
        "elect",
        "select",
        "run"
      ],
      correctAnswer: 0,
      explanation: "Votar es la acción de elegir."
    } as any
  },
  {
    id: "b1-u13-e43",
    type: "multiple-choice",
    level: "B1",
    topic: "Society & Culture",
    topicName: "Politics",
    difficulty: "easy",
    content: {
      question: "The ________ is the group of people who control a country.",
      options: [
        "government",
        "opposition",
        "citizen",
        "candidate"
      ],
      correctAnswer: 0,
      explanation: "'Government' es el gobierno."
    } as any
  },
  {
    id: "b1-u13-e44",
    type: "multiple-choice",
    level: "B1",
    topic: "Society & Culture",
    topicName: "Politics",
    difficulty: "medium",
    content: {
      question: "A ________ is a person who is running for a political position.",
      options: [
        "candidate",
        "politician",
        "president",
        "mayor"
      ],
      correctAnswer: 0,
      explanation: "Un candidato es quien se postula."
    } as any
  },
  {
    id: "b1-u13-e45",
    type: "multiple-choice",
    level: "B1",
    topic: "Society & Culture",
    topicName: "Politics",
    difficulty: "medium",
    content: {
      question: "A political ________ is an organization with shared political ideas.",
      options: [
        "party",
        "club",
        "union",
        "force"
      ],
      correctAnswer: 0,
      explanation: "'Party' significa partido político."
    } as any
  },
  {
    id: "b1-u13-e46",
    type: "multiple-choice",
    level: "B1",
    topic: "Society & Culture",
    topicName: "Politics",
    difficulty: "medium",
    content: {
      question: "A ________ is a person who lives in a particular country and has rights there.",
      options: [
        "citizen",
        "tourist",
        "foreigner",
        "refugee"
      ],
      correctAnswer: 0,
      explanation: "'Citizen' es ciudadano."
    } as any
  },
  {
    id: "b1-u13-e47",
    type: "multiple-choice",
    level: "B1",
    topic: "Society & Culture",
    topicName: "Politics",
    difficulty: "medium",
    content: {
      question: "The ________ is the leader of a city or town.",
      options: [
        "mayor",
        "president",
        "minister",
        "governor"
      ],
      correctAnswer: 0,
      explanation: "'Mayor' es el alcalde."
    } as any
  },
  {
    id: "b1-u13-e48",
    type: "multiple-choice",
    level: "B1",
    topic: "Society & Culture",
    topicName: "Politics",
    difficulty: "hard",
    content: {
      question: "________ is a system of government where people choose their leaders.",
      options: [
        "Democracy",
        "Monarchy",
        "Dictatorship",
        "Republic"
      ],
      correctAnswer: 0,
      explanation: "La democracia se basa en la elección popular."
    } as any
  },
  {
    id: "b1-u13-e49",
    type: "multiple-choice",
    level: "B1",
    topic: "Society & Culture",
    topicName: "Politics",
    difficulty: "hard",
    content: {
      question: "The Prime Minister ________ that the taxes would be reduced.",
      options: [
        "announced",
        "told",
        "spoke",
        "talked"
      ],
      correctAnswer: 0,
      explanation: "El verbo 'announced' (anunciar) es adecuado para declaraciones oficiales."
    } as any
  },
  {
    id: "b1-u13-e50",
    type: "multiple-choice",
    level: "B1",
    topic: "Society & Culture",
    topicName: "Politics",
    difficulty: "hard",
    content: {
      question: "Political ________ are held to inform people about candidates' ideas.",
      options: [
        "campaigns",
        "elections",
        "ballots",
        "parliaments"
      ],
      correctAnswer: 0,
      explanation: "Una campaña política sirve para promocionar ideas."
    } as any
  },

  // --- SUB-TOPIC 6: Law & Justice (51-60) ---
  {
    id: "b1-u13-e51",
    type: "multiple-choice",
    level: "B1",
    topic: "Society & Culture",
    topicName: "Law & Justice",
    difficulty: "easy",
    content: {
      question: "A ________ is a person who helps people with legal problems.",
      options: [
        "lawyer",
        "judge",
        "police officer",
        "witness"
      ],
      correctAnswer: 0,
      explanation: "Un abogado (lawyer) asesora en temas legales."
    } as any
  },
  {
    id: "b1-u13-e52",
    type: "multiple-choice",
    level: "B1",
    topic: "Society & Culture",
    topicName: "Law & Justice",
    difficulty: "easy",
    content: {
      question: "The ________ is the place where legal cases are decided.",
      options: [
        "court",
        "prison",
        "station",
        "office"
      ],
      correctAnswer: 0,
      explanation: "Los casos se deciden en el tribunal (court)."
    } as any
  },
  {
    id: "b1-u13-e53",
    type: "multiple-choice",
    level: "B1",
    topic: "Society & Culture",
    topicName: "Law & Justice",
    difficulty: "medium",
    content: {
      question: "The ________ decides the punishment for a criminal.",
      options: [
        "judge",
        "lawyer",
        "victim",
        "suspect"
      ],
      correctAnswer: 0,
      explanation: "El juez (judge) dicta la sentencia."
    } as any
  },
  {
    id: "b1-u13-e54",
    type: "multiple-choice",
    level: "B1",
    topic: "Society & Culture",
    topicName: "Law & Justice",
    difficulty: "medium",
    content: {
      question: "The ________ is a group of citizens who decide if someone is guilty.",
      options: [
        "jury",
        "cabinet",
        "committee",
        "board"
      ],
      correctAnswer: 0,
      explanation: "El jurado (jury) determina la culpabilidad."
    } as any
  },
  {
    id: "b1-u13-e55",
    type: "multiple-choice",
    level: "B1",
    topic: "Society & Culture",
    topicName: "Law & Justice",
    difficulty: "medium",
    content: {
      question: "A ________ is a person who has seen a crime.",
      options: [
        "witness",
        "criminal",
        "lawyer",
        "judge"
      ],
      correctAnswer: 0,
      explanation: "Un testigo (witness) presencia los hechos."
    } as any
  },
  {
    id: "b1-u13-e56",
    type: "multiple-choice",
    level: "B1",
    topic: "Society & Culture",
    topicName: "Law & Justice",
    difficulty: "medium",
    content: {
      question: "To ________ someone means to take them to the police station for a crime.",
      options: [
        "arrest",
        "accuse",
        "convict",
        "sentence"
      ],
      correctAnswer: 0,
      explanation: "Arrestar es detener a alguien."
    } as any
  },
  {
    id: "b1-u13-e57",
    type: "multiple-choice",
    level: "B1",
    topic: "Society & Culture",
    topicName: "Law & Justice",
    difficulty: "hard",
    content: {
      question: "If you are ________, it means you have committed a crime.",
      options: [
        "guilty",
        "innocent",
        "fair",
        "legal"
      ],
      correctAnswer: 0,
      explanation: "'Guilty' significa culpable."
    } as any
  },
  {
    id: "b1-u13-e58",
    type: "multiple-choice",
    level: "B1",
    topic: "Society & Culture",
    topicName: "Law & Justice",
    difficulty: "hard",
    content: {
      question: "The ________ is the punishment given by a judge.",
      options: [
        "sentence",
        "trial",
        "verdict",
        "charge"
      ],
      correctAnswer: 0,
      explanation: "'Sentence' es la condena o sentencia."
    } as any
  },
  {
    id: "b1-u13-e59",
    type: "multiple-choice",
    level: "B1",
    topic: "Society & Culture",
    topicName: "Law & Justice",
    difficulty: "hard",
    content: {
      question: "He was ________ to five years in prison.",
      options: [
        "sentenced",
        "arrested",
        "charged",
        "accused"
      ],
      correctAnswer: 0,
      explanation: "Se usa 'sentenced to' para indicar la duración de la condena."
    } as any
  },
  {
    id: "b1-u13-e60",
    type: "multiple-choice",
    level: "B1",
    topic: "Society & Culture",
    topicName: "Law & Justice",
    difficulty: "hard",
    content: {
      question: "The police are ________ the cause of the fire.",
      options: [
        "investigating",
        "sentencing",
        "arresting",
        "judging"
      ],
      correctAnswer: 0,
      explanation: "Investigar (investigating) es buscar la causa."
    } as any
  },

  // --- SUB-TOPIC 7: Multiculturalism & Diversity (61-70) ---
  {
    id: "b1-u13-e61",
    type: "multiple-choice",
    level: "B1",
    topic: "Society & Culture",
    topicName: "Diversity",
    difficulty: "easy",
    content: {
      question: "A ________ society has people from many different cultures.",
      options: [
        "multicultural",
        "traditional",
        "conservative",
        "global"
      ],
      correctAnswer: 0,
      explanation: "'Multicultural' indica diversidad de culturas."
    } as any
  },
  {
    id: "b1-u13-e62",
    type: "multiple-choice",
    level: "B1",
    topic: "Society & Culture",
    topicName: "Diversity",
    difficulty: "easy",
    content: {
      question: "We should respect people regardless of their ________.",
      options: [
        "background",
        "height",
        "hobbies",
        "shoes"
      ],
      correctAnswer: 0,
      explanation: "'Background' se refiere al origen o historia personal."
    } as any
  },
  {
    id: "b1-u13-e63",
    type: "multiple-choice",
    level: "B1",
    topic: "Society & Culture",
    topicName: "Diversity",
    difficulty: "medium",
    content: {
      question: "Many people ________ to other countries to find better jobs.",
      options: [
        "migrate",
        "visit",
        "travel",
        "stay"
      ],
      correctAnswer: 0,
      explanation: "'Migrate' (migrar) implica mudarse a otro país."
    } as any
  },
  {
    id: "b1-u13-e64",
    type: "multiple-choice",
    level: "B1",
    topic: "Society & Culture",
    topicName: "Diversity",
    difficulty: "medium",
    content: {
      question: "Cultural ________ makes a society more interesting.",
      options: [
        "diversity",
        "unity",
        "poverty",
        "identity"
      ],
      correctAnswer: 0,
      explanation: "'Diversity' (diversidad) aporta riqueza cultural."
    } as any
  },
  {
    id: "b1-u13-e65",
    type: "multiple-choice",
    level: "B1",
    topic: "Society & Culture",
    topicName: "Diversity",
    difficulty: "medium",
    content: {
      question: "People should have equal ________ regardless of their gender.",
      options: [
        "opportunities",
        "problems",
        "clothes",
        "languages"
      ],
      correctAnswer: 0,
      explanation: "'Opportunities' (oportunidades) deben ser iguales para todos."
    } as any
  },
  {
    id: "b1-u13-e66",
    type: "multiple-choice",
    level: "B1",
    topic: "Society & Culture",
    topicName: "Diversity",
    difficulty: "medium",
    content: {
      question: "Learning a new language helps you understand another ________.",
      options: [
        "culture",
        "alphabet",
        "sentence",
        "voice"
      ],
      correctAnswer: 0,
      explanation: "El idioma es una puerta a la cultura."
    } as any
  },
  {
    id: "b1-u13-e67",
    type: "multiple-choice",
    level: "B1",
    topic: "Society & Culture",
    topicName: "Diversity",
    difficulty: "hard",
    content: {
      question: "________ is the belief that some races are better than others.",
      options: [
        "Racism",
        "Sexist",
        "Feminism",
        "Nationalism"
      ],
      correctAnswer: 0,
      explanation: "El racismo es esa creencia discriminatoria."
    } as any
  },
  {
    id: "b1-u13-e68",
    type: "multiple-choice",
    level: "B1",
    topic: "Society & Culture",
    topicName: "Diversity",
    difficulty: "hard",
    content: {
      question: "It is important to promote ________ in the workplace.",
      options: [
        "inclusion",
        "exclusion",
        "pollution",
        "reduction"
      ],
      correctAnswer: 0,
      explanation: "'Inclusion' (inclusión) es integrar a todos."
    } as any
  },
  {
    id: "b1-u13-e69",
    type: "multiple-choice",
    level: "B1",
    topic: "Society & Culture",
    topicName: "Diversity",
    difficulty: "hard",
    content: {
      question: "________ is having a preconceived opinion that is not based on reason.",
      options: [
        "Prejudice",
        "Justice",
        "Honesty",
        "Wisdom"
      ],
      correctAnswer: 0,
      explanation: "'Prejudice' (prejuicio) es una opinión sin base real."
    } as any
  },
  {
    id: "b1-u13-e70",
    type: "multiple-choice",
    level: "B1",
    topic: "Society & Culture",
    topicName: "Diversity",
    difficulty: "hard",
    content: {
      question: "The country is known for its religious ________.",
      options: [
        "tolerance",
        "conflict",
        "problem",
        "issue"
      ],
      correctAnswer: 0,
      explanation: "'Tolerance' (tolerancia) es el respeto a otras creencias."
    } as any
  },

  // --- SUB-TOPIC 8: Religion & Beliefs (71-80) ---
  {
    id: "b1-u13-e71",
    type: "multiple-choice",
    level: "B1",
    topic: "Society & Culture",
    topicName: "Religion",
    difficulty: "easy",
    content: {
      question: "A ________ is a person who follows a religion.",
      options: [
        "believer",
        "leader",
        "speaker",
        "player"
      ],
      correctAnswer: 0,
      explanation: "Un creyente (believer) profesa una fe."
    } as any
  },
  {
    id: "b1-u13-e72",
    type: "multiple-choice",
    level: "B1",
    topic: "Society & Culture",
    topicName: "Religion",
    difficulty: "easy",
    content: {
      question: "People go to a ________ to pray in many religions.",
      options: [
        "temple",
        "stadium",
        "factory",
        "market"
      ],
      correctAnswer: 0,
      explanation: "Un templo (temple) es un lugar de oración."
    } as any
  },
  {
    id: "b1-u13-e73",
    type: "multiple-choice",
    level: "B1",
    topic: "Society & Culture",
    topicName: "Religion",
    difficulty: "medium",
    content: {
      question: "________ is the belief in one or more gods.",
      options: [
        "Religion",
        "Science",
        "Politics",
        "Culture"
      ],
      correctAnswer: 0,
      explanation: "La religión trata sobre la fe en divinidades."
    } as any
  },
  {
    id: "b1-u13-e74",
    type: "multiple-choice",
    level: "B1",
    topic: "Society & Culture",
    topicName: "Religion",
    difficulty: "medium",
    content: {
      question: "A ________ is a formal religious act.",
      options: [
        "ritual",
        "habit",
        "meeting",
        "speech"
      ],
      correctAnswer: 0,
      explanation: "Un ritual es un acto religioso formal."
    } as any
  },
  {
    id: "b1-u13-e75",
    type: "multiple-choice",
    level: "B1",
    topic: "Society & Culture",
    topicName: "Religion",
    difficulty: "medium",
    content: {
      question: "He has a lot of ________ in his own abilities.",
      options: [
        "faith",
        "religion",
        "ritual",
        "prayer"
      ],
      correctAnswer: 0,
      explanation: "'Faith' (fe/confianza) puede aplicarse a uno mismo."
    } as any
  },
  {
    id: "b1-u13-e76",
    type: "multiple-choice",
    level: "B1",
    topic: "Society & Culture",
    topicName: "Religion",
    difficulty: "medium",
    content: {
      question: "The ________ is the holy book of Christianity.",
      options: [
        "Bible",
        "Quran",
        "Torah",
        "Vedas"
      ],
      correctAnswer: 0,
      explanation: "La Biblia es el libro sagrado cristiano."
    } as any
  },
  {
    id: "b1-u13-e77",
    type: "multiple-choice",
    level: "B1",
    topic: "Society & Culture",
    topicName: "Religion",
    difficulty: "hard",
    content: {
      question: "Freedom of ________ is a basic human right.",
      options: [
        "worship",
        "praying",
        "belief",
        "religion"
      ],
      correctAnswer: 0,
      explanation: "'Freedom of worship' (libertad de culto) es un derecho común."
    } as any
  },
  {
    id: "b1-u13-e78",
    type: "multiple-choice",
    level: "B1",
    topic: "Society & Culture",
    topicName: "Religion",
    difficulty: "hard",
    content: {
      question: "A ________ is a religious leader in some communities.",
      options: [
        "priest",
        "officer",
        "manager",
        "doctor"
      ],
      correctAnswer: 0,
      explanation: "Un sacerdote (priest) es un líder religioso."
    } as any
  },
  {
    id: "b1-u13-e79",
    type: "multiple-choice",
    level: "B1",
    topic: "Society & Culture",
    topicName: "Religion",
    difficulty: "hard",
    content: {
      question: "They ________ every day at sunrise.",
      options: [
        "pray",
        "praise",
        "preach",
        "believe"
      ],
      correctAnswer: 0,
      explanation: "'Pray' (orar/rezar) es la acción de comunicarse con lo divino."
    } as any
  },
  {
    id: "b1-u13-e80",
    type: "multiple-choice",
    level: "B1",
    topic: "Society & Culture",
    topicName: "Religion",
    difficulty: "hard",
    content: {
      question: "Spirituality is very ________ to her.",
      options: [
        "meaningful",
        "religious",
        "faithful",
        "sacred"
      ],
      correctAnswer: 0,
      explanation: "'Meaningful' significa que tiene mucho significado o importancia."
    } as any
  },

  // --- SUB-TOPIC 9: Phrasal Verbs related to Society & People (81-90) ---
  {
    id: "b1-u13-e81",
    type: "multiple-choice",
    level: "B1",
    topic: "Society & Culture",
    topicName: "Phrasal Verbs",
    difficulty: "medium",
    content: {
      question: "I ________ with my neighbors very well.",
      options: [
        "get along",
        "get up",
        "get by",
        "get in"
      ],
      correctAnswer: 0,
      explanation: "'Get along' significa llevarse bien con alguien."
    } as any
  },
  {
    id: "b1-u13-e82",
    type: "multiple-choice",
    level: "B1",
    topic: "Society & Culture",
    topicName: "Phrasal Verbs",
    difficulty: "medium",
    content: {
      question: "We should ________ the environment.",
      options: [
        "look after",
        "look for",
        "look into",
        "look at"
      ],
      correctAnswer: 0,
      explanation: "'Look after' significa cuidar."
    } as any
  },
  {
    id: "b1-u13-e83",
    type: "multiple-choice",
    level: "B1",
    topic: "Society & Culture",
    topicName: "Phrasal Verbs",
    difficulty: "medium",
    content: {
      question: "He ________ to his grandfather as a role model.",
      options: [
        "looks up",
        "looks down",
        "looks after",
        "looks for"
      ],
      correctAnswer: 0,
      explanation: "'Look up to' significa admirar a alguien."
    } as any
  },
  {
    id: "b1-u13-e84",
    type: "multiple-choice",
    level: "B1",
    topic: "Society & Culture",
    topicName: "Phrasal Verbs",
    difficulty: "medium",
    content: {
      question: "The protest was ________ by the police.",
      options: [
        "broken up",
        "broken down",
        "broken in",
        "broken off"
      ],
      correctAnswer: 0,
      explanation: "'Break up' puede significar dispersar una multitud."
    } as any
  },
  {
    id: "b1-u13-e85",
    type: "multiple-choice",
    level: "B1",
    topic: "Society & Culture",
    topicName: "Phrasal Verbs",
    difficulty: "medium",
    content: {
      question: "Many people ________ on their parents when they are young.",
      options: [
        "rely",
        "count",
        "depend",
        "stay"
      ],
      correctAnswer: 2,
      explanation: "Aunque no es un phrasal verb puro, 'depend on' es la estructura correcta (o 'rely on')."
    } as any
  },
  {
    id: "b1-u13-e86",
    type: "multiple-choice",
    level: "B1",
    topic: "Society & Culture",
    topicName: "Phrasal Verbs",
    difficulty: "medium",
    content: {
      question: "The government needs to ________ with a plan to fix the economy.",
      options: [
        "come up",
        "come in",
        "come along",
        "come out"
      ],
      correctAnswer: 0,
      explanation: "'Come up with' significa idear o proponer un plan."
    } as any
  },
  {
    id: "b1-u13-e87",
    type: "multiple-choice",
    level: "B1",
    topic: "Society & Culture",
    topicName: "Phrasal Verbs",
    difficulty: "hard",
    content: {
      question: "He ________ from his parents' home at the age of 18.",
      options: [
        "moved out",
        "moved in",
        "moved on",
        "moved up"
      ],
      correctAnswer: 0,
      explanation: "'Move out' significa mudarse de una casa."
    } as any
  },
  {
    id: "b1-u13-e88",
    type: "multiple-choice",
    level: "B1",
    topic: "Society & Culture",
    topicName: "Phrasal Verbs",
    difficulty: "hard",
    content: {
      question: "We shouldn't ________ people who are less fortunate than us.",
      options: [
        "look down on",
        "look up to",
        "look after",
        "look into"
      ],
      correctAnswer: 0,
      explanation: "'Look down on' significa menospreciar a alguien."
    } as any
  },
  {
    id: "b1-u13-e89",
    type: "multiple-choice",
    level: "B1",
    topic: "Society & Culture",
    topicName: "Phrasal Verbs",
    difficulty: "hard",
    content: {
      question: "The community center helps people ________ with their new lives.",
      options: [
        "get on",
        "get off",
        "get in",
        "get by"
      ],
      correctAnswer: 0,
      explanation: "'Get on' puede significar progresar o continuar con algo."
    } as any
  },
  {
    id: "b1-u13-e90",
    type: "multiple-choice",
    level: "B1",
    topic: "Society & Culture",
    topicName: "Phrasal Verbs",
    difficulty: "hard",
    content: {
      question: "They are ________ for better working conditions.",
      options: [
        "calling",
        "looking",
        "putting",
        "setting"
      ],
      correctAnswer: 0,
      explanation: "'Call for' significa exigir o pedir algo públicamente."
    } as any
  },

  // --- SUB-TOPIC 10: Review & Discussing Current Affairs (91-100) ---
  {
    id: "b1-u13-e91",
    type: "multiple-choice",
    level: "B1",
    topic: "Society & Culture",
    topicName: "Review",
    difficulty: "medium",
    content: {
      question: "What is the reported speech of: 'I'm happy,' she said?",
      options: [
        "She said she was happy.",
        "She said she is happy.",
        "She says she was happy.",
        "She said she has been happy."
      ],
      correctAnswer: 0,
      explanation: "Cambio de presente simple a pasado simple."
    } as any
  },
  {
    id: "b1-u13-e92",
    type: "multiple-choice",
    level: "B1",
    topic: "Society & Culture",
    topicName: "Review",
    difficulty: "medium",
    content: {
      question: "Report the question: 'Where are you going?' he asked.",
      options: [
        "He asked me where I was going.",
        "He asked me where was I going.",
        "He asked me where I am going.",
        "He asked me where did I go."
      ],
      correctAnswer: 0,
      explanation: "Pregunta indirecta con orden de afirmación y cambio de tiempo."
    } as any
  },
  {
    id: "b1-u13-e93",
    type: "multiple-choice",
    level: "B1",
    topic: "Society & Culture",
    topicName: "Review",
    difficulty: "medium",
    content: {
      question: "What does 'unemployment' mean?",
      options: [
        "Not having a job",
        "Working too much",
        "Living in a city",
        "Voting in elections"
      ],
      correctAnswer: 0,
      explanation: "Desempleo significa no tener trabajo."
    } as any
  },
  {
    id: "b1-u13-e94",
    type: "multiple-choice",
    level: "B1",
    topic: "Society & Culture",
    topicName: "Review",
    difficulty: "medium",
    content: {
      question: "Complete: 'The judge ________ the criminal to ten years.'",
      options: [
        "sentenced",
        "arrested",
        "called",
        "judged"
      ],
      correctAnswer: 0,
      explanation: "El juez sentencia (sentenced) a alguien."
    } as any
  },
  {
    id: "b1-u13-e95",
    type: "multiple-choice",
    level: "B1",
    topic: "Society & Culture",
    topicName: "Review",
    difficulty: "medium",
    content: {
      question: "Choose the correct reporting verb: 'Please, sit down,' she ________ me.",
      options: [
        "asked",
        "said",
        "told",
        "ordered"
      ],
      correctAnswer: 0,
      explanation: "'Asked' es apropiado para peticiones corteses."
    } as any
  },
  {
    id: "b1-u13-e96",
    type: "multiple-choice",
    level: "B1",
    topic: "Society & Culture",
    topicName: "Review",
    difficulty: "hard",
    content: {
      question: "Which phrasal verb means 'to admire someone'?",
      options: [
        "Look up to",
        "Look down on",
        "Look after",
        "Look for"
      ],
      correctAnswer: 0,
      explanation: "'Look up to' es admirar."
    } as any
  },
  {
    id: "b1-u13-e97",
    type: "multiple-choice",
    level: "B1",
    topic: "Society & Culture",
    topicName: "Review",
    difficulty: "hard",
    content: {
      question: "Report the command: 'Don't shout!' he told them.",
      options: [
        "He told them not to shout.",
        "He told them to not shout.",
        "He told them don't shout.",
        "He told them no shouting."
      ],
      correctAnswer: 0,
      explanation: "Orden negativa: 'not to + infinitive'."
    } as any
  },
  {
    id: "b1-u13-e98",
    type: "multiple-choice",
    level: "B1",
    topic: "Society & Culture",
    topicName: "Review",
    difficulty: "hard",
    content: {
      question: "What is 'multiculturalism'?",
      options: [
        "The presence of several cultures",
        "The belief in one culture only",
        "Moving to another city",
        "Voting for a new leader"
      ],
      correctAnswer: 0,
      explanation: "Multiculturalidad es la presencia de varias culturas."
    } as any
  },
  {
    id: "b1-u13-e99",
    type: "multiple-choice",
    level: "B1",
    topic: "Society & Culture",
    topicName: "Review",
    difficulty: "hard",
    content: {
      question: "Complete: 'The government has ________ a new law.'",
      options: [
        "passed",
        "done",
        "made",
        "voted"
      ],
      correctAnswer: 0,
      explanation: "'Pass a law' es la expresión para aprobar una ley."
    } as any
  },
  {
    id: "b1-u13-e100",
    type: "multiple-choice",
    level: "B1",
    topic: "Society & Culture",
    topicName: "Review",
    difficulty: "hard",
    content: {
      question: "Speculating: 'The election results ________ be announced tonight.'",
      options: [
        "might",
        "shall",
        "do",
        "have"
      ],
      correctAnswer: 0,
      explanation: "'Might' indica posibilidad para el futuro."
    } as any
  }
];

