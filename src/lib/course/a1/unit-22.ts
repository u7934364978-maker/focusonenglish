import { Exercise } from '@/lib/exercise-generator';

export const UNIT_22_EXERCISES: Exercise[] = [
  // FLASHCARDS (1-10)
  {
    "id": "a1-u22-e1",
    "type": "flashcard",
    "level": "A1",
    "topic": "Household Objects",
    "difficulty": "easy",
    "transcript": "Clock. Mirror. Pillow. Blanket. Lamp",
    "content": {
      "title": "Objetos del Hogar",
      "instructions": "Aprende el nombre de estos objetos comunes.",
      "items": [
        { "front": "[[Clock|Reloj (de pared)]]", "back": "Reloj (de pared)" },
        { "front": "[[Mirror|Espejo]]", "back": "Espejo" },
        { "front": "[[Pillow|Almohada]]", "back": "Almohada" },
        { "front": "[[Blanket|Manta]]", "back": "Manta" },
        { "front": "[[Lamp|Lámpara]]", "back": "Lámpara" }
      ]
    },
    "audioUrl": "audio/a1/unit-22/e1.mp3",
    "topicName": "Vocabulary"
  },
  {
    "id": "a1-u22-e2",
    "type": "flashcard",
    "level": "A1",
    "topic": "Kitchenware",
    "difficulty": "easy",
    "transcript": "Plate. Spoon. Fork. Knife. Glass",
    "content": {
      "title": "Utensilios de Cocina",
      "instructions": "Aprende los nombres de lo que usas para comer.",
      "items": [
        { "front": "[[Plate|Plato]]", "back": "Plato" },
        { "front": "[[Spoon|Cuchara]]", "back": "Cuchara" },
        { "front": "[[Fork|Tenedor]]", "back": "Tenedor" },
        { "front": "[[Knife|Cuchillo]]", "back": "Cuchillo" },
        { "front": "[[Glass|Vaso]]", "back": "Vaso" }
      ]
    },
    "audioUrl": "audio/a1/unit-22/e2.mp3",
    "topicName": "Vocabulary"
  },
  {
    "id": "a1-u22-e3",
    "type": "flashcard",
    "level": "A1",
    "topic": "Electronics",
    "difficulty": "easy",
    "transcript": "Computer. Laptop. Telephone. Camera. TV",
    "content": {
      "title": "Electrónica",
      "instructions": "Aprende los nombres de aparatos electrónicos.",
      "items": [
        { "front": "[[Computer|Ordenador]]", "back": "Ordenador" },
        { "front": "[[Laptop|Portátil]]", "back": "Portátil" },
        { "front": "[[Telephone|Teléfono]]", "back": "Teléfono" },
        { "front": "[[Camera|Cámara]]", "back": "Cámara" },
        { "front": "[[TV|Televisor]]", "back": "Televisor" }
      ]
    },
    "audioUrl": "audio/a1/unit-22/e3.mp3",
    "topicName": "Vocabulary"
  },
  {
    "id": "a1-u22-e4",
    "type": "flashcard",
    "level": "A1",
    "topic": "Personal Objects",
    "difficulty": "easy",
    "transcript": "Keys. Wallet. Watch. Umbrella. Glasses",
    "content": {
      "title": "Objetos Personales",
      "instructions": "Objetos que sueles llevar contigo.",
      "items": [
        { "front": "[[Keys|Llaves]]", "back": "Llaves" },
        { "front": "[[Wallet|Cartera]]", "back": "Cartera" },
        { "front": "[[Watch|Reloj (de pulsera)]]", "back": "Reloj (de pulsera)" },
        { "front": "[[Umbrella|Paraguas]]", "back": "Paraguas" },
        { "front": "[[Glasses|Gafas]]", "back": "Gafas" }
      ]
    },
    "audioUrl": "audio/a1/unit-22/e4.mp3",
    "topicName": "Vocabulary"
  },
  {
    "id": "a1-u22-e5",
    "type": "flashcard",
    "level": "A1",
    "topic": "Bedroom Items",
    "difficulty": "easy",
    "transcript": "Bed. Wardrobe. Curtains. Rug. Shelf",
    "content": {
      "title": "En el Dormitorio",
      "instructions": "Más objetos de la habitación.",
      "items": [
        { "front": "[[Bed|Cama]]", "back": "Cama" },
        { "front": "[[Wardrobe|Armario]]", "back": "Armario" },
        { "front": "[[Curtains|Cortinas]]", "back": "Cortinas" },
        { "front": "[[Rug|Alfombra]]", "back": "Alfombra" },
        { "front": "[[Shelf|Estantería]]", "back": "Estantería" }
      ]
    },
    "audioUrl": "audio/a1/unit-22/e5.mp3",
    "topicName": "Vocabulary"
  },
  {
    "id": "a1-u22-e6",
    "type": "flashcard",
    "level": "A1",
    "topic": "Living Room Items",
    "difficulty": "easy",
    "transcript": "Armchair. Coffee table. Painting. Vase. Bookshelf",
    "content": {
      "title": "En el Salón",
      "instructions": "Muebles y decoración del salón.",
      "items": [
        { "front": "[[Armchair|Sillón]]", "back": "Sillón" },
        { "front": "[[Coffee table|Mesa de centro]]", "back": "Mesa de centro" },
        { "front": "[[Painting|Cuadro/Pintura]]", "back": "Cuadro/Pintura" },
        { "front": "[[Vase|Jarrón]]", "back": "Jarrón" },
        { "front": "[[Bookshelf|Librería]]", "back": "Librería" }
      ]
    },
    "audioUrl": "audio/a1/unit-22/e6.mp3",
    "topicName": "Vocabulary"
  },
  {
    "id": "a1-u22-e7",
    "type": "flashcard",
    "level": "A1",
    "topic": "Bathroom Items",
    "difficulty": "easy",
    "transcript": "Towel. Soap. Shampoo. Toothbrush. Toothpaste",
    "content": {
      "title": "En el Baño",
      "instructions": "Objetos de higiene personal.",
      "items": [
        { "front": "[[Towel|Toalla]]", "back": "Toalla" },
        { "front": "[[Soap|Jabón]]", "back": "Jabón" },
        { "front": "[[Shampoo|Champú]]", "back": "Champú" },
        { "front": "[[Toothbrush|Cepillo de dientes]]", "back": "Cepillo de dientes" },
        { "front": "[[Toothpaste|Pasta de dientes]]", "back": "Pasta de dientes" }
      ]
    },
    "audioUrl": "audio/a1/unit-22/e7.mp3",
    "topicName": "Vocabulary"
  },
  {
    "id": "a1-u22-e8",
    "type": "flashcard",
    "level": "A1",
    "topic": "Actions with Objects",
    "difficulty": "medium",
    "transcript": "Turn on. Turn off. Open. Close. Use",
    "content": {
      "title": "Acciones con Objetos",
      "instructions": "Verbos para interactuar con cosas.",
      "items": [
        { "front": "[[Turn on|Encender]]", "back": "Encender" },
        { "front": "[[Turn off|Apagar]]", "back": "Apagar" },
        { "front": "[[Open|Abrir]]", "back": "Abrir" },
        { "front": "[[Close|Cerrar]]", "back": "Cerrar" },
        { "front": "[[Use|Usar]]", "back": "Usar" }
      ]
    },
    "audioUrl": "audio/a1/unit-22/e8.mp3",
    "topicName": "Vocabulary"
  },
  {
    "id": "a1-u22-e9",
    "type": "flashcard",
    "level": "A1",
    "topic": "Describing Objects",
    "difficulty": "medium",
    "transcript": "New. Old. Heavy. Light. Expensive",
    "content": {
      "title": "Describiendo Objetos",
      "instructions": "Adjetivos para describir cosas.",
      "items": [
        { "front": "[[New|Nuevo]]", "back": "Nuevo" },
        { "front": "[[Old|Viejo/Antiguo]]", "back": "Viejo/Antiguo" },
        { "front": "[[Heavy|Pesado]]", "back": "Pesado" },
        { "front": "[[Light|Ligero]]", "back": "Ligero" },
        { "front": "[[Expensive|Caro]]", "back": "Caro" }
      ]
    },
    "audioUrl": "audio/a1/unit-22/e9.mp3",
    "topicName": "Vocabulary"
  },
  {
    "id": "a1-u22-e10",
    "type": "flashcard",
    "level": "A1",
    "topic": "Describing Objects 2",
    "difficulty": "medium",
    "transcript": "Cheap. Broken. Useful. Beautiful. Big",
    "content": {
      "title": "Describiendo Objetos 2",
      "instructions": "Más adjetivos.",
      "items": [
        { "front": "[[Cheap|Barato]]", "back": "Barato" },
        { "front": "[[Broken|Roto]]", "back": "Roto" },
        { "front": "[[Useful|Útil]]", "back": "Útil" },
        { "front": "[[Beautiful|Hermoso]]", "back": "Hermoso" },
        { "front": "[[Big|Grande]]", "back": "Grande" }
      ]
    },
    "audioUrl": "audio/a1/unit-22/e10.mp3",
    "topicName": "Vocabulary"
  },

  // MULTIPLE CHOICE (11-40)
  {
    "id": "a1-u22-e11",
    "type": "multiple-choice",
    "level": "A1",
    "topic": "Demonstratives",
    "difficulty": "easy",
    "transcript": "This is my computer.",
    "content": {
      "title": "Este/Esta (This)",
      "instructions": "Elige el demostrativo correcto para algo cercano y singular.",
      "questions": [
        {
          "question": "_______ [[is|es]] [[my|mi]] [[computer|ordenador]] ([[here|aquí]]).",
          "options": ["[[This|Este/Esta]]", "[[That|Ese/Esa]]", "[[Those|Esos/Esas]]"],
          "correctAnswer": 0,
          "explanation": "Usamos 'This' para un objeto singular que está cerca."
        }
      ]
    },
    "audioUrl": "audio/a1/unit-22/e11.mp3",
    "topicName": "Grammar"
  },
  {
    "id": "a1-u22-e12",
    "type": "multiple-choice",
    "level": "A1",
    "topic": "Demonstratives",
    "difficulty": "easy",
    "transcript": "That is a beautiful painting over there.",
    "content": {
      "title": "Ese/Esa (That)",
      "instructions": "Elige el demostrativo correcto para algo lejano y singular.",
      "questions": [
        {
          "question": "_______ [[is|es]] [[a|un]] [[beautiful|hermoso]] [[painting|cuadro]] [[over there|allí]].",
          "options": ["[[This|Este/Esta]]", "[[That|Ese/Esa]]", "[[These|Estos/Estas]]"],
          "correctAnswer": 1,
          "explanation": "Usamos 'That' para un objeto singular que está lejos."
        }
      ]
    },
    "audioUrl": "audio/a1/unit-22/e12.mp3",
    "topicName": "Grammar"
  },
  {
    "id": "a1-u22-e13",
    "type": "multiple-choice",
    "level": "A1",
    "topic": "Demonstratives",
    "difficulty": "medium",
    "transcript": "These are my keys.",
    "content": {
      "title": "Estos/Estas (These)",
      "instructions": "Elige el demostrativo correcto para plural cercano.",
      "questions": [
        {
          "question": "_______ [[are|son]] [[my|mis]] [[keys|llaves]] ([[here|aquí]]).",
          "options": ["[[This|Este]]", "[[These|Estos]]", "[[Those|Esos]]"],
          "correctAnswer": 1,
          "explanation": "Usamos 'These' para objetos plurales que están cerca."
        }
      ]
    },
    "audioUrl": "audio/a1/unit-22/e13.mp3",
    "topicName": "Grammar"
  },
  {
    "id": "a1-u22-e14",
    "type": "multiple-choice",
    "level": "A1",
    "topic": "Demonstratives",
    "difficulty": "medium",
    "transcript": "Those glasses are expensive.",
    "content": {
      "title": "Esos/Esas (Those)",
      "instructions": "Elige el demostrativo correcto para plural lejano.",
      "questions": [
        {
          "question": "_______ [[glasses|gafas]] [[are|son]] [[expensive|caras]] ([[there|allí]]).",
          "options": ["[[These|Estas]]", "[[Those|Esas]]", "[[This|Esta]]"],
          "correctAnswer": 1,
          "explanation": "Usamos 'Those' para objetos plurales que están lejos."
        }
      ]
    },
    "audioUrl": "audio/a1/unit-22/e14.mp3",
    "topicName": "Grammar"
  },
  {
    "id": "a1-u22-e15",
    "type": "multiple-choice",
    "level": "A1",
    "topic": "Possessives",
    "difficulty": "medium",
    "transcript": "It is Maria's watch.",
    "content": {
      "title": "El Posesivo 's",
      "instructions": "Indica posesión.",
      "questions": [
        {
          "question": "[[It|Ello]] [[is|es]] [[Maria|María]]_______ [[watch|reloj]].",
          "options": ["[['s|de (posesivo)]]", "[[s|plural]]", "[[is|es]]"],
          "correctAnswer": 0,
          "explanation": "Añadimos 's al nombre del poseedor para indicar propiedad."
        }
      ]
    },
    "audioUrl": "audio/a1/unit-22/e15.mp3",
    "topicName": "Grammar"
  },
  {
    "id": "a1-u22-e16",
    "type": "multiple-choice",
    "level": "A1",
    "topic": "Vocabulary: Kitchen",
    "difficulty": "easy",
    "transcript": "I eat soup with a spoon.",
    "content": {
      "title": "Comiendo Sopa",
      "instructions": "Elige el objeto correcto.",
      "questions": [
        {
          "question": "[[I|Yo]] [[eat|como]] [[soup|sopa]] [[with|con]] [[a|una]] _______.",
          "options": ["[[fork|tenedor]]", "[[spoon|cuchara]]", "[[knife|cuchillo]]"],
          "correctAnswer": 1,
          "explanation": "Usamos una cuchara (spoon) para la sopa."
        }
      ]
    },
    "audioUrl": "audio/a1/unit-22/e16.mp3",
    "topicName": "Vocabulary"
  },
  {
    "id": "a1-u22-e17",
    "type": "multiple-choice",
    "level": "A1",
    "topic": "Vocabulary: Bedroom",
    "difficulty": "easy",
    "transcript": "I need a blanket. It's cold.",
    "content": {
      "title": "Tengo Frío",
      "instructions": "Elige el objeto adecuado.",
      "questions": [
        {
          "question": "[[I|Yo]] [[need|necesito]] [[a|una]] _______. [[It's|Hace]] [[cold|frío]].",
          "options": ["[[blanket|manta]]", "[[mirror|espejo]]", "[[camera|cámara]]"],
          "correctAnswer": 0,
          "explanation": "Una manta (blanket) nos calienta cuando hace frío."
        }
      ]
    },
    "audioUrl": "audio/a1/unit-22/e17.mp3",
    "topicName": "Vocabulary"
  },
  {
    "id": "a1-u22-e18",
    "type": "multiple-choice",
    "level": "A1",
    "topic": "Actions",
    "difficulty": "medium",
    "transcript": "Turn on the lamp, please. It's dark.",
    "content": {
      "title": "Encendiendo la Luz",
      "instructions": "Elige el verbo correcto.",
      "questions": [
        {
          "question": "_______ [[the|la]] [[lamp|lámpara]], [[please|por favor]]. [[It's|Está]] [[dark|oscuro]].",
          "options": ["[[Turn on|Enciende]]", "[[Turn off|Apaga]]", "[[Close|Cierra]]"],
          "correctAnswer": 0,
          "explanation": "Usamos 'Turn on' para encender aparatos o luces."
        }
      ]
    },
    "audioUrl": "audio/a1/unit-22/e18.mp3",
    "topicName": "Vocabulary"
  },
  {
    "id": "a1-u22-e19",
    "type": "multiple-choice",
    "level": "A1",
    "topic": "Vocabulary: Personal Items",
    "difficulty": "easy",
    "transcript": "My money is in my wallet.",
    "content": {
      "title": "Dinero",
      "instructions": "Completa la oración.",
      "questions": [
        {
          "question": "[[My|Mi]] [[money|dinero]] [[is|está]] [[in|en]] [[my|mi]] _______.",
          "options": ["[[wallet|cartera]]", "[[watch|reloj]]", "[[shelf|estantería]]"],
          "correctAnswer": 0,
          "explanation": "El dinero se guarda en la cartera (wallet)."
        }
      ]
    },
    "audioUrl": "audio/a1/unit-22/e19.mp3",
    "topicName": "Vocabulary"
  },
  {
    "id": "a1-u22-e20",
    "type": "multiple-choice",
    "level": "A1",
    "topic": "Grammar: Plurals",
    "difficulty": "medium",
    "transcript": "I have two watches.",
    "content": {
      "title": "Plurales con -es",
      "instructions": "Elige la forma correcta del plural.",
      "questions": [
        {
          "question": "[[I|Yo]] [[have|tengo]] [[two|dos]] _______.",
          "options": ["[[watchs|relojes (error)]]", "[[watches|relojes]]", "[[watch|reloj]]"],
          "correctAnswer": 1,
          "explanation": "Las palabras que terminan en 'ch' forman el plural añadiendo '-es'."
        }
      ]
    },
    "audioUrl": "audio/a1/unit-22/e20.mp3",
    "topicName": "Grammar"
  },
  {
    "id": "a1-u22-e21",
    "type": "multiple-choice",
    "level": "A1",
    "topic": "Vocabulary: Home",
    "difficulty": "medium",
    "transcript": "The clock is on the wall.",
    "content": {
      "title": "El Reloj",
      "instructions": "Completa la oración.",
      "questions": [
        {
          "question": "[[The|El]] _______ [[is|está]] [[on|en]] [[the|la]] [[wall|pared]].",
          "options": ["[[clock|reloj (pared)]]", "[[watch|reloj (pulsera)]]", "[[camera|cámara]]"],
          "correctAnswer": 0,
          "explanation": "'Clock' es el reloj de pared, 'watch' es el de pulsera."
        }
      ]
    },
    "audioUrl": "audio/a1/unit-22/e21.mp3",
    "topicName": "Vocabulary"
  },
  {
    "id": "a1-u22-e22",
    "type": "multiple-choice",
    "level": "A1",
    "topic": "Demonstratives",
    "difficulty": "medium",
    "transcript": "Are these your books?",
    "content": {
      "title": "Preguntas con Demostrativos",
      "instructions": "Elige la opción correcta.",
      "questions": [
        {
          "question": "_______ [[these|estos]] [[your|tus]] [[books|libros]]?",
          "options": ["[[Is|¿Es?]]", "[[Are|¿Son?]]", "[[Do|Auxiliar]]"],
          "correctAnswer": 1,
          "explanation": "Como 'these' es plural, usamos el verbo 'are'."
        }
      ]
    },
    "audioUrl": "audio/a1/unit-22/e22.mp3",
    "topicName": "Grammar"
  },
  {
    "id": "a1-u22-e23",
    "type": "multiple-choice",
    "level": "A1",
    "topic": "Vocabulary: Adjectives",
    "difficulty": "medium",
    "transcript": "This computer is old and slow.",
    "content": {
      "title": "El Ordenador Viejo",
      "instructions": "Elige el adjetivo opuesto a 'new'.",
      "questions": [
        {
          "question": "[[This|Este]] [[computer|ordenador]] [[is|es]] _______ [[and|y]] [[slow|lento]].",
          "options": ["[[new|nuevo]]", "[[old|viejo]]", "[[expensive|caro]]"],
          "correctAnswer": 1,
          "explanation": "'Old' es lo opuesto a 'new'."
        }
      ]
    },
    "audioUrl": "audio/a1/unit-22/e23.mp3",
    "topicName": "Vocabulary"
  },
  {
    "id": "a1-u22-e24",
    "type": "multiple-choice",
    "level": "A1",
    "topic": "Vocabulary: Objects",
    "difficulty": "easy",
    "transcript": "I take photos with my camera.",
    "content": {
      "title": "Fotografía",
      "instructions": "Elige el objeto correcto.",
      "questions": [
        {
          "question": "[[I|Yo]] [[take|tomo]] [[photos|fotos]] [[with|con]] [[my|mi]] _______.",
          "options": ["[[camera|cámara]]", "[[laptop|portátil]]", "[[pillow|almohada]]"],
          "correctAnswer": 0,
          "explanation": "Usamos una cámara (camera) para tomar fotos."
        }
      ]
    },
    "audioUrl": "audio/a1/unit-22/e24.mp3",
    "topicName": "Vocabulary"
  },
  {
    "id": "a1-u22-e25",
    "type": "multiple-choice",
    "level": "A1",
    "topic": "Possessives",
    "difficulty": "medium",
    "transcript": "Where are John's keys?",
    "content": {
      "title": "Las Llaves de John",
      "instructions": "Completa con el posesivo.",
      "questions": [
        {
          "question": "[[Where|Dónde]] [[are|están]] [[John|Juan]]_______ [[keys|llaves]]?",
          "options": ["[['s|de]]", "[[s|plural]]", "[[is|es]]"],
          "correctAnswer": 0,
          "explanation": "John's keys = las llaves de John."
        }
      ]
    },
    "audioUrl": "audio/a1/unit-22/e25.mp3",
    "topicName": "Grammar"
  },
  {
    "id": "a1-u22-e26",
    "type": "multiple-choice",
    "level": "A1",
    "topic": "Vocabulary: Home",
    "difficulty": "medium",
    "transcript": "The laptop is on the coffee table.",
    "content": {
      "title": "En el Salón",
      "instructions": "Elige el mueble correcto.",
      "questions": [
        {
          "question": "[[The|El]] [[laptop|portátil]] [[is|está]] [[on|sobre]] [[the|la]] _______.",
          "options": ["[[coffee table|mesa de centro]]", "[[wardrobe|armario]]", "[[sink|lavabo]]"],
          "correctAnswer": 0,
          "explanation": "Una mesa de centro (coffee table) está en el salón."
        }
      ]
    },
    "audioUrl": "audio/a1/unit-22/e26.mp3",
    "topicName": "Vocabulary"
  },
  {
    "id": "a1-u22-e27",
    "type": "multiple-choice",
    "level": "A1",
    "topic": "Vocabulary: Personal Items",
    "difficulty": "medium",
    "transcript": "I can't see! Where are my glasses?",
    "content": {
      "title": "Problemas de Visión",
      "instructions": "Elige el objeto necesario.",
      "questions": [
        {
          "question": "[[I|Yo]] [[can't|no puedo]] [[see|ver]]! [[Where|Dónde]] [[are|están]] [[my|mis]] _______?",
          "options": ["[[glasses|gafas]]", "[[wallet|cartera]]", "[[umbrella|paraguas]]"],
          "correctAnswer": 0,
          "explanation": "Necesitamos las gafas (glasses) para ver bien."
        }
      ]
    },
    "audioUrl": "audio/a1/unit-22/e27.mp3",
    "topicName": "Vocabulary"
  },
  {
    "id": "a1-u22-e28",
    "type": "multiple-choice",
    "level": "A1",
    "topic": "Actions",
    "difficulty": "medium",
    "transcript": "Close the window, please. It's raining.",
    "content": {
      "title": "La Ventana",
      "instructions": "Elige la acción correcta.",
      "questions": [
        {
          "question": "_______ [[the|la]] [[window|ventana]], [[please|por favor]]. [[It's|Está]] [[raining|lloviendo]].",
          "options": ["[[Open|Abre]]", "[[Close|Cierra]]", "[[Turn off|Apaga]]"],
          "correctAnswer": 1,
          "explanation": "Cerramos (close) la ventana si llueve."
        }
      ]
    },
    "audioUrl": "audio/a1/unit-22/e28.mp3",
    "topicName": "Vocabulary"
  },
  {
    "id": "a1-u22-e29",
    "type": "multiple-choice",
    "level": "A1",
    "topic": "Vocabulary: Bathroom",
    "difficulty": "easy",
    "transcript": "Use a towel to dry your hands.",
    "content": {
      "title": "Secarse las manos",
      "instructions": "Elige el objeto correcto.",
      "questions": [
        {
          "question": "[[Use|Usa]] [[a|una]] _______ [[to|para]] [[dry|secar]] [[your|tus]] [[hands|manos]].",
          "options": ["[[towel|toalla]]", "[[soap|jabón]]", "[[shelf|estantería]]"],
          "correctAnswer": 0,
          "explanation": "Usamos una toalla (towel) para secarnos."
        }
      ]
    },
    "audioUrl": "audio/a1/unit-22/e29.mp3",
    "topicName": "Vocabulary"
  },
  {
    "id": "a1-u22-e30",
    "type": "multiple-choice",
    "level": "A1",
    "topic": "Vocabulary: Objects",
    "difficulty": "medium",
    "transcript": "That vase is very beautiful.",
    "content": {
      "title": "Decoración",
      "instructions": "Completa la oración.",
      "questions": [
        {
          "question": "[[That|Ese]] _______ [[is|es]] [[very|muy]] [[beautiful|hermoso]].",
          "options": ["[[vase|jarrón]]", "[[knife|cuchillo]]", "[[oven|horno]]"],
          "correctAnswer": 0,
          "explanation": "Un jarrón (vase) es un objeto decorativo."
        }
      ]
    },
    "audioUrl": "audio/a1/unit-22/e30.mp3",
    "topicName": "Vocabulary"
  },
  {
    "id": "a1-u22-e31",
    "type": "multiple-choice",
    "level": "A1",
    "topic": "Vocabulary: Adjectives",
    "difficulty": "medium",
    "transcript": "This box is very heavy.",
    "content": {
      "title": "Pesado",
      "instructions": "Elige el adjetivo correcto.",
      "questions": [
        {
          "question": "[[This|Esta]] [[box|caja]] [[is|es]] [[very|muy]] _______.",
          "options": ["[[heavy|pesada]]", "[[light|ligera]]", "[[cheap|barata]]"],
          "correctAnswer": 0,
          "explanation": "'Heavy' significa que algo pesa mucho."
        }
      ]
    },
    "audioUrl": "audio/a1/unit-22/e31.mp3",
    "topicName": "Vocabulary"
  },
  {
    "id": "a1-u22-e32",
    "type": "multiple-choice",
    "level": "A1",
    "topic": "Demonstratives",
    "difficulty": "medium",
    "transcript": "What are those things over there?",
    "content": {
      "title": "Aquellas cosas",
      "instructions": "Elige el demostrativo plural para algo lejano.",
      "questions": [
        {
          "question": "[[What|Qué]] [[are|son]] _______ [[things|cosas]] [[over there|allí]]?",
          "options": ["[[these|estas]]", "[[those|aquellas/esas]]", "[[this|esta]]"],
          "correctAnswer": 1,
          "explanation": "Usamos 'those' para plural y distancia."
        }
      ]
    },
    "audioUrl": "audio/a1/unit-22/e32.mp3",
    "topicName": "Grammar"
  },
  {
    "id": "a1-u22-e33",
    "type": "multiple-choice",
    "level": "A1",
    "topic": "Possessives",
    "difficulty": "medium",
    "transcript": "This is my brother's laptop.",
    "content": {
      "title": "Posesión",
      "instructions": "Completa la oración.",
      "questions": [
        {
          "question": "[[This|Este]] [[is|es]] [[my|mi]] [[brother|hermano]]_______ [[laptop|portátil]].",
          "options": ["[['s|de]]", "[[s|plural]]", "[[is|es]]"],
          "correctAnswer": 0,
          "explanation": "Brother's laptop = el portátil de mi hermano."
        }
      ]
    },
    "audioUrl": "audio/a1/unit-22/e33.mp3",
    "topicName": "Grammar"
  },
  {
    "id": "a1-u22-e34",
    "type": "multiple-choice",
    "level": "A1",
    "topic": "Vocabulary: Objects",
    "difficulty": "easy",
    "transcript": "I put my books on the shelf.",
    "content": {
      "title": "Organización",
      "instructions": "Elige el mueble correcto.",
      "questions": [
        {
          "question": "[[I|Yo]] [[put|pongo]] [[my|mis]] [[books|libros]] [[on|en]] [[the|la]] _______.",
          "options": ["[[shelf|estantería]]", "[[pillow|almohada]]", "[[mirror|espejo]]"],
          "correctAnswer": 0,
          "explanation": "Ponemos los libros en la estantería (shelf)."
        }
      ]
    },
    "audioUrl": "audio/a1/unit-22/e34.mp3",
    "topicName": "Vocabulary"
  },
  {
    "id": "a1-u22-e35",
    "type": "multiple-choice",
    "level": "A1",
    "topic": "Actions",
    "difficulty": "medium",
    "transcript": "Turn off the TV and go to sleep.",
    "content": {
      "title": "Apagar",
      "instructions": "Elige la acción correcta.",
      "questions": [
        {
          "question": "_______ [[the|la]] [[TV|televisión]] [[and|y]] [[go|ve]] [[to|a]] [[sleep|dormir]].",
          "options": ["[[Turn off|Apaga]]", "[[Turn on|Enciende]]", "[[Open|Abre]]"],
          "correctAnswer": 0,
          "explanation": "Usamos 'Turn off' para apagar."
        }
      ]
    },
    "audioUrl": "audio/a1/unit-22/e35.mp3",
    "topicName": "Vocabulary"
  },
  {
    "id": "a1-u22-e36",
    "type": "multiple-choice",
    "level": "A1",
    "topic": "Vocabulary: Objects",
    "difficulty": "medium",
    "transcript": "The toothbrush is in the bathroom.",
    "content": {
      "title": "Higiene",
      "instructions": "Elige la palabra correcta.",
      "questions": [
        {
          "question": "[[The|El]] _______ [[is|está]] [[in|en]] [[the|el]] [[bathroom|baño]].",
          "options": ["[[toothbrush|cepillo de dientes]]", "[[knife|cuchillo]]", "[[blanket|manta]]"],
          "correctAnswer": 0,
          "explanation": "El cepillo de dientes (toothbrush) está en el baño."
        }
      ]
    },
    "audioUrl": "audio/a1/unit-22/e36.mp3",
    "topicName": "Vocabulary"
  },
  {
    "id": "a1-u22-e37",
    "type": "multiple-choice",
    "level": "A1",
    "topic": "Vocabulary: Kitchen",
    "difficulty": "medium",
    "transcript": "I need a knife to cut the bread.",
    "content": {
      "title": "Cortar el pan",
      "instructions": "Elige el utensilio correcto.",
      "questions": [
        {
          "question": "[[I|Yo]] [[need|necesito]] [[a|un]] _______ [[to|para]] [[cut|cortar]] [[the|el]] [[bread|pan]].",
          "options": ["[[knife|cuchillo]]", "[[spoon|cuchara]]", "[[glass|vaso]]"],
          "correctAnswer": 0,
          "explanation": "Usamos el cuchillo (knife) para cortar."
        }
      ]
    },
    "audioUrl": "audio/a1/unit-22/e37.mp3",
    "topicName": "Vocabulary"
  },
  {
    "id": "a1-u22-e38",
    "type": "multiple-choice",
    "level": "A1",
    "topic": "Vocabulary: Electronics",
    "difficulty": "easy",
    "transcript": "I work on my laptop.",
    "content": {
      "title": "Trabajo",
      "instructions": "Completa la oración.",
      "questions": [
        {
          "question": "[[I|Yo]] [[work|trabajo]] [[on|en]] [[my|mi]] _______.",
          "options": ["[[laptop|portátil]]", "[[wallet|cartera]]", "[[rug|alfombra]]"],
          "correctAnswer": 0,
          "explanation": "Trabajamos en el portátil (laptop) o el ordenador."
        }
      ]
    },
    "audioUrl": "audio/a1/unit-22/e38.mp3",
    "topicName": "Vocabulary"
  },
  {
    "id": "a1-u22-e39",
    "type": "multiple-choice",
    "level": "A1",
    "topic": "Demonstratives",
    "difficulty": "easy",
    "transcript": "Is this your umbrella?",
    "content": {
      "title": "¿Es tuyo?",
      "instructions": "Pregunta por un objeto cercano.",
      "questions": [
        {
          "question": "[[Is|¿Es?]] _______ [[your|tu]] [[umbrella|paraguas]]?",
          "options": ["[[this|este]]", "[[these|estos]]", "[[those|esos]]"],
          "correctAnswer": 0,
          "explanation": "'Umbrella' es singular, por lo que usamos 'this'."
        }
      ]
    },
    "audioUrl": "audio/a1/unit-22/e39.mp3",
    "topicName": "Grammar"
  },
  {
    "id": "a1-u22-e40",
    "type": "multiple-choice",
    "level": "A1",
    "topic": "Possessives",
    "difficulty": "medium",
    "transcript": "These are my parents' glasses.",
    "content": {
      "title": "Posesivo Plural",
      "instructions": "Posesivo para sustantivos que terminan en -s.",
      "questions": [
        {
          "question": "[[These|Estas]] [[are|son]] [[my|mis]] [[parents|padres]]_______ [[glasses|gafas]].",
          "options": ["[['|de (plural)]]", "[[s|error]]", "[[is|es]]"],
          "correctAnswer": 0,
          "explanation": "Para palabras en plural que terminan en 's', solo añadimos el apóstrofe: '."
        }
      ]
    },
    "audioUrl": "audio/a1/unit-22/e40.mp3",
    "topicName": "Grammar"
  },

  // SENTENCE BUILDING (41-50)
  {
    "id": "a1-u22-e41",
    "type": "sentence-building",
    "level": "A1",
    "topic": "Demonstratives",
    "difficulty": "medium",
    "transcript": "This is my favorite book.",
    "content": {
      "title": "Construcción de Oraciones",
      "instructions": "Ordena las palabras.",
      "correctSentence": "[[This|Este]] [[is|es]] [[my|mi]] [[favorite|favorito]] [[book|libro]].",
      "words": ["[[is|es]]", "[[This|Este]]", "[[my|mi]]", "[[book|libro]].", "[[favorite|favorito]]"],
      "explanation": "Estructura: Demostrativo + verbo + adjetivo + sustantivo."
    },
    "audioUrl": "audio/a1/unit-22/e41.mp3",
    "topicName": "Grammar"
  },
  {
    "id": "a1-u22-e42",
    "type": "sentence-building",
    "level": "A1",
    "topic": "Possessives",
    "difficulty": "medium",
    "transcript": "Where is Peter's watch?",
    "content": {
      "title": "Preguntas",
      "instructions": "Ordena las palabras.",
      "correctSentence": "[[Where|Dónde]] [[is|está]] [[Peter|Pedro]]'[[s|de]] [[watch|reloj]]?",
      "words": ["[[is|está]]", "[[watch|reloj]]?", "[[Where|Dónde]]", "[[Peter|Pedro]]", "'[[s|de]]"],
      "explanation": "Orden en preguntas: Palabra interrogativa + verbo + sujeto."
    },
    "audioUrl": "audio/a1/unit-22/e42.mp3",
    "topicName": "Grammar"
  },
  {
    "id": "a1-u22-e43",
    "type": "sentence-building",
    "level": "A1",
    "topic": "Demonstratives",
    "difficulty": "medium",
    "transcript": "These glasses are very expensive.",
    "content": {
      "title": "Demostrativos Plurales",
      "instructions": "Ordena las palabras.",
      "correctSentence": "[[These|Estas]] [[glasses|gafas]] [[are|son]] [[very|muy]] [[expensive|caras]].",
      "words": ["[[are|son]]", "[[glasses|gafas]]", "[[These|Estas]]", "[[expensive|caras]].", "[[very|muy]]"],
      "explanation": "Usamos 'These' con sustantivos en plural."
    },
    "audioUrl": "audio/a1/unit-22/e43.mp3",
    "topicName": "Grammar"
  },
  {
    "id": "a1-u22-e44",
    "type": "sentence-building",
    "level": "A1",
    "topic": "Vocabulary: Home",
    "difficulty": "medium",
    "transcript": "The clock is above the mirror.",
    "content": {
      "title": "Ubicación",
      "instructions": "Ordena las palabras.",
      "correctSentence": "[[The|El]] [[clock|reloj]] [[is|está]] [[above|encima de]] [[the|el]] [[mirror|espejo]].",
      "words": ["[[above|encima de]]", "[[clock|reloj]]", "[[mirror|espejo]].", "[[is|está]]", "[[the|el]]", "[[the|el]]"],
      "explanation": "Preposiciones de lugar para indicar posición relativa."
    },
    "audioUrl": "audio/a1/unit-22/e44.mp3",
    "topicName": "Vocabulary"
  },
  {
    "id": "a1-u22-e45",
    "type": "sentence-building",
    "level": "A1",
    "topic": "Actions",
    "difficulty": "medium",
    "transcript": "Please turn off your telephone.",
    "content": {
      "title": "Peticiones",
      "instructions": "Ordena las palabras.",
      "correctSentence": "[[Please|Por favor]] [[turn off|apaga]] [[your|tu]] [[telephone|teléfono]].",
      "words": ["[[your|tu]]", "[[telephone|teléfono]].", "[[turn off|apaga]]", "[[Please|Por favor]]"],
      "explanation": "Uso del imperativo con 'please'."
    },
    "audioUrl": "audio/a1/unit-22/e45.mp3",
    "topicName": "Vocabulary"
  },
  {
    "id": "a1-u22-e46",
    "type": "sentence-building",
    "level": "A1",
    "topic": "Vocabulary: Bedroom",
    "difficulty": "medium",
    "transcript": "I have a big wardrobe in my room.",
    "content": {
      "title": "Mi Habitación",
      "instructions": "Ordena las palabras.",
      "correctSentence": "[[I|Yo]] [[have|tengo]] [[a|un]] [[big|grande]] [[wardrobe|armario]] [[in|en]] [[my|mi]] [[room|habitación]].",
      "words": ["[[wardrobe|armario]]", "[[in|en]]", "[[my|mi]]", "[[room|habitación]].", "[[big|grande]]", "[[I|Yo]]", "[[have|tengo]]", "[[a|un]]"],
      "explanation": "Estructura: Sujeto + verbo + objeto + lugar."
    },
    "audioUrl": "audio/a1/unit-22/e46.mp3",
    "topicName": "Vocabulary"
  },
  {
    "id": "a1-u22-e47",
    "type": "sentence-building",
    "level": "A1",
    "topic": "Demonstratives",
    "difficulty": "hard",
    "transcript": "Is that expensive watch yours?",
    "content": {
      "title": "Pregunta Compleja",
      "instructions": "Ordena las palabras.",
      "correctSentence": "[[Is|¿Es?]] [[that|ese]] [[expensive|caro]] [[watch|reloj]] [[yours|tuyo]]?",
      "words": ["[[watch|reloj]]", "[[expensive|caro]]", "[[yours|tuyo]]?", "[[Is|¿Es?]]", "[[that|ese]]"],
      "explanation": "Los adjetivos van antes del sustantivo."
    },
    "audioUrl": "audio/a1/unit-22/e47.mp3",
    "topicName": "Grammar"
  },
  {
    "id": "a1-u22-e48",
    "type": "sentence-building",
    "level": "A1",
    "topic": "Vocabulary: Kitchen",
    "difficulty": "medium",
    "transcript": "The forks are in the drawer.",
    "content": {
      "title": "En el Cajón",
      "instructions": "Ordena las palabras.",
      "correctSentence": "[[The|Los]] [[forks|tenedores]] [[are|están]] [[in|en]] [[the|el]] [[drawer|cajón]].",
      "words": ["[[in|en]]", "[[forks|tenedores]]", "[[drawer|cajón]].", "[[The|Los]]", "[[are|están]]", "[[the|el]]"],
      "explanation": "Ubicación de utensilios."
    },
    "audioUrl": "audio/a1/unit-22/e48.mp3",
    "topicName": "Vocabulary"
  },
  {
    "id": "a1-u22-e49",
    "type": "sentence-building",
    "level": "A1",
    "topic": "Demonstratives",
    "difficulty": "medium",
    "transcript": "Those paintings are very old.",
    "content": {
      "title": "Cuadros Antiguos",
      "instructions": "Ordena las palabras.",
      "correctSentence": "[[Those|Aquellos]] [[paintings|cuadros]] [[are|son]] [[very|muy]] [[old|viejos]].",
      "words": ["[[old|viejos]].", "[[are|son]]", "[[paintings|cuadros]]", "[[Those|Aquellos]]", "[[very|muy]]"],
      "explanation": "Uso de 'those' para plural y lejanía."
    },
    "audioUrl": "audio/a1/unit-22/e49.mp3",
    "topicName": "Grammar"
  },
  {
    "id": "a1-u22-e50",
    "type": "sentence-building",
    "level": "A1",
    "topic": "Vocabulary: Home",
    "difficulty": "medium",
    "transcript": "Put the keys on the shelf.",
    "content": {
      "title": "Las Llaves",
      "instructions": "Ordena las palabras.",
      "correctSentence": "[[Put|Pon]] [[the|las]] [[keys|llaves]] [[on|sobre]] [[the|la]] [[shelf|estantería]].",
      "words": ["[[keys|llaves]]", "[[on|sobre]]", "[[shelf|estantería]].", "[[the|las]]", "[[the|la]]", "[[Put|Pon]]"],
      "explanation": "Uso del imperativo."
    },
    "audioUrl": "audio/a1/unit-22/e50.mp3",
    "topicName": "Vocabulary"
  },

  // READING COMPREHENSION (51-60)
  {
    "id": "a1-u22-e51",
    "type": "multiple-choice",
    "level": "A1",
    "topic": "Reading: My Desk",
    "difficulty": "medium",
    "transcript": "Reading: My Desk. I have a big desk in my room. On the desk, there is a modern laptop and a black lamp. My notebooks are next to the computer. There is also a small clock. I like my desk because it is very clean.",
    "content": {
      "title": "Lectura: Mi Escritorio",
      "instructions": "Lee el texto y responde.",
      "questions": [
        {
          "question": "*\"[[I|Yo]] [[have|tengo]] [[a|un]] [[big|grande]] [[desk|escritorio]] [[in|en]] [[my|mi]] [[room|habitación]]. [[On|Sobre]] [[the|el]] [[desk|escritorio]], [[there|allí]] [[is|hay]] [[a|un]] [[modern|moderno]] [[laptop|portátil]] [[and|y]] [[a|una]] [[black|negra]] [[lamp|lámpara]]. [[My|Mis]] [[notebooks|cuadernos]] [[are|están]] [[next to|al lado de]] [[the|el]] [[computer|ordenador]]. [[There|Allí]] [[is|hay]] [[also|también]] [[a|un]] [[small|pequeño]] [[clock|reloj]]. [[I|Yo]] [[like|gusta]] [[my|mi]] [[desk|escritorio]] [[because|porque]] [[it|ello]] [[is|está]] [[very|muy]] [[clean|limpio]].\"*\n\n[[What|De qué]] [[color|color]] [[is|es]] [[the|la]] [[lamp|lámpara]]?",
          "options": ["[[Black|Negra]]", "[[White|Blanca]]", "[[Blue|Azul]]"],
          "correctAnswer": 0,
          "explanation": "El texto dice: 'a black lamp'."
        }
      ]
    },
    "audioUrl": "audio/a1/unit-22/e51.mp3",
    "topicName": "Reading"
  },
  {
    "id": "a1-u22-e52",
    "type": "multiple-choice",
    "level": "A1",
    "topic": "Reading: My Desk",
    "difficulty": "medium",
    "transcript": "Where are the notebooks? Next to the computer.",
    "content": {
      "title": "Lectura: Mi Escritorio",
      "instructions": "Lee el texto anterior y responde.",
      "questions": [
        {
          "question": "[[Where|Dónde]] [[are|están]] [[the|los]] [[notebooks|cuadernos]]?",
          "options": ["[[Under|Debajo de]] [[the|el]] [[desk|escritorio]]", "[[Next to|Al lado de]] [[the|el]] [[computer|ordenador]]", "[[In|En]] [[the|el]] [[wardrobe|armario]]"],
          "correctAnswer": 1,
          "explanation": "El texto dice: 'My notebooks are next to the computer'."
        }
      ]
    },
    "audioUrl": "audio/a1/unit-22/e52.mp3",
    "topicName": "Reading"
  },
  {
    "id": "a1-u22-e53",
    "type": "multiple-choice",
    "level": "A1",
    "topic": "Reading: Sarah's New Camera",
    "difficulty": "medium",
    "transcript": "Reading: Sarah's New Camera. Sarah has a new camera. It is small and very expensive. She takes photos of her dog in the garden. Today, she is happy because her brother's camera is broken, but hers is perfect.",
    "content": {
      "title": "Lectura: La Cámara de Sarah",
      "instructions": "Lee el texto y responde.",
      "questions": [
        {
          "question": "*\"[[Sarah|Sara]] [[has|tiene]] [[a|una]] [[new|nueva]] [[camera|cámara]]. [[It|Ella]] [[is|es]] [[small|pequeña]] [[and|y]] [[very|muy]] [[expensive|cara]]. [[She|Ella]] [[takes|toma]] [[photos|fotos]] [[of|de]] [[her|su]] [[dog|perro]] [[in|en]] [[the|el]] [[garden|jardín]]. [[Today|Hoy]], [[she|ella]] [[is|está]] [[happy|feliz]] [[because|porque]] [[her|su]] [[brother|hermano]]'[[s|de]] [[camera|cámara]] [[is|está]] [[broken|rota]], [[but|pero]] [[hers|la suya]] [[is|está]] [[perfect|perfecta]].\"*\n\n[[Is|¿Es?]] [[the|la]] [[camera|cámara]] [[cheap|barata]]?",
          "options": ["[[Yes, it is.|Sí.]]", "[[No, it is expensive.|No, es cara.]]", "[[It is old.|Es vieja.]]"],
          "correctAnswer": 1,
          "explanation": "El texto dice: 'It is small and very expensive'."
        }
      ]
    },
    "audioUrl": "audio/a1/unit-22/e53.mp3",
    "topicName": "Reading"
  },
  {
    "id": "a1-u22-e54",
    "type": "multiple-choice",
    "level": "A1",
    "topic": "Reading: Sarah's New Camera",
    "difficulty": "medium",
    "transcript": "Whose camera is broken? Her brother's.",
    "content": {
      "title": "Lectura: La Cámara de Sarah",
      "instructions": "Lee el texto anterior y responde.",
      "questions": [
        {
          "question": "[[Whose|De quién]] [[camera|cámara]] [[is|está]] [[broken|rota]]?",
          "options": ["[[Sarah|Sara]]'[[s|de]]", "[[Her|Su]] [[brother|hermano]]'[[s|de]]", "[[Her|Su]] [[father|padre]]'[[s|de]]"],
          "correctAnswer": 1,
          "explanation": "El texto dice: 'her brother's camera is broken'."
        }
      ]
    },
    "audioUrl": "audio/a1/unit-22/e54.mp3",
    "topicName": "Reading"
  },
  {
    "id": "a1-u22-e55",
    "type": "multiple-choice",
    "level": "A1",
    "topic": "Reading: The Kitchen",
    "difficulty": "medium",
    "transcript": "Reading: The Kitchen. Look at this kitchen. It is very modern. These glasses are new, and that fridge is very big. We have many plates and silver forks. Please, put the knives in the drawer.",
    "content": {
      "title": "Lectura: La Cocina",
      "instructions": "Lee el texto y responde.",
      "questions": [
        {
          "question": "*\"[[Look|Mira]] [[at|a]] [[this|esta]] [[kitchen|cocina]]. [[It|Ella]] [[is|es]] [[very|muy]] [[modern|moderna]]. [[These|Estos]] [[glasses|vasos]] [[are|son]] [[new|nuevos]], [[and|y]] [[that|aquella]] [[fridge|nevera]] [[is|es]] [[very|muy]] [[big|grande]]. [[We|Nosotros]] [[have|tenemos]] [[many|muchos]] [[plates|platos]] [[and|y]] [[silver|plateados]] [[forks|tenedores]]. [[Please|Por favor]], [[put|pon]] [[the|los]] [[knives|cuchillos]] [[in|en]] [[the|el]] [[drawer|cajón]].\"*\n\n[[How|Cómo]] [[is|es]] [[the|la]] [[fridge|nevera]]?",
          "options": ["[[Small|Pequeña]]", "[[Big|Grande]]", "[[Broken|Rota]]"],
          "correctAnswer": 1,
          "explanation": "El texto dice: 'that fridge is very big'."
        }
      ]
    },
    "audioUrl": "audio/a1/unit-22/e55.mp3",
    "topicName": "Reading"
  },
  {
    "id": "a1-u22-e56",
    "type": "multiple-choice",
    "level": "A1",
    "topic": "Reading: The Kitchen",
    "difficulty": "medium",
    "transcript": "Where are the knives? In the drawer.",
    "content": {
      "title": "Lectura: La Cocina",
      "instructions": "Lee el texto anterior y responde.",
      "questions": [
        {
          "question": "[[Where|Dónde]] [[are|están]] [[the|los]] [[knives|cuchillos]]?",
          "options": ["[[In|En]] [[the|el]] [[drawer|cajón]]", "[[On|Sobre]] [[the|la]] [[table|mesa]]", "[[Under|Debajo de]] [[the|el]] [[sink|fregadero]]"],
          "correctAnswer": 0,
          "explanation": "El texto dice: 'put the knives in the drawer'."
        }
      ]
    },
    "audioUrl": "audio/a1/unit-22/e56.mp3",
    "topicName": "Reading"
  },
  {
    "id": "a1-u22-e57",
    "type": "multiple-choice",
    "level": "A1",
    "topic": "Reading: Lost Things",
    "difficulty": "medium",
    "transcript": "Reading: Lost Things. Tom is very tired. He can't find his things. 'Where is my watch?' he asks. His watch is on the shelf. 'And where are my keys?' Those keys are under the sofa. Oh Tom!",
    "content": {
      "title": "Lectura: Cosas Perdidas",
      "instructions": "Lee el texto y responde.",
      "questions": [
        {
          "question": "*\"[[Tom|Tom]] [[is|está]] [[very|muy]] [[tired|cansado]]. [[He|Él]] [[can't|no puede]] [[find|encontrar]] [[his|sus]] [[things|cosas]]. '[[Where|Dónde]] [[is|está]] [[my|mi]] [[watch|reloj]]?' [[he|él]] [[asks|pregunta]]. [[His|Su]] [[watch|reloj]] [[is|está]] [[on|sobre]] [[the|la]] [[shelf|estantería]]. '[[And|Y]] [[where|dónde]] [[are|están]] [[my|mis]] [[keys|llaves]]?' [[Those|Aquellas]] [[keys|llaves]] [[are|están]] [[under|debajo de]] [[the|el]] [[sofa|sofá]]. [[Oh|Oh]] [[Tom|Tom]]!\"*\n\n[[Where|Dónde]] [[is|está]] [[Tom|Tom]]'[[s|de]] [[watch|reloj]]?",
          "options": ["[[Under|Debajo de]] [[the|el]] [[sofa|sofá]]", "[[On|Sobre]] [[the|la]] [[shelf|estantería]]", "[[In|En]] [[his|su]] [[pocket|bolsillo]]"],
          "correctAnswer": 1,
          "explanation": "El texto dice: 'His watch is on the shelf'."
        }
      ]
    },
    "audioUrl": "audio/a1/unit-22/e57.mp3",
    "topicName": "Reading"
  },
  {
    "id": "a1-u22-e58",
    "type": "multiple-choice",
    "level": "A1",
    "topic": "Reading: Lost Things",
    "difficulty": "medium",
    "transcript": "Where are the keys? Under the sofa.",
    "content": {
      "title": "Lectura: Cosas Perdidas",
      "instructions": "Lee el texto anterior y responde.",
      "questions": [
        {
          "question": "[[Where|Dónde]] [[are|están]] [[the|las]] [[keys|llaves]]?",
          "options": ["[[On|Sobre]] [[the|la]] [[shelf|estantería]]", "[[Under|Debajo de]] [[the|el]] [[sofa|sofá]]", "[[Behind|Detrás de]] [[the|la]] [[door|puerta]]"],
          "correctAnswer": 1,
          "explanation": "El texto dice: 'Those keys are under the sofa'."
        }
      ]
    },
    "audioUrl": "audio/a1/unit-22/e58.mp3",
    "topicName": "Reading"
  },
  {
    "id": "a1-u22-e59",
    "type": "multiple-choice",
    "level": "A1",
    "topic": "Reading: Home Office",
    "difficulty": "medium",
    "transcript": "Reading: Home Office. This is my office. I have a new computer and a small telephone. These books on the desk are very interesting. That big bookshelf is for my documents. I use my office every day.",
    "content": {
      "title": "Lectura: Oficina en Casa",
      "instructions": "Lee el texto y responde.",
      "questions": [
        {
          "question": "*\"[[This|Esta]] [[is|es]] [[my|mi]] [[office|oficina]]. [[I|Yo]] [[have|tengo]] [[a|un]] [[new|nuevo]] [[computer|ordenador]] [[and|y]] [[a|un]] [[small|pequeño]] [[telephone|teléfono]]. [[These|Estos]] [[books|libros]] [[on|sobre]] [[the|el]] [[desk|escritorio]] [[are|son]] [[very|muy]] [[interesting|interesantes]]. [[That|Aquella]] [[big|grande]] [[bookshelf|librería]] [[is|es]] [[for|para]] [[my|mis]] [[documents|documentos]]. [[I|Yo]] [[use|uso]] [[my|mi]] [[office|oficina]] [[every|cada]] [[day|día]].\"*\n\n[[How|Cómo]] [[is|es]] [[the|el]] [[telephone|teléfono]]?",
          "options": ["[[Small|Pequeño]]", "[[Big|Grande]]", "[[Broken|Roto]]"],
          "correctAnswer": 0,
          "explanation": "El texto dice: 'a small telephone'."
        }
      ]
    },
    "audioUrl": "audio/a1/unit-22/e59.mp3",
    "topicName": "Reading"
  },
  {
    "id": "a1-u22-e60",
    "type": "multiple-choice",
    "level": "A1",
    "topic": "Reading: Home Office",
    "difficulty": "medium",
    "transcript": "What is the big bookshelf for? For documents.",
    "content": {
      "title": "Lectura: Oficina en Casa",
      "instructions": "Lee el texto anterior y responde.",
      "questions": [
        {
          "question": "[[What|Para qué]] [[is|es]] [[the|la]] [[big|grande]] [[bookshelf|librería]] [[for|para]]?",
          "options": ["[[For|Para]] [[books|libros]]", "[[For|Para]] [[documents|documentos]]", "[[For|Para]] [[photos|fotos]]"],
          "correctAnswer": 1,
          "explanation": "El texto dice: 'That big bookshelf is for my documents'."
        }
      ]
    },
    "audioUrl": "audio/a1/unit-22/e60.mp3",
    "topicName": "Reading"
  }
];
