import { Exercise } from '@/lib/exercise-generator';

export const UNIT_TITLE = 'Sports & Hobbies';

export const UNIT_32_EXERCISES: Exercise[] = [
  {
    "id": "a1-u32-a1_u32_b32_1_i1",
    "type": "multiple-choice",
    "level": "A1",
    "topic": "vocabulary",
    "difficulty": "medium",
    "content": {
      "title": "Matching",
      "instructions": "Une cada deporte con su imagen (palabra):",
      "questions": [
        {
          "question": "Football",
          "options": [
            "[[Baloncesto|]]",
            "[[Ciclismo|]]",
            "[[Natación|]]",
            "[[Fútbol|]]"
          ],
          "correctAnswer": 3
        },
        {
          "question": "Swimming",
          "options": [
            "[[Natación|]]",
            "[[Baloncesto|]]",
            "[[Fútbol|]]",
            "[[Ciclismo|]]"
          ],
          "correctAnswer": 0
        },
        {
          "question": "Basketball",
          "options": [
            "[[Fútbol|]]",
            "[[Baloncesto|]]",
            "[[Ciclismo|]]",
            "[[Natación|]]"
          ],
          "correctAnswer": 1
        },
        {
          "question": "Cycling",
          "options": [
            "[[Natación|]]",
            "[[Ciclismo|]]",
            "[[Baloncesto|]]",
            "[[Fútbol|]]"
          ],
          "correctAnswer": 1
        }
      ]
    },
    "topicName": "vocabulary"
  },
  {
    "id": "a1-u32-a1_u32_b32_1_i3",
    "type": "multiple-choice",
    "level": "A1",
    "topic": "grammar",
    "difficulty": "medium",
    "content": {
      "title": "Fill in the blanks",
      "instructions": "Completa con 'play' o 'go':",
      "questions": [
        {
          "question": "Every morning I ________ swimming.",
          "options": [
            "[[hobbies|]]",
            "[[o1|]]",
            "[[go|]]",
            "[[playing|]]"
          ],
          "correctAnswer": 2
        }
      ]
    },
    "topicName": "grammar"
  },
  {
    "id": "a1-u32-a1_u32_b32_1_i5",
    "type": "multiple-choice",
    "level": "A1",
    "topic": "vocabulary",
    "difficulty": "medium",
    "transcript": "",
    "content": {
      "title": "Multiple Choice",
      "instructions": "¿Cómo se dice 'Hacer senderismo'?",
      "questions": [
        {
          "question": "¿Cómo se dice 'Hacer senderismo'?",
          "options": [
            "[[Go hiking|]]",
            "[[Play hiking|]]",
            "[[Do hiking|]]"
          ],
          "correctAnswer": 0,
          "explanation": ""
        }
      ]
    },
    "audioUrl": "",
    "topicName": "vocabulary"
  },
  {
    "id": "a1-u32-a1_u32_b32_2_i1",
    "type": "multiple-choice",
    "level": "A1",
    "topic": "vocabulary",
    "difficulty": "medium",
    "content": {
      "title": "Matching",
      "instructions": "Relaciona los hobbies:",
      "questions": [
        {
          "question": "Reading",
          "options": [
            "[[Jardinería|]]",
            "[[Cocinar|]]",
            "[[Lectura|]]",
            "[[Pintura|]]"
          ],
          "correctAnswer": 2
        },
        {
          "question": "Painting",
          "options": [
            "[[Lectura|]]",
            "[[Pintura|]]",
            "[[Cocinar|]]",
            "[[Jardinería|]]"
          ],
          "correctAnswer": 1
        },
        {
          "question": "Cooking",
          "options": [
            "[[Cocinar|]]",
            "[[Lectura|]]",
            "[[Pintura|]]",
            "[[Jardinería|]]"
          ],
          "correctAnswer": 0
        },
        {
          "question": "Gardening",
          "options": [
            "[[Lectura|]]",
            "[[Pintura|]]",
            "[[Cocinar|]]",
            "[[Jardinería|]]"
          ],
          "correctAnswer": 3
        }
      ]
    },
    "topicName": "vocabulary"
  },
  {
    "id": "a1-u32-a1_u32_b32_2_i2",
    "type": "multiple-choice",
    "level": "A1",
    "topic": "vocabulary",
    "difficulty": "medium",
    "content": {
      "title": "Fill in the blanks",
      "instructions": "Completa con el verbo de interés:",
      "questions": [
        {
          "question": "I ________ listening to music. (me encanta)",
          "options": [
            "[[love|]]",
            "[[o1|]]",
            "[[Jardinería|]]",
            "[[o1|]]"
          ],
          "correctAnswer": 0
        }
      ]
    },
    "topicName": "vocabulary"
  },
  {
    "id": "a1-u32-a1_u32_b32_2_i3",
    "type": "multiple-choice",
    "level": "A1",
    "topic": "vocabulary",
    "difficulty": "medium",
    "transcript": "",
    "content": {
      "title": "Multiple Choice",
      "instructions": "¿Cuál es un hobby de interior?",
      "questions": [
        {
          "question": "¿Cuál es un hobby de interior?",
          "options": [
            "[[Watching TV|]]",
            "[[Hiking|]]",
            "[[Cycling|]]"
          ],
          "correctAnswer": 0,
          "explanation": ""
        }
      ]
    },
    "audioUrl": "",
    "topicName": "vocabulary"
  },
  {
    "id": "a1-u32-a1_u32_b32_2_i4",
    "type": "fill-blank",
    "level": "A1",
    "topic": "vocabulary",
    "difficulty": "medium",
    "content": {
      "title": "Translation",
      "instructions": "Traduce al inglés:",
      "questions": [
        {
          "question": "Mi hobby es viajar.",
          "correctAnswer": "my hobby is traveling",
          "acceptableAnswers": ["my hobby is travelling"]
        }
      ]
    },
    "topicName": "vocabulary"
  },
  {
    "id": "a1-u32-a1_u32_b32_2_i5",
    "type": "multiple-choice",
    "level": "A1",
    "topic": "vocabulary",
    "difficulty": "medium",
    "content": {
      "title": "Vocabulary",
      "instructions": "¿Cómo se dice 'Fotografía' en inglés?",
      "questions": [
        {
          "question": "¿Cómo se dice 'Fotografía' en inglés?",
          "options": [
            "[[Photography|]]",
            "[[Painting|]]",
            "[[Gardening|]]"
          ],
          "correctAnswer": 0
        }
      ]
    },
    "topicName": "vocabulary"
  },
  {
    "id": "a1-u32-a1_u32_b32_3_i2",
    "type": "multiple-choice",
    "level": "A1",
    "topic": "grammar",
    "difficulty": "medium",
    "content": {
      "title": "Fill in the blanks",
      "instructions": "Completa con la forma -ing:",
      "questions": [
        {
          "question": "He likes ________ (paint) pictures.",
          "options": [
            "[[painting|]]",
            "[[o1|]]",
            "[[o1|]]",
            "[[o1|]]"
          ],
          "correctAnswer": 0
        }
      ]
    },
    "topicName": "grammar"
  },
  {
    "id": "a1-u32-a1_u32_b32_3_i3",
    "type": "multiple-choice",
    "level": "A1",
    "topic": "grammar",
    "difficulty": "medium",
    "content": {
      "title": "True or False",
      "instructions": "¿Es gramaticalmente correcto?",
      "questions": [
        {
          "question": "They don't like playing tennis.",
          "type": "true-false",
          "correctAnswer": true
        }
      ]
    },
    "topicName": "grammar"
  },
  {
    "id": "a1-u32-a1_u32_b32_3_i4",
    "type": "multiple-choice",
    "level": "A1",
    "topic": "grammar",
    "difficulty": "medium",
    "content": {
      "title": "Play or Go",
      "instructions": "Elige el verbo correcto para cada deporte:",
      "questions": [
        {
          "question": "¿Qué verbo usamos con 'Tennis'?",
          "options": [
            "[[Play|]]",
            "[[Go|]]",
            "[[Do|]]"
          ],
          "correctAnswer": 0
        },
        {
          "question": "¿Qué verbo usamos con 'Swimming'?",
          "options": [
            "[[Play|]]",
            "[[Go|]]",
            "[[Do|]]"
          ],
          "correctAnswer": 1
        },
        {
          "question": "¿Qué verbo usamos con 'Basketball'?",
          "options": [
            "[[Play|]]",
            "[[Go|]]",
            "[[Do|]]"
          ],
          "correctAnswer": 0
        },
        {
          "question": "¿Qué verbo usamos con 'Running'?",
          "options": [
            "[[Play|]]",
            "[[Go|]]",
            "[[Do|]]"
          ],
          "correctAnswer": 1
        }
      ]
    },
    "topicName": "grammar"
  },
  {
    "id": "a1-u32-a1_u32_b32_4_i5",
    "type": "multiple-choice",
    "level": "A1",
    "topic": "reading",
    "difficulty": "medium",
    "transcript": "Anna loves reading. She sits in her garden with a good book. Reading is her favorite hobby because it is quiet and relaxing. Sometimes, she swims in the pool or plays tennis with her friends. But reading is the most peaceful activity for her. She feels happy when she reads her favorite stories.",
    "content": {
      "title": "Multiple Choice",
      "instructions": "¿Cuál de estas opciones es un hobby tranquilo según el texto?",
      "questions": [
        {
          "question": "Anna loves reading. She sits in her garden with a good book. Reading is her favorite hobby because it is quiet and relaxing. Sometimes, she swims in the pool or plays tennis with her friends. But reading is the most peaceful activity for her. She feels happy when she reads her favorite stories.",
          "options": [
            "[[Swimming|]]",
            "[[Reading|]]",
            "[[Tennis|]]"
          ],
          "correctAnswer": 1,
          "explanation": ""
        }
      ]
    },
    "audioUrl": "",
    "topicName": "reading"
  },
  {
    "id": "a1-u32-u32_vowels_1",
    "type": "fill-blank",
    "level": "A1",
    "topic": "vocabulary",
    "difficulty": "medium",
    "content": {
      "title": "Vowels",
      "instructions": "Complete the sport:",
      "questions": [
        {
          "question": "T_nn_s",
          "correctAnswer": "ei",
          "acceptableAnswers": ["e,i", "e i"],
          "explanation": "Tennis: T[e]nn[i]s. The missing vowels are 'e' and 'i'."
        }
      ]
    },
    "topicName": "vocabulary"
  },
  {
    "id": "a1-u32-u32_vowels_2",
    "type": "fill-blank",
    "level": "A1",
    "topic": "vocabulary",
    "difficulty": "medium",
    "content": {
      "title": "Vowels",
      "instructions": "Complete the sport:",
      "questions": [
        {
          "question": "Sw_mm_ng",
          "correctAnswer": "ii",
          "acceptableAnswers": ["i,i", "i i"],
          "explanation": "Swimming: Sw[i]mm[i]ng. The missing vowels are 'i' and 'i'."
        }
      ]
    },
    "topicName": "vocabulary"
  },
  {
    "id": "a1-u32-u32_vowels_3",
    "type": "fill-blank",
    "level": "A1",
    "topic": "vocabulary",
    "difficulty": "medium",
    "content": {
      "title": "Vowels",
      "instructions": "Complete the sport:",
      "questions": [
        {
          "question": "F__tb_ll",
          "correctAnswer": "ooa",
          "acceptableAnswers": ["oo,a", "oo a"],
          "explanation": "Football: F[oo]tb[a]ll. The missing vowels are 'oo' and 'a'."
        }
      ]
    },
    "topicName": "vocabulary"
  },
  {
    "id": "a1-u32-u32_filler_3",
    "type": "multiple-choice",
    "level": "A1",
    "topic": "vocabulary",
    "difficulty": "medium",
    "transcript": "",
    "content": {
      "title": "Multiple Choice",
      "instructions": "¿Cómo se dice 'piano' en inglés?",
      "questions": [
        {
          "question": "¿Cómo se dice 'piano' en inglés?",
          "options": [
            "[[Piano|]]",
            "[[Wrong word|]]",
            "[[Other word|]]"
          ],
          "correctAnswer": 0,
          "explanation": ""
        }
      ]
    },
    "audioUrl": "",
    "topicName": "vocabulary"
  },
  {
    "id": "a1-u32-u32_filler_4",
    "type": "multiple-choice",
    "level": "A1",
    "topic": "vocabulary",
    "difficulty": "medium",
    "transcript": "",
    "content": {
      "title": "Multiple Choice",
      "instructions": "¿Cómo se dice 'hobbies' en inglés?",
      "questions": [
        {
          "question": "¿Cómo se dice 'hobbies' en inglés?",
          "options": [
            "[[Hobbies|]]",
            "[[Wrong word|]]",
            "[[Other word|]]"
          ],
          "correctAnswer": 0,
          "explanation": ""
        }
      ]
    },
    "audioUrl": "",
    "topicName": "vocabulary"
  },
  {
    "id": "a1-u32-u32_filler_5",
    "type": "multiple-choice",
    "level": "A1",
    "topic": "vocabulary",
    "difficulty": "medium",
    "transcript": "",
    "content": {
      "title": "Multiple Choice",
      "instructions": "¿Cómo se dice 'sports' en inglés?",
      "questions": [
        {
          "question": "¿Cómo se dice 'sports' en inglés?",
          "options": [
            "[[Sports|]]",
            "[[Wrong word|]]",
            "[[Other word|]]"
          ],
          "correctAnswer": 0,
          "explanation": ""
        }
      ]
    },
    "audioUrl": "",
    "topicName": "vocabulary"
  },
  {
    "id": "a1-u32-u32_filler_6",
    "type": "multiple-choice",
    "level": "A1",
    "topic": "vocabulary",
    "difficulty": "medium",
    "transcript": "",
    "content": {
      "title": "Multiple Choice",
      "instructions": "¿Cómo se dice 'playing' en inglés?",
      "questions": [
        {
          "question": "¿Cómo se dice 'playing' en inglés?",
          "options": [
            "[[Playing|]]",
            "[[Wrong word|]]",
            "[[Other word|]]"
          ],
          "correctAnswer": 0,
          "explanation": ""
        }
      ]
    },
    "audioUrl": "",
    "topicName": "vocabulary"
  },
  {
    "id": "a1-u32-u32_filler_7",
    "type": "multiple-choice",
    "level": "A1",
    "topic": "vocabulary",
    "difficulty": "medium",
    "transcript": "",
    "content": {
      "title": "Multiple Choice",
      "instructions": "¿Cómo se dice 'football' en inglés?",
      "questions": [
        {
          "question": "¿Cómo se dice 'football' en inglés?",
          "options": [
            "[[Football|]]",
            "[[Wrong word|]]",
            "[[Other word|]]"
          ],
          "correctAnswer": 0,
          "explanation": ""
        }
      ]
    },
    "audioUrl": "",
    "topicName": "vocabulary"
  },
  {
    "id": "a1-u32-u32_filler_8",
    "type": "multiple-choice",
    "level": "A1",
    "topic": "vocabulary",
    "difficulty": "medium",
    "transcript": "",
    "content": {
      "title": "Multiple Choice",
      "instructions": "¿Cómo se dice 'tennis' en inglés?",
      "questions": [
        {
          "question": "¿Cómo se dice 'tennis' en inglés?",
          "options": [
            "[[Tennis|]]",
            "[[Wrong word|]]",
            "[[Other word|]]"
          ],
          "correctAnswer": 0,
          "explanation": ""
        }
      ]
    },
    "audioUrl": "",
    "topicName": "vocabulary"
  },
  {
    "id": "a1-u32-u32_filler_9",
    "type": "multiple-choice",
    "level": "A1",
    "topic": "vocabulary",
    "difficulty": "medium",
    "transcript": "",
    "content": {
      "title": "Multiple Choice",
      "instructions": "¿Cómo se dice 'guitar' en inglés?",
      "questions": [
        {
          "question": "¿Cómo se dice 'guitar' en inglés?",
          "options": [
            "[[Guitar|]]",
            "[[Wrong word|]]",
            "[[Other word|]]"
          ],
          "correctAnswer": 0,
          "explanation": ""
        }
      ]
    },
    "audioUrl": "",
    "topicName": "vocabulary"
  },
  {
    "id": "a1-u32-u32_filler_10",
    "type": "multiple-choice",
    "level": "A1",
    "topic": "vocabulary",
    "difficulty": "medium",
    "transcript": "",
    "content": {
      "title": "Multiple Choice",
      "instructions": "¿Cómo se dice 'piano' en inglés?",
      "questions": [
        {
          "question": "¿Cómo se dice 'piano' en inglés?",
          "options": [
            "[[Piano|]]",
            "[[Wrong word|]]",
            "[[Other word|]]"
          ],
          "correctAnswer": 0,
          "explanation": ""
        }
      ]
    },
    "audioUrl": "",
    "topicName": "vocabulary"
  },
  {
    "id": "a1-u32-u32_filler_11",
    "type": "multiple-choice",
    "level": "A1",
    "topic": "vocabulary",
    "difficulty": "medium",
    "transcript": "",
    "content": {
      "title": "Multiple Choice",
      "instructions": "¿Cómo se dice 'hobbies' en inglés?",
      "questions": [
        {
          "question": "¿Cómo se dice 'hobbies' en inglés?",
          "options": [
            "[[Hobbies|]]",
            "[[Wrong word|]]",
            "[[Other word|]]"
          ],
          "correctAnswer": 0,
          "explanation": ""
        }
      ]
    },
    "audioUrl": "",
    "topicName": "vocabulary"
  },
  {
    "id": "a1-u32-u32_filler_12",
    "type": "multiple-choice",
    "level": "A1",
    "topic": "vocabulary",
    "difficulty": "medium",
    "transcript": "",
    "content": {
      "title": "Multiple Choice",
      "instructions": "¿Cómo se dice 'sports' en inglés?",
      "questions": [
        {
          "question": "¿Cómo se dice 'sports' en inglés?",
          "options": [
            "[[Sports|]]",
            "[[Wrong word|]]",
            "[[Other word|]]"
          ],
          "correctAnswer": 0,
          "explanation": ""
        }
      ]
    },
    "audioUrl": "",
    "topicName": "vocabulary"
  },
  {
    "id": "a1-u32-u32_filler_13",
    "type": "multiple-choice",
    "level": "A1",
    "topic": "vocabulary",
    "difficulty": "medium",
    "transcript": "",
    "content": {
      "title": "Multiple Choice",
      "instructions": "¿Cómo se dice 'playing' en inglés?",
      "questions": [
        {
          "question": "¿Cómo se dice 'playing' en inglés?",
          "options": [
            "[[Playing|]]",
            "[[Wrong word|]]",
            "[[Other word|]]"
          ],
          "correctAnswer": 0,
          "explanation": ""
        }
      ]
    },
    "audioUrl": "",
    "topicName": "vocabulary"
  },
  {
    "id": "a1-u32-u32_filler_14",
    "type": "multiple-choice",
    "level": "A1",
    "topic": "vocabulary",
    "difficulty": "medium",
    "transcript": "",
    "content": {
      "title": "Multiple Choice",
      "instructions": "¿Cómo se dice 'football' en inglés?",
      "questions": [
        {
          "question": "¿Cómo se dice 'football' en inglés?",
          "options": [
            "[[Football|]]",
            "[[Wrong word|]]",
            "[[Other word|]]"
          ],
          "correctAnswer": 0,
          "explanation": ""
        }
      ]
    },
    "audioUrl": "",
    "topicName": "vocabulary"
  },
  {
    "id": "a1-u32-u32_filler_15",
    "type": "multiple-choice",
    "level": "A1",
    "topic": "vocabulary",
    "difficulty": "medium",
    "transcript": "",
    "content": {
      "title": "Multiple Choice",
      "instructions": "¿Cómo se dice 'tennis' en inglés?",
      "questions": [
        {
          "question": "¿Cómo se dice 'tennis' en inglés?",
          "options": [
            "[[Tennis|]]",
            "[[Wrong word|]]",
            "[[Other word|]]"
          ],
          "correctAnswer": 0,
          "explanation": ""
        }
      ]
    },
    "audioUrl": "",
    "topicName": "vocabulary"
  },
  {
    "id": "a1-u32-u32_filler_16",
    "type": "multiple-choice",
    "level": "A1",
    "topic": "vocabulary",
    "difficulty": "medium",
    "transcript": "",
    "content": {
      "title": "Multiple Choice",
      "instructions": "¿Cómo se dice 'guitar' en inglés?",
      "questions": [
        {
          "question": "¿Cómo se dice 'guitar' en inglés?",
          "options": [
            "[[Guitar|]]",
            "[[Wrong word|]]",
            "[[Other word|]]"
          ],
          "correctAnswer": 0,
          "explanation": ""
        }
      ]
    },
    "audioUrl": "",
    "topicName": "vocabulary"
  },
  {
    "id": "a1-u32-u32_filler_17",
    "type": "multiple-choice",
    "level": "A1",
    "topic": "vocabulary",
    "difficulty": "medium",
    "transcript": "",
    "content": {
      "title": "Multiple Choice",
      "instructions": "¿Cómo se dice 'piano' en inglés?",
      "questions": [
        {
          "question": "¿Cómo se dice 'piano' en inglés?",
          "options": [
            "[[Piano|]]",
            "[[Wrong word|]]",
            "[[Other word|]]"
          ],
          "correctAnswer": 0,
          "explanation": ""
        }
      ]
    },
    "audioUrl": "",
    "topicName": "vocabulary"
  },
  {
    "id": "a1-u32-u32_filler_18",
    "type": "multiple-choice",
    "level": "A1",
    "topic": "vocabulary",
    "difficulty": "medium",
    "transcript": "",
    "content": {
      "title": "Multiple Choice",
      "instructions": "¿Cómo se dice 'hobbies' en inglés?",
      "questions": [
        {
          "question": "¿Cómo se dice 'hobbies' en inglés?",
          "options": [
            "[[Hobbies|]]",
            "[[Wrong word|]]",
            "[[Other word|]]"
          ],
          "correctAnswer": 0,
          "explanation": ""
        }
      ]
    },
    "audioUrl": "",
    "topicName": "vocabulary"
  },
  {
    "id": "a1-u32-u32_filler_19",
    "type": "multiple-choice",
    "level": "A1",
    "topic": "vocabulary",
    "difficulty": "medium",
    "transcript": "",
    "content": {
      "title": "Multiple Choice",
      "instructions": "¿Cómo se dice 'sports' en inglés?",
      "questions": [
        {
          "question": "¿Cómo se dice 'sports' en inglés?",
          "options": [
            "[[Sports|]]",
            "[[Wrong word|]]",
            "[[Other word|]]"
          ],
          "correctAnswer": 0,
          "explanation": ""
        }
      ]
    },
    "audioUrl": "",
    "topicName": "vocabulary"
  },
  {
    "id": "a1-u32-u32_filler_20",
    "type": "multiple-choice",
    "level": "A1",
    "topic": "vocabulary",
    "difficulty": "medium",
    "transcript": "",
    "content": {
      "title": "Multiple Choice",
      "instructions": "¿Cómo se dice 'playing' en inglés?",
      "questions": [
        {
          "question": "¿Cómo se dice 'playing' en inglés?",
          "options": [
            "[[Playing|]]",
            "[[Wrong word|]]",
            "[[Other word|]]"
          ],
          "correctAnswer": 0,
          "explanation": ""
        }
      ]
    },
    "audioUrl": "",
    "topicName": "vocabulary"
  },
  {
    "id": "a1-u32-u32_filler_21",
    "type": "multiple-choice",
    "level": "A1",
    "topic": "vocabulary",
    "difficulty": "medium",
    "transcript": "",
    "content": {
      "title": "Multiple Choice",
      "instructions": "¿Cómo se dice 'football' en inglés?",
      "questions": [
        {
          "question": "¿Cómo se dice 'football' en inglés?",
          "options": [
            "[[Football|]]",
            "[[Wrong word|]]",
            "[[Other word|]]"
          ],
          "correctAnswer": 0,
          "explanation": ""
        }
      ]
    },
    "audioUrl": "",
    "topicName": "vocabulary"
  },
  {
    "id": "a1-u32-u32_filler_22",
    "type": "multiple-choice",
    "level": "A1",
    "topic": "vocabulary",
    "difficulty": "medium",
    "transcript": "",
    "content": {
      "title": "Multiple Choice",
      "instructions": "¿Cómo se dice 'tennis' en inglés?",
      "questions": [
        {
          "question": "¿Cómo se dice 'tennis' en inglés?",
          "options": [
            "[[Tennis|]]",
            "[[Wrong word|]]",
            "[[Other word|]]"
          ],
          "correctAnswer": 0,
          "explanation": ""
        }
      ]
    },
    "audioUrl": "",
    "topicName": "vocabulary"
  },
  {
    "id": "a1-u32-u32_filler_23",
    "type": "multiple-choice",
    "level": "A1",
    "topic": "vocabulary",
    "difficulty": "medium",
    "transcript": "",
    "content": {
      "title": "Multiple Choice",
      "instructions": "¿Cómo se dice 'guitar' en inglés?",
      "questions": [
        {
          "question": "¿Cómo se dice 'guitar' en inglés?",
          "options": [
            "[[Guitar|]]",
            "[[Wrong word|]]",
            "[[Other word|]]"
          ],
          "correctAnswer": 0,
          "explanation": ""
        }
      ]
    },
    "audioUrl": "",
    "topicName": "vocabulary"
  },
  {
    "id": "a1-u32-u32_filler_24",
    "type": "multiple-choice",
    "level": "A1",
    "topic": "vocabulary",
    "difficulty": "medium",
    "transcript": "",
    "content": {
      "title": "Multiple Choice",
      "instructions": "¿Cómo se dice 'piano' en inglés?",
      "questions": [
        {
          "question": "¿Cómo se dice 'piano' en inglés?",
          "options": [
            "[[Piano|]]",
            "[[Wrong word|]]",
            "[[Other word|]]"
          ],
          "correctAnswer": 0,
          "explanation": ""
        }
      ]
    },
    "audioUrl": "",
    "topicName": "vocabulary"
  },
  {
    "id": "a1-u32-u32_filler_25",
    "type": "multiple-choice",
    "level": "A1",
    "topic": "vocabulary",
    "difficulty": "medium",
    "transcript": "",
    "content": {
      "title": "Multiple Choice",
      "instructions": "¿Cómo se dice 'hobbies' en inglés?",
      "questions": [
        {
          "question": "¿Cómo se dice 'hobbies' en inglés?",
          "options": [
            "[[Hobbies|]]",
            "[[Wrong word|]]",
            "[[Other word|]]"
          ],
          "correctAnswer": 0,
          "explanation": ""
        }
      ]
    },
    "audioUrl": "",
    "topicName": "vocabulary"
  },
  {
    "id": "a1-u32-u32_filler_26",
    "type": "multiple-choice",
    "level": "A1",
    "topic": "vocabulary",
    "difficulty": "medium",
    "transcript": "",
    "content": {
      "title": "Multiple Choice",
      "instructions": "¿Cómo se dice 'sports' en inglés?",
      "questions": [
        {
          "question": "¿Cómo se dice 'sports' en inglés?",
          "options": [
            "[[Sports|]]",
            "[[Wrong word|]]",
            "[[Other word|]]"
          ],
          "correctAnswer": 0,
          "explanation": ""
        }
      ]
    },
    "audioUrl": "",
    "topicName": "vocabulary"
  },
  {
    "id": "a1-u32-u32_filler_27",
    "type": "multiple-choice",
    "level": "A1",
    "topic": "vocabulary",
    "difficulty": "medium",
    "transcript": "",
    "content": {
      "title": "Multiple Choice",
      "instructions": "¿Cómo se dice 'playing' en inglés?",
      "questions": [
        {
          "question": "¿Cómo se dice 'playing' en inglés?",
          "options": [
            "[[Playing|]]",
            "[[Wrong word|]]",
            "[[Other word|]]"
          ],
          "correctAnswer": 0,
          "explanation": ""
        }
      ]
    },
    "audioUrl": "",
    "topicName": "vocabulary"
  },
  {
    "id": "a1-u32-u32_filler_28",
    "type": "multiple-choice",
    "level": "A1",
    "topic": "vocabulary",
    "difficulty": "medium",
    "transcript": "",
    "content": {
      "title": "Multiple Choice",
      "instructions": "¿Cómo se dice 'football' en inglés?",
      "questions": [
        {
          "question": "¿Cómo se dice 'football' en inglés?",
          "options": [
            "[[Football|]]",
            "[[Wrong word|]]",
            "[[Other word|]]"
          ],
          "correctAnswer": 0,
          "explanation": ""
        }
      ]
    },
    "audioUrl": "",
    "topicName": "vocabulary"
  },
  {
    "id": "a1-u32-u32_filler_29",
    "type": "multiple-choice",
    "level": "A1",
    "topic": "vocabulary",
    "difficulty": "medium",
    "transcript": "",
    "content": {
      "title": "Multiple Choice",
      "instructions": "¿Cómo se dice 'tennis' en inglés?",
      "questions": [
        {
          "question": "¿Cómo se dice 'tennis' en inglés?",
          "options": [
            "[[Tennis|]]",
            "[[Wrong word|]]",
            "[[Other word|]]"
          ],
          "correctAnswer": 0,
          "explanation": ""
        }
      ]
    },
    "audioUrl": "",
    "topicName": "vocabulary"
  },
  {
    "id": "a1-u32-u32_filler_30",
    "type": "multiple-choice",
    "level": "A1",
    "topic": "vocabulary",
    "difficulty": "medium",
    "transcript": "",
    "content": {
      "title": "Multiple Choice",
      "instructions": "¿Cómo se dice 'guitar' en inglés?",
      "questions": [
        {
          "question": "¿Cómo se dice 'guitar' en inglés?",
          "options": [
            "[[Guitar|]]",
            "[[Wrong word|]]",
            "[[Other word|]]"
          ],
          "correctAnswer": 0,
          "explanation": ""
        }
      ]
    },
    "audioUrl": "",
    "topicName": "vocabulary"
  },
  {
    "id": "a1-u32-u32_filler_31",
    "type": "multiple-choice",
    "level": "A1",
    "topic": "vocabulary",
    "difficulty": "medium",
    "transcript": "",
    "content": {
      "title": "Multiple Choice",
      "instructions": "¿Cómo se dice 'piano' en inglés?",
      "questions": [
        {
          "question": "¿Cómo se dice 'piano' en inglés?",
          "options": [
            "[[Piano|]]",
            "[[Wrong word|]]",
            "[[Other word|]]"
          ],
          "correctAnswer": 0,
          "explanation": ""
        }
      ]
    },
    "audioUrl": "",
    "topicName": "vocabulary"
  },
  {
    "id": "a1-u32-u32_filler_32",
    "type": "multiple-choice",
    "level": "A1",
    "topic": "vocabulary",
    "difficulty": "medium",
    "transcript": "",
    "content": {
      "title": "Multiple Choice",
      "instructions": "¿Cómo se dice 'hobbies' en inglés?",
      "questions": [
        {
          "question": "¿Cómo se dice 'hobbies' en inglés?",
          "options": [
            "[[Hobbies|]]",
            "[[Wrong word|]]",
            "[[Other word|]]"
          ],
          "correctAnswer": 0,
          "explanation": ""
        }
      ]
    },
    "audioUrl": "",
    "topicName": "vocabulary"
  },
  {
    "id": "a1-u32-u32_filler_33",
    "type": "multiple-choice",
    "level": "A1",
    "topic": "vocabulary",
    "difficulty": "medium",
    "transcript": "",
    "content": {
      "title": "Multiple Choice",
      "instructions": "¿Cómo se dice 'sports' en inglés?",
      "questions": [
        {
          "question": "¿Cómo se dice 'sports' en inglés?",
          "options": [
            "[[Sports|]]",
            "[[Wrong word|]]",
            "[[Other word|]]"
          ],
          "correctAnswer": 0,
          "explanation": ""
        }
      ]
    },
    "audioUrl": "",
    "topicName": "vocabulary"
  },
  {
    "id": "a1-u32-u32_filler_34",
    "type": "multiple-choice",
    "level": "A1",
    "topic": "vocabulary",
    "difficulty": "medium",
    "transcript": "",
    "content": {
      "title": "Multiple Choice",
      "instructions": "¿Cómo se dice 'playing' en inglés?",
      "questions": [
        {
          "question": "¿Cómo se dice 'playing' en inglés?",
          "options": [
            "[[Playing|]]",
            "[[Wrong word|]]",
            "[[Other word|]]"
          ],
          "correctAnswer": 0,
          "explanation": ""
        }
      ]
    },
    "audioUrl": "",
    "topicName": "vocabulary"
  },
  {
    "id": "a1-u32-u32_filler_35",
    "type": "multiple-choice",
    "level": "A1",
    "topic": "vocabulary",
    "difficulty": "medium",
    "transcript": "",
    "content": {
      "title": "Multiple Choice",
      "instructions": "¿Cómo se dice 'football' en inglés?",
      "questions": [
        {
          "question": "¿Cómo se dice 'football' en inglés?",
          "options": [
            "[[Football|]]",
            "[[Wrong word|]]",
            "[[Other word|]]"
          ],
          "correctAnswer": 0,
          "explanation": ""
        }
      ]
    },
    "audioUrl": "",
    "topicName": "vocabulary"
  },
  {
    "id": "a1-u32-u32_filler_36",
    "type": "multiple-choice",
    "level": "A1",
    "topic": "vocabulary",
    "difficulty": "medium",
    "transcript": "",
    "content": {
      "title": "Multiple Choice",
      "instructions": "¿Cómo se dice 'tennis' en inglés?",
      "questions": [
        {
          "question": "¿Cómo se dice 'tennis' en inglés?",
          "options": [
            "[[Tennis|]]",
            "[[Wrong word|]]",
            "[[Other word|]]"
          ],
          "correctAnswer": 0,
          "explanation": ""
        }
      ]
    },
    "audioUrl": "",
    "topicName": "vocabulary"
  },
  {
    "id": "a1-u32-u32_filler_37",
    "type": "multiple-choice",
    "level": "A1",
    "topic": "vocabulary",
    "difficulty": "medium",
    "transcript": "",
    "content": {
      "title": "Multiple Choice",
      "instructions": "¿Cómo se dice 'guitar' en inglés?",
      "questions": [
        {
          "question": "¿Cómo se dice 'guitar' en inglés?",
          "options": [
            "[[Guitar|]]",
            "[[Wrong word|]]",
            "[[Other word|]]"
          ],
          "correctAnswer": 0,
          "explanation": ""
        }
      ]
    },
    "audioUrl": "",
    "topicName": "vocabulary"
  },
  {
    "id": "a1-u32-u32_filler_38",
    "type": "multiple-choice",
    "level": "A1",
    "topic": "vocabulary",
    "difficulty": "medium",
    "transcript": "",
    "content": {
      "title": "Multiple Choice",
      "instructions": "¿Cómo se dice 'piano' en inglés?",
      "questions": [
        {
          "question": "¿Cómo se dice 'piano' en inglés?",
          "options": [
            "[[Piano|]]",
            "[[Wrong word|]]",
            "[[Other word|]]"
          ],
          "correctAnswer": 0,
          "explanation": ""
        }
      ]
    },
    "audioUrl": "",
    "topicName": "vocabulary"
  },
  {
    "id": "a1-u32-u32_filler_39",
    "type": "multiple-choice",
    "level": "A1",
    "topic": "vocabulary",
    "difficulty": "medium",
    "transcript": "",
    "content": {
      "title": "Multiple Choice",
      "instructions": "¿Cómo se dice 'hobbies' en inglés?",
      "questions": [
        {
          "question": "¿Cómo se dice 'hobbies' en inglés?",
          "options": [
            "[[Hobbies|]]",
            "[[Wrong word|]]",
            "[[Other word|]]"
          ],
          "correctAnswer": 0,
          "explanation": ""
        }
      ]
    },
    "audioUrl": "",
    "topicName": "vocabulary"
  },
  {
    "id": "a1-u32-u32_filler_40",
    "type": "multiple-choice",
    "level": "A1",
    "topic": "vocabulary",
    "difficulty": "medium",
    "transcript": "",
    "content": {
      "title": "Multiple Choice",
      "instructions": "¿Cómo se dice 'sports' en inglés?",
      "questions": [
        {
          "question": "¿Cómo se dice 'sports' en inglés?",
          "options": [
            "[[Sports|]]",
            "[[Wrong word|]]",
            "[[Other word|]]"
          ],
          "correctAnswer": 0,
          "explanation": ""
        }
      ]
    },
    "audioUrl": "",
    "topicName": "vocabulary"
  },
  {
    "id": "a1-u32-u32_filler_41",
    "type": "multiple-choice",
    "level": "A1",
    "topic": "vocabulary",
    "difficulty": "medium",
    "transcript": "",
    "content": {
      "title": "Multiple Choice",
      "instructions": "¿Cómo se dice 'playing' en inglés?",
      "questions": [
        {
          "question": "¿Cómo se dice 'playing' en inglés?",
          "options": [
            "[[Playing|]]",
            "[[Wrong word|]]",
            "[[Other word|]]"
          ],
          "correctAnswer": 0,
          "explanation": ""
        }
      ]
    },
    "audioUrl": "",
    "topicName": "vocabulary"
  },
  {
    "id": "a1-u32-u32_filler_42",
    "type": "multiple-choice",
    "level": "A1",
    "topic": "vocabulary",
    "difficulty": "medium",
    "transcript": "",
    "content": {
      "title": "Multiple Choice",
      "instructions": "¿Cómo se dice 'football' en inglés?",
      "questions": [
        {
          "question": "¿Cómo se dice 'football' en inglés?",
          "options": [
            "[[Football|]]",
            "[[Wrong word|]]",
            "[[Other word|]]"
          ],
          "correctAnswer": 0,
          "explanation": ""
        }
      ]
    },
    "audioUrl": "",
    "topicName": "vocabulary"
  },
  {
    "id": "a1-u32-u32_filler_43",
    "type": "multiple-choice",
    "level": "A1",
    "topic": "vocabulary",
    "difficulty": "medium",
    "transcript": "",
    "content": {
      "title": "Multiple Choice",
      "instructions": "¿Cómo se dice 'tennis' en inglés?",
      "questions": [
        {
          "question": "¿Cómo se dice 'tennis' en inglés?",
          "options": [
            "[[Tennis|]]",
            "[[Wrong word|]]",
            "[[Other word|]]"
          ],
          "correctAnswer": 0,
          "explanation": ""
        }
      ]
    },
    "audioUrl": "",
    "topicName": "vocabulary"
  },
  {
    "id": "a1-u32-u32_filler_44",
    "type": "multiple-choice",
    "level": "A1",
    "topic": "vocabulary",
    "difficulty": "medium",
    "transcript": "",
    "content": {
      "title": "Multiple Choice",
      "instructions": "¿Cómo se dice 'guitar' en inglés?",
      "questions": [
        {
          "question": "¿Cómo se dice 'guitar' en inglés?",
          "options": [
            "[[Guitar|]]",
            "[[Wrong word|]]",
            "[[Other word|]]"
          ],
          "correctAnswer": 0,
          "explanation": ""
        }
      ]
    },
    "audioUrl": "",
    "topicName": "vocabulary"
  },
  {
    "id": "a1-u32-u32_filler_45",
    "type": "multiple-choice",
    "level": "A1",
    "topic": "vocabulary",
    "difficulty": "medium",
    "transcript": "",
    "content": {
      "title": "Multiple Choice",
      "instructions": "¿Cómo se dice 'piano' en inglés?",
      "questions": [
        {
          "question": "¿Cómo se dice 'piano' en inglés?",
          "options": [
            "[[Piano|]]",
            "[[Wrong word|]]",
            "[[Other word|]]"
          ],
          "correctAnswer": 0,
          "explanation": ""
        }
      ]
    },
    "audioUrl": "",
    "topicName": "vocabulary"
  },
  {
    "id": "a1-u32-u32_filler_46",
    "type": "multiple-choice",
    "level": "A1",
    "topic": "vocabulary",
    "difficulty": "medium",
    "transcript": "",
    "content": {
      "title": "Multiple Choice",
      "instructions": "¿Cómo se dice 'hobbies' en inglés?",
      "questions": [
        {
          "question": "¿Cómo se dice 'hobbies' en inglés?",
          "options": [
            "[[Hobbies|]]",
            "[[Wrong word|]]",
            "[[Other word|]]"
          ],
          "correctAnswer": 0,
          "explanation": ""
        }
      ]
    },
    "audioUrl": "",
    "topicName": "vocabulary"
  },
  {
    "id": "a1-u32-u32_filler_47",
    "type": "multiple-choice",
    "level": "A1",
    "topic": "vocabulary",
    "difficulty": "medium",
    "transcript": "",
    "content": {
      "title": "Multiple Choice",
      "instructions": "¿Cómo se dice 'sports' en inglés?",
      "questions": [
        {
          "question": "¿Cómo se dice 'sports' en inglés?",
          "options": [
            "[[Sports|]]",
            "[[Wrong word|]]",
            "[[Other word|]]"
          ],
          "correctAnswer": 0,
          "explanation": ""
        }
      ]
    },
    "audioUrl": "",
    "topicName": "vocabulary"
  },
  {
    "id": "a1-u32-u32_l5_i1",
    "type": "multiple-choice",
    "level": "A1",
    "topic": "Listening",
    "difficulty": "medium",
    "transcript": "The football is here.",
    "content": {
      "title": "Listening Practice",
      "instructions": "Escucha y completa la frase:",
      "questions": [
        {
          "question": "The [[football|]] is here.",
          "options": [
            "[[football|]]",
            "[[o1|]]"
          ],
          "correctAnswer": 0,
          "explanation": "Escucha atentamente para identificar la palabra correcta."
        }
      ]
    },
    "audioUrl": "/audio/courses/ingles-a1/unit32/lesson5-task-1.mp3",
    "topicName": "Listening"
  },
  {
    "id": "a1-u32-u32_l5_i2",
    "type": "multiple-choice",
    "level": "A1",
    "topic": "Listening",
    "difficulty": "medium",
    "transcript": "The tennis is here.",
    "content": {
      "title": "Listening Practice",
      "instructions": "Escucha y completa la frase:",
      "questions": [
        {
          "question": "The [[tennis|]] is here.",
          "options": [
            "[[o1|]]",
            "[[tennis|]]"
          ],
          "correctAnswer": 1,
          "explanation": "Escucha atentamente para identificar la palabra correcta."
        }
      ]
    },
    "audioUrl": "/audio/courses/ingles-a1/unit32/lesson5-task-2.mp3",
    "topicName": "Listening"
  },
  {
    "id": "a1-u32-u32_l5_i3",
    "type": "multiple-choice",
    "level": "A1",
    "topic": "Listening",
    "difficulty": "medium",
    "transcript": "The guitar is here.",
    "content": {
      "title": "Listening Practice",
      "instructions": "Escucha y completa la frase:",
      "questions": [
        {
          "question": "The [[guitar|]] is here.",
          "options": [
            "[[guitar|]]",
            "[[o1|]]"
          ],
          "correctAnswer": 0,
          "explanation": "Escucha atentamente para identificar la palabra correcta."
        }
      ]
    },
    "audioUrl": "/audio/courses/ingles-a1/unit32/lesson5-task-3.mp3",
    "topicName": "Listening"
  },
  {
    "id": "a1-u32-u32_l5_i4",
    "type": "multiple-choice",
    "level": "A1",
    "topic": "Listening",
    "difficulty": "medium",
    "transcript": "The piano is here.",
    "content": {
      "title": "Listening Practice",
      "instructions": "Escucha y completa la frase:",
      "questions": [
        {
          "question": "The [[piano|]] is here.",
          "options": [
            "[[piano|]]",
            "[[o1|]]"
          ],
          "correctAnswer": 0,
          "explanation": "Escucha atentamente para identificar la palabra correcta."
        }
      ]
    },
    "audioUrl": "/audio/courses/ingles-a1/unit32/lesson5-task-4.mp3",
    "topicName": "Listening"
  },
  {
    "id": "a1-u32-u32_l5_i5",
    "type": "multiple-choice",
    "level": "A1",
    "topic": "Listening",
    "difficulty": "medium",
    "transcript": "The hobbies is here.",
    "content": {
      "title": "Listening Practice",
      "instructions": "Escucha y completa la frase:",
      "questions": [
        {
          "question": "The [[hobbies|]] is here.",
          "options": [
            "[[football|]]",
            "[[hobbies|]]"
          ],
          "correctAnswer": 1,
          "explanation": "Escucha atentamente para identificar la palabra correcta."
        }
      ]
    },
    "audioUrl": "/audio/courses/ingles-a1/unit32/lesson5-task-5.mp3",
    "topicName": "Listening"
  },
  {
    "id": "a1-u32-u32_l5_i6",
    "type": "multiple-choice",
    "level": "A1",
    "topic": "Listening",
    "difficulty": "medium",
    "transcript": "The sports is here.",
    "content": {
      "title": "Listening Practice",
      "instructions": "Escucha y completa la frase:",
      "questions": [
        {
          "question": "The [[sports|]] is here.",
          "options": [
            "[[o2|]]",
            "[[sports|]]"
          ],
          "correctAnswer": 1,
          "explanation": "Escucha atentamente para identificar la palabra correcta."
        }
      ]
    },
    "audioUrl": "/audio/courses/ingles-a1/unit32/lesson5-task-6.mp3",
    "topicName": "Listening"
  },
  {
    "id": "a1-u32-u32_l5_i7",
    "type": "multiple-choice",
    "level": "A1",
    "topic": "Listening",
    "difficulty": "medium",
    "transcript": "The playing is here.",
    "content": {
      "title": "Listening Practice",
      "instructions": "Escucha y completa la frase:",
      "questions": [
        {
          "question": "The [[playing|]] is here.",
          "options": [
            "[[go|]]",
            "[[playing|]]"
          ],
          "correctAnswer": 1,
          "explanation": "Escucha atentamente para identificar la palabra correcta."
        }
      ]
    },
    "audioUrl": "/audio/courses/ingles-a1/unit32/lesson5-task-7.mp3",
    "topicName": "Listening"
  },
  {
    "id": "a1-u32-u32_l5_i8",
    "type": "pronunciation",
    "level": "A1",
    "topic": "Pronunciation",
    "difficulty": "medium",
    "transcript": "I like this football.",
    "content": {
      "title": "Pronunciación: Sports & Hobbies",
      "instructions": "Escucha y repite la frase enfocándote en la entonación.",
      "expectedResponse": "I like this football.",
      "phonetic": ""
    },
    "audioUrl": "/audio/courses/ingles-a1/unit32/lesson5-task-8.mp3",
    "topicName": "Pronunciation"
  },
  {
    "id": "a1-u32-u32_l5_i9",
    "type": "pronunciation",
    "level": "A1",
    "topic": "Pronunciation",
    "difficulty": "medium",
    "transcript": "I like this tennis.",
    "content": {
      "title": "Pronunciación: Sports & Hobbies",
      "instructions": "Escucha y repite la frase enfocándote en la entonación.",
      "expectedResponse": "I like this tennis.",
      "phonetic": ""
    },
    "audioUrl": "/audio/courses/ingles-a1/unit32/lesson5-task-9.mp3",
    "topicName": "Pronunciation"
  },
  {
    "id": "a1-u32-u32_l5_i10",
    "type": "pronunciation",
    "level": "A1",
    "topic": "Pronunciation",
    "difficulty": "medium",
    "transcript": "I like this guitar.",
    "content": {
      "title": "Pronunciación: Sports & Hobbies",
      "instructions": "Escucha y repite la frase enfocándote en la entonación.",
      "expectedResponse": "I like this guitar.",
      "phonetic": ""
    },
    "audioUrl": "/audio/courses/ingles-a1/unit32/lesson5-task-10.mp3",
    "topicName": "Pronunciation"
  },
  {
    "id": "a1-u32-u32_l5_i11",
    "type": "pronunciation",
    "level": "A1",
    "topic": "Pronunciation",
    "difficulty": "medium",
    "transcript": "I like this piano.",
    "content": {
      "title": "Pronunciación: Sports & Hobbies",
      "instructions": "Escucha y repite la frase enfocándote en la entonación.",
      "expectedResponse": "I like this piano.",
      "phonetic": ""
    },
    "audioUrl": "/audio/courses/ingles-a1/unit32/lesson5-task-11.mp3",
    "topicName": "Pronunciation"
  },
  {
    "id": "a1-u32-u32_l5_i12",
    "type": "pronunciation",
    "level": "A1",
    "topic": "Pronunciation",
    "difficulty": "medium",
    "transcript": "I like this hobbies.",
    "content": {
      "title": "Pronunciación: Sports & Hobbies",
      "instructions": "Escucha y repite la frase enfocándote en la entonación.",
      "expectedResponse": "I like this hobbies.",
      "phonetic": ""
    },
    "audioUrl": "/audio/courses/ingles-a1/unit32/lesson5-task-12.mp3",
    "topicName": "Pronunciation"
  },
  {
    "id": "a1-u32-u32_l5_i13",
    "type": "pronunciation",
    "level": "A1",
    "topic": "Pronunciation",
    "difficulty": "medium",
    "transcript": "I like this sports.",
    "content": {
      "title": "Pronunciación: Sports & Hobbies",
      "instructions": "Escucha y repite la frase enfocándote en la entonación.",
      "expectedResponse": "I like this sports.",
      "phonetic": ""
    },
    "audioUrl": "/audio/courses/ingles-a1/unit32/lesson5-task-13.mp3",
    "topicName": "Pronunciation"
  },
  {
    "id": "a1-u32-u32_l5_i14",
    "type": "pronunciation",
    "level": "A1",
    "topic": "Pronunciation",
    "difficulty": "medium",
    "transcript": "I like this playing.",
    "content": {
      "title": "Pronunciación: Sports & Hobbies",
      "instructions": "Escucha y repite la frase enfocándote en la entonación.",
      "expectedResponse": "I like this playing.",
      "phonetic": ""
    },
    "audioUrl": "/audio/courses/ingles-a1/unit32/lesson5-task-14.mp3",
    "topicName": "Pronunciation"
  },
  {
    "id": "a1-u32-u32_l5_i15",
    "type": "pronunciation",
    "level": "A1",
    "topic": "Pronunciation",
    "difficulty": "medium",
    "transcript": "I like this football.",
    "content": {
      "title": "Pronunciación: Sports & Hobbies",
      "instructions": "Escucha y repite la frase enfocándote en la entonación.",
      "expectedResponse": "I like this football.",
      "phonetic": ""
    },
    "audioUrl": "/audio/courses/ingles-a1/unit32/lesson5-task-15.mp3",
    "topicName": "Pronunciation"
  }
];
