import { Exercise } from "@/types/course";

export const UNIT_19_EXERCISES: Exercise[] = [
  // 1-10: Grammar - Definite Article (The)
  {
    id: "b2-u19-e1",
    type: "multiple-choice",
    level: "B2",
    topic: "Economic Trends",
    difficulty: "medium",
    transcript: "The economy of the Eurozone is recovering.",
    content: {
      title: "Advanced Articles: The",
      instructions: "Choose the correct article.",
      question: "[[___|La]] [[economy|economía]] [[of|de]] [[the|la]] [[Eurozone|Eurozona]] [[is|se está]] [[recovering.|recuperando.]]",
      options: ["The", "(zero)", "A"],
      correctAnswer: "The",
      explanation: "Use 'the' for specific economies or entities followed by 'of'.",
      translation: "La economía de la Eurozona se está recuperando.",
      hint: "Specific economy."
    },
    topicName: "Grammar"
  },
  {
    id: "b2-u19-e2",
    type: "multiple-choice",
    level: "B2",
    topic: "Economic Trends",
    difficulty: "medium",
    transcript: "The IMF has predicted a slowdown.",
    content: {
      title: "Advanced Articles: The",
      instructions: "Choose the correct article.",
      question: "[[___|El]] [[IMF|FMI]] [[has|ha]] [[predicted|predicho]] [[a|una]] [[slowdown.|desaceleración.]]",
      options: ["(zero)", "An", "The"],
      correctAnswer: "The",
      explanation: "Use 'the' for abbreviations of international organizations.",
      translation: "El FMI ha predicho una desaceleración.",
      hint: "International organization."
    },
    topicName: "Grammar"
  },
  {
    id: "b2-u19-e3",
    type: "multiple-choice",
    level: "B2",
    topic: "Economic Trends",
    difficulty: "medium",
    transcript: "The United States is a major player in trade.",
    content: {
      title: "Advanced Articles: The",
      instructions: "Choose the correct article.",
      question: "[[___|El]] [[United|Estados]] [[States|Unidos]] [[is|es]] [[a|un]] [[major|actor]] [[player|importante]] [[in|en]] [[trade.|el comercio.]]",
      options: ["A", "The", "(zero)"],
      correctAnswer: "The",
      explanation: "Use 'the' with country names that are plural or contain 'United/Republic/Kingdom'.",
      translation: "Estados Unidos es un actor importante en el comercio.",
      hint: "Country name with 'United'."
    },
    topicName: "Grammar"
  },
  {
    id: "b2-u19-e4",
    type: "multiple-choice",
    level: "B2",
    topic: "Economic Trends",
    difficulty: "medium",
    transcript: "The Pacific Ocean is vast.",
    content: {
      title: "Advanced Articles: The",
      instructions: "Choose the correct article.",
      question: "[[___|El]] [[Pacific|Océano]] [[Ocean|Pacífico]] [[is|es]] [[vast.|vasto.]]",
      options: ["A", "(zero)", "The"],
      correctAnswer: "The",
      explanation: "Use 'the' for names of oceans and seas.",
      translation: "El Océano Pacífico es vasto.",
      hint: "Name of an ocean."
    },
    topicName: "Grammar"
  },
  {
    id: "b2-u19-e5",
    type: "multiple-choice",
    level: "B2",
    topic: "Economic Trends",
    difficulty: "medium",
    transcript: "We visited the Netherlands last year.",
    content: {
      title: "Advanced Articles: The",
      instructions: "Choose the correct article.",
      question: "[[We|Nosotros]] [[visited|visitamos]] [[___|los]] [[Netherlands|Países Bajos]] [[last|el año]] [[year.|pasado.]]",
      options: ["a", "the", "(zero)"],
      correctAnswer: "the",
      explanation: "Some country names like 'The Netherlands' always use 'the'.",
      translation: "Visitamos los Países Bajos el año pasado.",
      hint: "Specific country name."
    },
    topicName: "Grammar"
  },
  {
    id: "b2-u19-e6",
    type: "multiple-choice",
    level: "B2",
    topic: "Economic Trends",
    difficulty: "medium",
    transcript: "The rich should pay more taxes.",
    content: {
      title: "Advanced Articles: The",
      instructions: "Choose the correct article.",
      question: "[[___|Los]] [[rich|ricos]] [[should|deberían]] [[pay|pagar]] [[more|más]] [[taxes.|impuestos.]]",
      options: ["A", "The", "(zero)"],
      correctAnswer: "The",
      explanation: "Use 'the + adjective' to refer to a whole group of people.",
      translation: "Los ricos deberían pagar más impuestos.",
      hint: "Group of people."
    },
    topicName: "Grammar"
  },
  {
    id: "b2-u19-e7",
    type: "multiple-choice",
    level: "B2",
    topic: "Economic Trends",
    difficulty: "medium",
    transcript: "He is the best economist in the country.",
    content: {
      title: "Advanced Articles: The",
      instructions: "Choose the correct article.",
      question: "[[He|Él]] [[is|es]] [[___|el]] [[best|mejor]] [[economist|economista]] [[in|en]] [[the|el]] [[country.|país.]]",
      options: ["an", "a", "the"],
      correctAnswer: "the",
      explanation: "Always use 'the' with superlative adjectives.",
      translation: "Es el mejor economista del país.",
      hint: "Superlative form."
    },
    topicName: "Grammar"
  },
  {
    id: "b2-u19-e8",
    type: "multiple-choice",
    level: "B2",
    topic: "Economic Trends",
    difficulty: "medium",
    transcript: "I live in the city center.",
    content: {
      title: "Advanced Articles: The",
      instructions: "Choose the correct article.",
      question: "[[I|Yo]] [[live|vivo]] [[in|en]] [[___|el]] [[city|centro]] [[center.|de la ciudad.]]",
      options: ["(zero)", "the", "a"],
      correctAnswer: "the",
      explanation: "Use 'the' for specific parts of a town or city.",
      translation: "Vivo en el centro de la ciudad.",
      hint: "Specific location."
    },
    topicName: "Grammar"
  },
  {
    id: "b2-u19-e9",
    type: "multiple-choice",
    level: "B2",
    topic: "Economic Trends",
    difficulty: "medium",
    transcript: "The environment is a global priority.",
    content: {
      title: "Advanced Articles: The",
      instructions: "Choose the correct article.",
      question: "[[___|El]] [[environment|medio ambiente]] [[is|es]] [[a|una]] [[global|prioridad]] [[priority.|global.]]",
      options: ["The", "An", "(zero)"],
      correctAnswer: "The",
      explanation: "Use 'the' for the environment as a unique global concept.",
      translation: "El medio ambiente es una prioridad global.",
      hint: "Unique concept."
    },
    topicName: "Grammar"
  },
  {
    id: "b2-u19-e10",
    type: "multiple-choice",
    level: "B2",
    topic: "Economic Trends",
    difficulty: "medium",
    transcript: "The sun rises in the east.",
    content: {
      title: "Advanced Articles: The",
      instructions: "Choose the correct article.",
      question: "[[___|El]] [[sun|sol]] [[rises|sale]] [[in|por]] [[the|el]] [[east.|este.]]",
      options: ["The", "(zero)", "A"],
      correctAnswer: "The",
      explanation: "Use 'the' for unique objects in nature.",
      translation: "El sol sale por el este.",
      hint: "Unique object."
    },
    topicName: "Grammar"
  },

  // 11-20: Grammar - Indefinite and Zero Article
  {
    id: "b2-u19-e11",
    type: "multiple-choice",
    level: "B2",
    topic: "Economic Trends",
    difficulty: "hard",
    transcript: "Globalization has changed the world.",
    content: {
      title: "Zero Article",
      instructions: "Choose 'a', 'an' or 'no article'.",
      question: "[[___ (zero)| ]] [[globalization|globalización]] [[has|ha]] [[changed|cambiado]] [[the|el]] [[world.|mundo.]]",
      options: ["[[a|un]]", "[[an|un/una]]", "[[ (no article)|(sin artículo)]]"],
      correctAnswer: "",
      explanation: "Use the zero article for abstract nouns in a general sense.",
      translation: "La globalización ha cambiado el mundo.",
      hint: "General concept."
    },
    topicName: "Grammar"
  },
  {
    id: "b2-u19-e12",
    type: "multiple-choice",
    level: "B2",
    topic: "Economic Trends",
    difficulty: "hard",
    transcript: "He is an expert in macroeconomics.",
    content: {
      title: "Indefinite Article",
      instructions: "Choose 'a', 'an' or 'no article'.",
      question: "[[He|Él]] [[is|es]] [[___ (an)|un]] [[expert|experto]] [[in|en]] [[macroeconomics.|macroeconomía.]]",
      options: ["[[a|un]]", "[[an|un]]", "[[ (no article)|(sin artículo)]]"],
      correctAnswer: "an",
      explanation: "Use 'an' before a noun starting with a vowel sound.",
      translation: "Es un experto en macroeconomía.",
      hint: "Vowel sound."
    },
    topicName: "Grammar"
  },
  {
    id: "b2-u19-e13",
    type: "multiple-choice",
    level: "B2",
    topic: "Economic Trends",
    difficulty: "hard",
    transcript: "Interest rates are low.",
    content: {
      title: "Zero Article",
      instructions: "Choose 'a', 'an' or 'no article'.",
      question: "[[___ (zero)| ]] [[interest|tasas]] [[rates|de interés]] [[are|son]] [[low.|bajas.]]",
      options: ["[[a|una]]", "[[an|un/una]]", "[[ (no article)|(sin artículo)]]"],
      correctAnswer: "",
      explanation: "Use zero article for plural countable nouns in a general sense.",
      translation: "Las tasas de interés son bajas.",
      hint: "General plural."
    },
    topicName: "Grammar"
  },
  {
    id: "b2-u19-e14",
    type: "multiple-choice",
    level: "B2",
    topic: "Economic Trends",
    difficulty: "hard",
    transcript: "I have a question about the budget.",
    content: {
      title: "Indefinite Article",
      instructions: "Choose 'a', 'an' or 'no article'.",
      question: "[[I|Yo]] [[have|tengo]] [[___ (a)|una]] [[question|pregunta]] [[about|sobre]] [[the|el]] [[budget.|presupuesto.]]",
      options: ["[[a|una]]", "[[an|una]]", "[[ (no article)|(sin artículo)]]"],
      correctAnswer: "a",
      explanation: "Use 'a' for singular countable nouns starting with a consonant sound.",
      translation: "Tengo una pregunta sobre el presupuesto.",
      hint: "Consonant sound."
    },
    topicName: "Grammar"
  },
  {
    id: "b2-u19-e15",
    type: "multiple-choice",
    level: "B2",
    topic: "Economic Trends",
    difficulty: "hard",
    transcript: "Money doesn't buy happiness.",
    content: {
      title: "Zero Article",
      instructions: "Choose 'a', 'an' or 'no article'.",
      question: "[[___ (zero)| ]] [[money|dinero]] [[doesn't|no]] [[buy|compra]] [[happiness.|felicidad.]]",
      options: ["[[a|un]]", "[[an|un/una]]", "[[ (no article)|(sin artículo)]]"],
      correctAnswer: "",
      explanation: "Use zero article for uncountable nouns in a general sense.",
      translation: "El dinero no compra la felicidad.",
      hint: "General uncountable."
    },
    topicName: "Grammar"
  },
  {
    id: "b2-u19-e16",
    type: "multiple-choice",
    level: "B2",
    topic: "Economic Trends",
    difficulty: "hard",
    transcript: "She is a consultant for the government.",
    content: {
      title: "Indefinite Article",
      instructions: "Choose 'a', 'an' or 'no article'.",
      question: "[[She|Ella]] [[is|es]] [[___ (a)|una]] [[consultant|consultora]] [[for|para]] [[the|el]] [[government.|gobierno.]]",
      options: ["[[a|una]]", "[[an|una]]", "[[ (no article)|(sin artículo)]]"],
      correctAnswer: "a",
      explanation: "Use 'a' for singular jobs/professions.",
      translation: "Ella es consultora para el gobierno.",
      hint: "Profession."
    },
    topicName: "Grammar"
  },
  {
    id: "b2-u19-e17",
    type: "multiple-choice",
    level: "B2",
    topic: "Economic Trends",
    difficulty: "hard",
    transcript: "We need information before we decide.",
    content: {
      title: "Zero Article",
      instructions: "Choose 'a', 'an' or 'no article'.",
      question: "[[We|Necesitamos]] [[need| ]] [[___ (zero)| ]] [[information|información]] [[before|antes]] [[we|de]] [[decide.|decidir.]]",
      options: ["[[a|una]]", "[[an|una]]", "[[ (no article)|(sin artículo)]]"],
      correctAnswer: "",
      explanation: "'Information' is uncountable and used here in a general sense.",
      translation: "Necesitamos información antes de decidir.",
      hint: "Uncountable general."
    },
    topicName: "Grammar"
  },
  {
    id: "b2-u19-e18",
    type: "multiple-choice",
    level: "B2",
    topic: "Economic Trends",
    difficulty: "hard",
    transcript: "Technology is evolving fast.",
    content: {
      title: "Zero Article",
      instructions: "Choose 'a', 'an' or 'no article'.",
      question: "[[___ (zero)| ]] [[technology|tecnología]] [[is|está]] [[evolving|evolucionando]] [[fast.|rápido.]]",
      options: ["[[a|una]]", "[[an|una]]", "[[ (no article)|(sin artículo)]]"],
      correctAnswer: "",
      explanation: "Use zero article for abstract nouns like 'technology' in general.",
      translation: "La tecnología está evolucionando rápido.",
      hint: "General abstract."
    },
    topicName: "Grammar"
  },
  {
    id: "b2-u19-e19",
    type: "multiple-choice",
    level: "B2",
    topic: "Economic Trends",
    difficulty: "hard",
    transcript: "It was an honor to meet him.",
    content: {
      title: "Indefinite Article",
      instructions: "Choose 'a', 'an' or 'no article'.",
      question: "[[It|Fue]] [[was| ]] [[___ (an)|un]] [[honor|honor]] [[to| ]] [[meet|conocerle.]] [[him.| ]]",
      options: ["[[a|un]]", "[[an|un]]", "[[ (no article)|(sin artículo)]]"],
      correctAnswer: "an",
      explanation: "Use 'an' before 'honor' because the 'h' is silent and it starts with a vowel sound.",
      translation: "Fue un honor conocerle.",
      hint: "Silent 'h'."
    },
    topicName: "Grammar"
  },
  {
    id: "b2-u19-e20",
    type: "multiple-choice",
    level: "B2",
    topic: "Economic Trends",
    difficulty: "hard",
    transcript: "People are worried about inflation.",
    content: {
      title: "Zero Article",
      instructions: "Choose 'a', 'an' or 'no article'.",
      question: "[[___ (zero)| ]] [[people|gente]] [[are|está]] [[worried|preocupada]] [[about|por]] [[inflation.|inflación.]]",
      options: ["[[a|una]]", "[[an|un/una]]", "[[ (no article)|(sin artículo)]]"],
      correctAnswer: "",
      explanation: "Use zero article for people in general.",
      translation: "La gente está preocupada por la inflación.",
      hint: "General group."
    },
    topicName: "Grammar"
  },

  // 21-30: Vocabulary - Economic Trends
  {
    id: "b2-u19-e21",
    type: "multiple-choice",
    level: "B2",
    topic: "Economic Trends",
    difficulty: "medium",
    transcript: "The country is currently in a recession.",
    content: {
      title: "Economic Terms",
      instructions: "Choose the correct word.",
      question: "[[The|El]] [[country|país]] [[is|se encuentra]] [[currently|actualmente]] [[in|en]] [[a|una]] [[___ (recession).|recesión.]]",
      options: ["[[recession|recesión]]", "[[reception|recepción]]", "[[regression|regresión]]"],
      correctAnswer: "recession",
      explanation: "A recession is a period of temporary economic decline.",
      translation: "El país se encuentra actualmente en recesión.",
      hint: "Economic decline."
    },
    topicName: "Vocabulary"
  },
  {
    id: "b2-u19-e22",
    type: "multiple-choice",
    level: "B2",
    topic: "Economic Trends",
    difficulty: "medium",
    transcript: "The inflation rate has reached 5%.",
    content: {
      title: "Economic Terms",
      instructions: "Choose the correct word.",
      question: "[[The|La]] [[___ (inflation)|tasa de inflación]] [[rate| ]] [[has|ha]] [[reached|alcanzado]] [[5%.|el 5%.]]",
      options: ["[[inflation|inflación]]", "[[inflationary|inflacionaria]]", "[[inflating|inflando]]"],
      correctAnswer: "inflation",
      explanation: "Inflation is the rate at which the general level of prices is rising.",
      translation: "La tasa de inflación ha alcanzado el 5%.",
      hint: "Rising prices."
    },
    topicName: "Vocabulary"
  },
  {
    id: "b2-u19-e23",
    type: "multiple-choice",
    level: "B2",
    topic: "Economic Trends",
    difficulty: "medium",
    transcript: "We have seen a growth in GDP.",
    content: {
      title: "Economic Terms",
      instructions: "Choose the correct word.",
      question: "[[We|Nosotros]] [[have|hemos]] [[seen|visto]] [[a|un]] [[___ (growth)|crecimiento]] [[in|en]] [[GDP.|el PIB.]]",
      options: ["[[growth|crecimiento]]", "[[grow|crecer]]", "[[grown|crecido]]"],
      correctAnswer: "growth",
      explanation: "Growth refers to an increase in economic activity.",
      translation: "Hemos visto un crecimiento en el PIB.",
      hint: "Economic increase."
    },
    topicName: "Vocabulary"
  },
  {
    id: "b2-u19-e24",
    type: "multiple-choice",
    level: "B2",
    topic: "Economic Trends",
    difficulty: "medium",
    transcript: "The unemployment rate is falling.",
    content: {
      title: "Economic Terms",
      instructions: "Choose the correct word.",
      question: "[[The|La]] [[___ (unemployment)|tasa de desempleo]] [[rate| ]] [[is|está]] [[falling.|bajando.]]",
      options: ["[[unemployment|desempleo]]", "[[employment|empleo]]", "[[employee|empleado]]"],
      correctAnswer: "unemployment",
      explanation: "Unemployment is the state of not having a job.",
      translation: "La tasa de desempleo está bajando.",
      hint: "Not having jobs."
    },
    topicName: "Vocabulary"
  },
  {
    id: "b2-u19-e25",
    type: "multiple-choice",
    level: "B2",
    topic: "Economic Trends",
    difficulty: "medium",
    transcript: "The stock market is very volatile.",
    content: {
      title: "Economic Terms",
      instructions: "Choose the correct word.",
      question: "[[The|El]] [[stock|mercado]] [[___ (market)|de valores]] [[is|es]] [[very|muy]] [[volatile.|volátil.]]",
      options: ["[[market|mercado]]", "[[shop|tienda]]", "[[street|calle]]"],
      correctAnswer: "market",
      explanation: "The stock market is where shares of companies are traded.",
      translation: "El mercado de valores es muy volátil.",
      hint: "Where shares are traded."
    },
    topicName: "Vocabulary"
  },
  {
    id: "b2-u19-e26",
    type: "multiple-choice",
    level: "B2",
    topic: "Economic Trends",
    difficulty: "medium",
    transcript: "Governments are worried about the national debt.",
    content: {
      title: "Economic Terms",
      instructions: "Choose the correct word.",
      question: "[[Governments|Los gobiernos]] [[are|están]] [[worried|preocupados]] [[about|por]] [[the|la]] [[national|deuda]] [[___ (debt).|nacional.]]",
      options: ["[[debt|deuda]]", "[[debit|débito]]", "[[doubt|duda]]"],
      correctAnswer: "debt",
      explanation: "National debt is the total amount of money a country's government has borrowed.",
      translation: "Los gobiernos están preocupados por la deuda nacional.",
      hint: "Borrowed money."
    },
    topicName: "Vocabulary"
  },
  {
    id: "b2-u19-e27",
    type: "multiple-choice",
    level: "B2",
    topic: "Economic Trends",
    difficulty: "medium",
    transcript: "Interest rates have increased again.",
    content: {
      title: "Economic Terms",
      instructions: "Choose the correct word.",
      question: "[[___ (Interest)|Las tasas de interés]] [[rates| ]] [[have|han]] [[increased|vuelto a subir.]] [[again.| ]]",
      options: ["[[Interest|Interés]]", "[[Internal|Interno]]", "[[Investment|Inversión]]"],
      correctAnswer: "Interest",
      explanation: "Interest rates are the percentage of a loan that is charged as interest to the borrower.",
      translation: "Las tasas de interés han vuelto a subir.",
      hint: "Cost of borrowing."
    },
    topicName: "Vocabulary"
  },
  {
    id: "b2-u19-e28",
    type: "multiple-choice",
    level: "B2",
    topic: "Economic Trends",
    difficulty: "medium",
    transcript: "The standard of living is improving.",
    content: {
      title: "Economic Terms",
      instructions: "Choose the correct word.",
      question: "[[The|El]] [[___ (standard)|nivel]] [[of|de]] [[living|vida]] [[is|está]] [[improving.|mejorando.]]",
      options: ["[[standard|nivel / estándar]]", "[[banner|pancarta]]", "[[level|nivel (general)]]"],
      correctAnswer: "standard",
      explanation: "Standard of living is the level of wealth, comfort, and services available to a person or group.",
      translation: "El nivel de vida está mejorando.",
      hint: "Level of comfort/wealth."
    },
    topicName: "Vocabulary"
  },
  {
    id: "b2-u19-e29",
    type: "multiple-choice",
    level: "B2",
    topic: "Economic Trends",
    difficulty: "medium",
    transcript: "Fiscal policy is used to control the economy.",
    content: {
      title: "Economic Terms",
      instructions: "Choose the correct word.",
      question: "[[___ (Fiscal)|La política fiscal]] [[policy| ]] [[is|se]] [[used|utiliza]] [[to|para]] [[control|controlar]] [[the|la]] [[economy.|economía.]]",
      options: ["[[Fiscal|Fiscal]]", "[[Financial|Financiera]]", "[[Physical|Física]]"],
      correctAnswer: "Fiscal",
      explanation: "Fiscal policy refers to the use of government spending and tax policies to influence economic conditions.",
      translation: "La política fiscal se utiliza para controlar la economía.",
      hint: "Related to gov spending/tax."
    },
    topicName: "Vocabulary"
  },
  {
    id: "b2-u19-e30",
    type: "multiple-choice",
    level: "B2",
    topic: "Economic Trends",
    difficulty: "medium",
    transcript: "The currency has weakened against the dollar.",
    content: {
      title: "Economic Terms",
      instructions: "Choose the correct word.",
      question: "[[The|La]] [[___ (currency)|moneda]] [[has|se ha]] [[weakened|debilitado]] [[against|frente]] [[the|al]] [[dollar.|dólar.]]",
      options: ["[[currency|moneda / divisa]]", "[[current|actual]]", "[[correction|corrección]]"],
      correctAnswer: "currency",
      explanation: "A currency is a system of money in general use in a particular country.",
      translation: "La moneda se ha debilitado frente al dólar.",
      hint: "Money system."
    },
    topicName: "Vocabulary"
  },

  // 31-40: Vocabulary - Globalization
  {
    id: "b2-u19-e31",
    type: "multiple-choice",
    level: "B2",
    topic: "Economic Trends",
    difficulty: "medium",
    transcript: "Globalization has led to more competition.",
    content: {
      title: "Globalization",
      instructions: "Choose the correct term.",
      question: "[[___|La globalización]] [[has|ha]] [[led|llevado]] [[to|a]] [[more|más]] [[competition|competencia]] [[and|y]] [[global|interconexión]] [[interconnectedness.|global.]]",
      options: ["Localization", "Globalization", "Nationalization"],
      correctAnswer: "Globalization",
      explanation: "Globalization is the process by which businesses or other organizations develop international influence.",
      translation: "La globalización ha llevado a más competencia e interconexión global.",
      hint: "Global interconnectedness."
    },
    topicName: "Vocabulary"
  },
  {
    id: "b2-u19-e32",
    type: "multiple-choice",
    level: "B2",
    topic: "Economic Trends",
    difficulty: "medium",
    transcript: "We need a free trade agreement.",
    content: {
      title: "Globalization",
      instructions: "Choose the correct term.",
      question: "[[We|Necesitamos]] [[need|un]] [[a|acuerdo de]] [[___|libre]] [[trade|comercio]] [[agreement| ]] [[to|para]] [[reduce|reducir]] [[tariffs.|aranceles.]]",
      options: ["free", "open", "clear"],
      correctAnswer: "free",
      explanation: "A free trade agreement is a pact between countries to reduce or eliminate barriers to trade.",
      translation: "Necesitamos un acuerdo de libre comercio para reducir los aranceles.",
      hint: "No trade barriers."
    },
    topicName: "Vocabulary"
  },
  {
    id: "b2-u19-e33",
    type: "multiple-choice",
    level: "B2",
    topic: "Economic Trends",
    difficulty: "medium",
    transcript: "The supply chain was disrupted.",
    content: {
      title: "Globalization",
      instructions: "Choose the correct term.",
      question: "[[The|La]] [[___|cadena de suministro]] [[chain| ]] [[was|se]] [[disrupted|interrumpió]] [[due|debido]] [[to|a]] [[the|la]] [[crisis.|crisis.]]",
      options: ["supply", "long", "fast"],
      correctAnswer: "supply",
      explanation: "A supply chain is the sequence of processes involved in the production and distribution of a commodity.",
      translation: "La cadena de suministro se interrumpió debido a la crisis.",
      hint: "Production/distribution sequence."
    },
    topicName: "Vocabulary"
  },
  {
    id: "b2-u19-e34",
    type: "multiple-choice",
    level: "B2",
    topic: "Economic Trends",
    difficulty: "medium",
    transcript: "He works for a multinational corporation.",
    content: {
      title: "Globalization",
      instructions: "Choose the correct term.",
      question: "[[He|Él]] [[works|trabaja]] [[for|para]] [[a|una]] [[___|corporación multinacional]] [[corporation| ]] [[with|con]] [[offices|oficinas]] [[in|en]] [[20|20]] [[countries.|países.]]",
      options: ["global", "big", "multinational"],
      correctAnswer: "multinational",
      explanation: "A multinational corporation has facilities and other assets in at least one country other than its home country.",
      translation: "Él trabaja para una corporación multinacional con oficinas en 20 países.",
      hint: "Operating in many nations."
    },
    topicName: "Vocabulary"
  },
  {
    id: "b2-u19-e35",
    type: "multiple-choice",
    level: "B2",
    topic: "Economic Trends",
    difficulty: "medium",
    transcript: "Outsourcing can reduce costs.",
    content: {
      title: "Globalization",
      instructions: "Choose the correct term.",
      question: "[[___|La subcontratación]] [[can|puede]] [[reduce|reducir]] [[costs|los costes]] [[by| ]] [[moving|al trasladar]] [[tasks|tareas]] [[to|a]] [[other|otros]] [[countries.|países.]]",
      options: ["Outsourcing", "Buying", "In-sourcing"],
      correctAnswer: "Outsourcing",
      explanation: "Outsourcing is obtaining goods or a service from an outside or foreign supplier.",
      translation: "La subcontratación puede reducir los costes al trasladar tareas a otros países.",
      hint: "Using foreign suppliers."
    },
    topicName: "Vocabulary"
  },
  {
    id: "b2-u19-e36",
    type: "multiple-choice",
    level: "B2",
    topic: "Economic Trends",
    difficulty: "medium",
    transcript: "Emerging markets are growing fast.",
    content: {
      title: "Globalization",
      instructions: "Choose the correct term.",
      question: "[[___|Los mercados emergentes]] [[markets| ]] [[are|están]] [[growing|creciendo]] [[fast|rápido]] [[and|y]] [[attracting|atrayendo]] [[investors.|inversores.]]",
      options: ["New", "Emerging", "Growing"],
      correctAnswer: "Emerging",
      explanation: "An emerging market is the economy of a developing nation that is becoming more engaged with global markets.",
      translation: "Los mercados emergentes están creciendo rápido y atrayendo inversores.",
      hint: "Developing economies."
    },
    topicName: "Vocabulary"
  },
  {
    id: "b2-u19-e37",
    type: "multiple-choice",
    level: "B2",
    topic: "Economic Trends",
    difficulty: "medium",
    transcript: "We need to reduce trade barriers.",
    content: {
      title: "Globalization",
      instructions: "Choose the correct term.",
      question: "[[We|Necesitamos]] [[need|reducir]] [[to| ]] [[reduce|las]] [[trade|barreras]] [[___|comerciales]] [[to|para]] [[encourage|fomentar el]] [[commerce.|comercio.]]",
      options: ["doors", "walls", "barriers"],
      correctAnswer: "barriers",
      explanation: "Trade barriers are government-induced restrictions on international trade.",
      translation: "Necesitamos reducir las barreras comerciales para fomentar el comercio.",
      hint: "Obstacles to trade."
    },
    topicName: "Vocabulary"
  },
  {
    id: "b2-u19-e38",
    type: "multiple-choice",
    level: "B2",
    topic: "Economic Trends",
    difficulty: "medium",
    transcript: "The import tax has increased.",
    content: {
      title: "Globalization",
      instructions: "Choose the correct term.",
      question: "[[The|El]] [[___|impuesto de importación]] [[tax| ]] [[has|ha]] [[increased,|aumentado,]] [[making|haciendo que]] [[foreign|los productos extranjeros]] [[goods| ]] [[more|sean más]] [[expensive.|caros.]]",
      options: ["import", "buy", "input"],
      correctAnswer: "import",
      explanation: "An import tax is a tax collected on goods brought in from another country.",
      translation: "El impuesto de importación ha aumentado, haciendo que los productos extranjeros sean más caros.",
      hint: "Tax on goods from abroad."
    },
    topicName: "Vocabulary"
  },
  {
    id: "b2-u19-e39",
    type: "multiple-choice",
    level: "B2",
    topic: "Economic Trends",
    difficulty: "medium",
    transcript: "Exports have reached an all-time high.",
    content: {
      title: "Globalization",
      instructions: "Choose the correct term.",
      question: "[[___|Las exportaciones]] [[have|han]] [[reached|alcanzado]] [[an|un]] [[all-time|máximo]] [[high|histórico]] [[this|este]] [[year.|año.]]",
      options: ["Outputs", "Sales", "Exports"],
      correctAnswer: "Exports",
      explanation: "Exports are goods or services sent to another country for sale.",
      translation: "Las exportaciones han alcanzado un máximo histórico este año.",
      hint: "Goods sent abroad."
    },
    topicName: "Vocabulary"
  },
  {
    id: "b2-u19-e40",
    type: "multiple-choice",
    level: "B2",
    topic: "Economic Trends",
    difficulty: "medium",
    transcript: "We live in a borderless world.",
    content: {
      title: "Globalization",
      instructions: "Choose the correct term.",
      question: "[[Digital|La tecnología]] [[technology|digital]] [[helps|nos ayuda a]] [[us|vivir]] [[live|en]] [[in|un]] [[a|mundo]] [[___|sin fronteras.]] [[world.| ]]",
      options: ["free", "borderless", "limitless"],
      correctAnswer: "borderless",
      explanation: "A borderless world is one where information and commerce move freely without physical or digital restrictions.",
      translation: "La tecnología digital nos ayuda a vivir en un mundo sin fronteras.",
      hint: "Without borders."
    },
    topicName: "Vocabulary"
  },

  // 41-50: Word Formation
  {
    id: "b2-u19-e41",
    type: "word-formation",
    level: "B2",
    topic: "Economic Trends",
    difficulty: "medium",
    transcript: "The economic situation is stable.",
    content: {
      title: "Word Formation",
      instructions: "Transform the word in brackets.",
      question: "[[The ___ (ECONOMY) situation is stable.|La situación económica es estable.]]",
      correctAnswer: "economic",
      explanation: "Adjective form of 'economy'.",
      translation: "La situación económica es estable.",
      hint: "Related to economy."
    },
    topicName: "Vocabulary"
  },
  {
    id: "b2-u19-e42",
    type: "word-formation",
    level: "B2",
    topic: "Economic Trends",
    difficulty: "medium",
    transcript: "Global dependence is increasing.",
    content: {
      title: "Word Formation",
      instructions: "Transform the word in brackets.",
      question: "[[Global ___ (DEPEND) is increasing.|La dependencia global está aumentando.]]",
      correctAnswer: "dependence",
      explanation: "Noun form of 'depend'.",
      translation: "La dependencia global está aumentando.",
      hint: "Noun from depend."
    },
    topicName: "Vocabulary"
  },
  {
    id: "b2-u19-e43",
    type: "word-formation",
    level: "B2",
    topic: "Economic Trends",
    difficulty: "medium",
    transcript: "The stability of the market is at risk.",
    content: {
      title: "Word Formation",
      instructions: "Transform the word in brackets.",
      question: "[[The ___ (STABLE) of the market is at risk.|La estabilidad del mercado está en riesgo.]]",
      correctAnswer: "stability",
      explanation: "Noun form of 'stable'.",
      translation: "La estabilidad del mercado está en riesgo.",
      hint: "Noun from stable."
    },
    topicName: "Vocabulary"
  },
  {
    id: "b2-u19-e44",
    type: "word-formation",
    level: "B2",
    topic: "Economic Trends",
    difficulty: "medium",
    transcript: "We need sustainable growth.",
    content: {
      title: "Word Formation",
      instructions: "Transform the word in brackets.",
      question: "[[We need ___ (SUSTAIN) growth.|Necesitamos un crecimiento sostenible.]]",
      correctAnswer: "sustainable",
      explanation: "Adjective form of 'sustain'.",
      translation: "Necesitamos un crecimiento sostenible.",
      hint: "Able to be maintained."
    },
    topicName: "Vocabulary"
  },
  {
    id: "b2-u19-e45",
    type: "word-formation",
    level: "B2",
    topic: "Economic Trends",
    difficulty: "medium",
    transcript: "The inequality between countries is huge.",
    content: {
      title: "Word Formation",
      instructions: "Transform the word in brackets.",
      question: "[[The ___ (INEQUAL) between countries is huge.|La desigualdad entre países es enorme.]]",
      correctAnswer: "inequality",
      explanation: "Noun form related to 'equal/unequal'.",
      translation: "La desigualdad entre países es enorme.",
      hint: "Lack of equality."
    },
    topicName: "Vocabulary"
  },
  {
    id: "b2-u19-e46",
    type: "word-formation",
    level: "B2",
    topic: "Economic Trends",
    difficulty: "medium",
    transcript: "The development of the region is impressive.",
    content: {
      title: "Word Formation",
      instructions: "Transform the word in brackets.",
      question: "[[The ___ (DEVELOP) of the region is impressive.|El desarrollo de la región es impresionante.]]",
      correctAnswer: "development",
      explanation: "Noun form of 'develop'.",
      translation: "El desarrollo de la región es impresionante.",
      hint: "Noun from develop."
    },
    topicName: "Vocabulary"
  },
  {
    id: "b2-u19-e47",
    type: "word-formation",
    level: "B2",
    topic: "Economic Trends",
    difficulty: "medium",
    transcript: "He is a very influential economist.",
    content: {
      title: "Word Formation",
      instructions: "Transform the word in brackets.",
      question: "[[He is a very ___ (INFLUENCE) economist.|Es un economista muy influyente.]]",
      correctAnswer: "influential",
      explanation: "Adjective form of 'influence'.",
      translation: "Es un economista muy influyente.",
      hint: "Having influence."
    },
    topicName: "Vocabulary"
  },
  {
    id: "b2-u19-e48",
    type: "word-formation",
    level: "B2",
    topic: "Economic Trends",
    difficulty: "medium",
    transcript: "The productivity of the factory is rising.",
    content: {
      title: "Word Formation",
      instructions: "Transform the word in brackets.",
      question: "[[The ___ (PRODUCT) of the factory is rising.|La productividad de la fábrica está aumentando.]]",
      correctAnswer: "productivity",
      explanation: "Noun form of 'product/productive'.",
      translation: "La productividad de la fábrica está aumentando.",
      hint: "Efficiency of production."
    },
    topicName: "Vocabulary"
  },
  {
    id: "b2-u19-e48",
    type: "word-formation",
    level: "B2",
    topic: "Economic Trends",
    difficulty: "medium",
    transcript: "We need to improve our competitiveness.",
    content: {
      title: "Word Formation",
      instructions: "Transform the word in brackets.",
      question: "[[We need to improve our ___ (COMPETE).|Necesitamos mejorar nuestra competitividad.]]",
      correctAnswer: "competitiveness",
      explanation: "Noun form of 'compete' (state of being competitive).",
      translation: "Necesitamos mejorar nuestra competitividad.",
      hint: "Ability to compete."
    },
    topicName: "Vocabulary"
  },
  {
    id: "b2-u19-e50",
    type: "word-formation",
    level: "B2",
    topic: "Economic Trends",
    difficulty: "medium",
    transcript: "The reduction in poverty is a goal.",
    content: {
      title: "Word Formation",
      instructions: "Transform the word in brackets.",
      question: "[[The ___ (REDUCE) in poverty is a goal.|La reducción de la pobreza es un objetivo.]]",
      correctAnswer: "reduction",
      explanation: "Noun form of 'reduce'.",
      translation: "La reducción de la pobreza es un objetivo.",
      hint: "Noun from reduce."
    },
    topicName: "Vocabulary"
  }
];
